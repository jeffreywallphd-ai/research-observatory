...
slice_id: CAP-08.S05
title: Evidence matrix and source-first analysis UI
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Slice | `CAP-08.S05` — Evidence matrix and source-first analysis UI |
| Slice outcome | Researchers can inspect, filter, compare, pivot, correct, and trace evidence at scale. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-08.S05.T01`, `CAP-08.S05.T02`, `CAP-08.S05.T03` |
| Slice dependencies | `CAP-08.S04.T02`, `CAP-01.S02.T01` |
...
Researchers can inspect, filter, compare, pivot, correct, and trace evidence at scale.
...
**Implementation thesis.** Make the Evidence Matrix the source-first operational surface for reviewing large structured corpora: every cell remains one interaction from its passage, status history, alternative candidates and correction/recomputation path.
...
- Virtualized scalable evidence matrix and saved views.
- Source inspection, alternative candidate and confidence interaction.
- Bulk review/correction/recomputation with preview and audit.

### 2.2 Explicit non-goals

- Spreadsheet-like direct mutation that bypasses domain commands.
- Showing generated values without evidence/status.
- Loading an entire corpus into the browser.
...
- **Consumes:** `CAP-08.S04.T02`, `CAP-01.S02.T01` and the handoffs in Section 13.
- **Produces:** Researchers can inspect, filter, compare, pivot, correct, and trace evidence at scale.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-08.S04.T02`, `CAP-01.S02.T01`.
...
|---|---|
| `packages/ui/evidence-matrix` | Virtual grid, filters, saved views, selection and status components. |
| `packages/ui/document-reader` | Shared anchored source details integration. |
| `services/core/research_observatory/modules/evidence_graph/queries.py` | Cursor/filter/aggregate projections. |
| `services/core/research_observatory/modules/evidence_graph/bulk_commands.py` | Preview/apply/cancel/recompute workflows. |
| `packages/contracts/evidence-matrix` | View/filter/selection/bulk operation schemas. |
| `tests/e2e/evidence-matrix` | Keyboard, source, bulk, stale and recovery flows. |
...
|---|---|
| `EvidenceViewDefinition` | columns, filters, sort, grouping, schema version and ownership |
| `EvidenceMatrixPage` | cursor, row projections, aggregates and snapshot revision |
| `BulkEvidenceOperation` | selection query/snapshot, command, preview counts, authorization, status and evidence |
| `CorrectionCandidate` | old/new value, source/rationale, affected dependencies and approval |
...
- `EvidenceQuery.page(view, cursor, snapshot)` returns stable projections.
- `BulkOperation.preview` freezes selection by snapshot/query hash before `apply`.
- Source drawer resolves immutable anchors and current document availability.
- Routes/regions conform to approved `evidence-matrix.html` and workflow catalog.
...
- Saved views include verified, disputed, needs adjudication, high consequence and stale.
- Status is conveyed by text/icon as well as color; headers, grid navigation and drawer meet WCAG/ARIA patterns.
- Bulk actions show exact count, sample, downstream staleness and reversibility before confirmation.
- Offline and unavailable-source states retain metadata/history and offer repair routes.
...
- Queries enforce project/field access in core; hidden columns are not delivered to client.
- Exports and bulk operations reauthorize at execution, not only preview.
- Source content is rendered safely and never executed as HTML/script.
...
|---|---|
| Cursor snapshot changed | Require refresh or return explicit stale-page token; do not apply to unseen records. |
| Bulk partial failure | Use durable batches, checkpoint and per-record result; safe retry is idempotent. |
| Anchor cannot render | Show evidence metadata and route to parsing correction. |
| UI crash/reload | Restore saved view and operation status; no duplicate command. |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S05.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `desktop`, `evidence`, `documents` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S05.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `desktop`, `evidence`, `service` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S05.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
3. Implement the approved Academic Minimal route and workflow state using shared components. Cover keyboard/focus, screen reader names, light/dark, loading, empty, offline, denied, stale, error and recovery states; update and approve the reference before any intentional UX divergence.
4. Exercise security, privacy, rights and research-integrity boundaries with malformed/untrusted inputs, authorization denial, confidential-content redaction and unsupported-state fixtures. Failure must leave canonical state unchanged or exactly recoverable.
5. Run the task verification profiles plus targeted unit, contract, integration and end-to-end tests. Capture named reports, manifests, performance evidence and changed-contract hashes against the reviewed commit.
...
|---|---|
| Initial grid | Usable first page < 1.5 s local for 100,000 evidence records. |
| Scroll/filter | p95 query < 500 ms; no DOM growth beyond virtualization window. |
| Source drawer | Open cached page/passages < 300 ms; uncached parse/render progressive. |
| Bulk preview | 100,000-record selection impact < 10 s with progress. |
...
- view/filter latency
- source opens/anchor failures
- bulk preview/apply/cancel
- corrections
- status transitions
- accessibility/keyboard test results
...
- Consumes S03 authoritative evidence and S04 verification plus CAP-05 source viewer.
- Produces researcher-reviewed/adjudicated evidence for CAP-09 graph/synthesis.
- Shared UI components support later technical reports/results without conflating evidence classes.
...
| Risk | Mitigation |
|---|---|
| Dense UX overload | Progressive detail, saved workflow views and source-first defaults. |
| Mass accidental change | Snapshot preview, explicit confirmation and durable operation. |
| Accessibility failure in virtual grid | ARIA grid pattern, keyboard tests and alternative table/export view. |

## 19. Required ADRs and human decisions

- ADR-CAP08-MATRIX: grid/query architecture and accessible interaction model.

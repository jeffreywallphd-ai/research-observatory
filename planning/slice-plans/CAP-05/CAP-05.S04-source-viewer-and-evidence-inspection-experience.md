...
2. **Serve PDF bytes through authenticated/range-capable Core endpoints or a Tauri custom protocol with project/document authorization; never `file://` paths.**
...
- ADR: Authenticated range stream versus Tauri custom protocol for local PDF bytes.
- ADR: PDF.js sandbox/CSP and external-action policy.
- ADR: Quotation/copy/export enforcement boundaries.
...
2. Confirm `CAP-05` is the active capability campaign and `CAP-05.S04` is the next eligible slice.
3. Claim the first READY task in order: `CAP-05.S04.T01`. Do not globally select work outside the capability.

...
slice_id: CAP-04.S03
title: Canonical work, version, and identity reconciliation
status: proposed
wave: W2
priority: P0
deployment_profiles:
...
| Slice | `CAP-04.S03` - Canonical work, version, and identity reconciliation |
| Slice outcome | Multiple provider records resolve to inspectable canonical scholarly entities without flattening uncertainty. |
| Wave / priority | `W2` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-04.S03.T01`, `CAP-04.S03.T02`, `CAP-04.S03.T03` |
| Slice dependencies | `CAP-04.S01.T03`, `CAP-04.S02.T03` |
...
Create a defensible canonical scholarly record while preserving competing source assertions, versions, corrections, retractions, and uncertain identities.
...
**Implementation thesis.** Normalize identifiers conservatively, auto-merge only exact/high-certainty cases, generate explainable duplicate candidates for human review, and model work/version/update relationships separately from source assertions.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-04.S01.T03`, `CAP-04.S02.T03`.
- **Produces:** Multiple provider records resolve to inspectable canonical scholarly entities without flattening uncertainty.
...
- `planning/backlog.yaml` for `CAP-04.S03` and its task state/dependencies.
...
- `CAP-04.S01.T03` is complete or explicitly gated.
- `CAP-04.S02.T03` is complete or explicitly gated.
...
1. **A canonical Work represents the intellectual work; WorkVersion represents manifestations such as preprint, accepted manuscript, version of record, correction, or retraction notice.**
2. **Normalize DOI according to DOI syntax, plus PMID/arXiv/OpenAlex/S2/ISBN and other typed identifiers; preserve original value and source.**
3. **Exact verified identifier matches may auto-link; conflicting verified identifiers block auto-merge.**
4. **Probabilistic matching uses explainable features such as normalized title, authors, year, venue, pages, and references; scores are candidates, not probabilities.**
5. **Merges are reversible through source assertions and alias/tombstone mapping; no source value is destroyed.**
6. **Retractions/corrections are typed relationships and status assertions with source/date, not destructive replacement.**
...
- Identifier value objects and normalization/validation library.
- SourceAssertion, Work, WorkVersion, Agent/Organization identity models.
- Exact reconciliation and conflict engine.
- Duplicate candidate generator, feature explanation, review/adjudication UI, merge/split service.
- Version/update/correction/retraction relation service.
...
- Canonical fields are derived/accepted assertions with provenance, not untraceable copied strings.
- Merge command requires compared identities, evidence/features, surviving ID, alias plan, conflict disposition, and actor.
- Split reconstructs independent records and recalculates downstream membership/dependencies.
...
- Reconciliation presents side-by-side source assertions, exact identifiers, feature contributions, conflicts, citations, and affected downstream objects.
- Bulk acceptance is limited to homogeneous high-certainty groups with audit sample.
- Retraction/correction status is prominent but does not erase the historical record.
...
- Untrusted metadata is escaped and never rendered as executable markup.
- Identity review access follows project permissions; external identity lookup is policy-controlled.
- Avoid sensitive-person inference beyond provided scholarly identity fields.
...
- Test identifier conflict, homonymous authors, translated titles, conference/journal versions, title change, missing year, group authors, correction, retraction, merge then split, and concurrent decisions.
- A failed merge leaves source/canonical records unchanged.
- Downstream updates use dependency graph and are resumable.
...
### 9.1 `CAP-04.S03.T01` - Implement identifier normalization and exact reconciliation
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S03.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-04.S03.T02` - Implement probabilistic duplicate candidate generation and review
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S03.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
### 9.3 `CAP-04.S03.T03` - Implement work-version, correction, and retraction relationships
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S03.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
4. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
5. Select affected unit/contract/integration checks under the workflow verification-breadth rule; record selected/deferred coverage. Full listed profiles remain slice/Wave coverage, not an automatic replay after every task edit. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
|---|---|
| Backlog profiles | `data`, `desktop`, `graph`, `service` |
...
- Use blocking/indexing to avoid all-pairs comparisons; benchmark candidate recall on gold fixture clusters.
- Feature computation is cached by source-record revision.
- Provide review queues prioritized by expected impact and uncertainty.
...
- Track auto-link counts, candidate volumes, feature distributions, merge/split decisions, conflict rates, review agreement, and later correction rates.
- Retain algorithm/version and threshold/config with every candidate.
...
- CAP-04.S01/S02 provide source records.
- CAP-04.S04 applies corpus membership to canonical works/versions.
- CAP-05 references and documents link to specific versions.
- CAP-10 search indexes canonical records while retaining source provenance.
...
- Matching algorithm changes create new candidate sets, not retroactive silent merges.
- Canonical alias and source assertion history support schema/identifier evolution.
...
- The promised outcome is demonstrable end to end: Multiple provider records resolve to inspectable canonical scholarly entities without flattening uncertainty.
...
- Binding detail: Work versus WorkVersion model and version taxonomy.
- Binding detail: Exact auto-link rules and probabilistic candidate thresholds.
- Binding detail: Reversible merge/split implementation.
...
| `ORCID_API` | [ORCID Public API](https://info.orcid.org/documentation/features/public-api/) - ORCID | Researcher identity reconciliation. |
| `ROR_API` | [ROR REST API](https://ror.readme.io/docs/rest-api) - Research Organization Registry | Organization identity reconciliation. |
...
2. Confirm W2 is the approved active campaign and `CAP-04.S03` is dependency-eligible.
3. Claim only the next READY task in `CAP-04.S03` through taskctl; do not bypass Wave dependencies or create a capability lease.

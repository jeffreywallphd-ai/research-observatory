...
- ADR: Parsing quality metric definitions and triage thresholds.
- ADR: Correction patch/overlay and acceptance model.
- ADR: Parser upgrade diff and batch current-revision switch policy.
...
2. Confirm `CAP-05` is the active capability campaign and `CAP-05.S06` is the next eligible slice.
3. Claim the first READY task in order: `CAP-05.S06.T01`. Do not globally select work outside the capability.

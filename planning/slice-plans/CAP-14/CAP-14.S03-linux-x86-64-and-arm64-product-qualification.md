...
slice_id: CAP-14.S03
title: Linux x86_64 and ARM64 product qualification
status: proposed
wave: W6
priority: P0
deployment_profiles:
...
| Slice | `CAP-14.S03` — Linux x86_64 and ARM64 product qualification |
| Slice outcome | The local product is production-ready on approved Ubuntu-compatible Linux workstation profiles. |
| Wave / priority | `W6` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `linux-x64`, `linux-arm64` |
| Backlog tasks | `CAP-14.S03.T01`, `CAP-14.S03.T02`, `CAP-14.S03.T03` |
| Slice dependencies | `CAP-14.S01.T03` |
...
The local product is production-ready on approved Ubuntu-compatible Linux workstation profiles.
...
**Implementation thesis.** Qualify Linux x86_64 and ARM64 with portable packaging, Secret Service/XDG integration and explicit distribution/environment support boundaries.
...
- Appimage and debian package pipelines.
- Secret service/xdg/process adapters.
- X86_64 and arm64 install/update/offline/accessibility/recovery.
...
- **Consumes:** `platform/build matrix` and formal dependencies `CAP-14.S01.T03`.
- **Produces:** The local product is production-ready on approved Ubuntu-compatible Linux workstation profiles.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-14.S01.T03`.
...
|---|---|---|---|---|
| `CAP-14-D04` | Linux packaging | **Provide AppImage plus Debian package baseline, with signed/checksummed metadata** | Choose one distribution-specific package and call Linux complete | The baseline balances portability and managed Ubuntu/Debian lab deployment. | ADR-LINUX-PACKAGING |
| `CAP-14-D05` | Credential storage | **Use Credential Manager, Keychain and Secret Service through one secrets port** | Store credentials in project configuration files | Each OS provides protected credential facilities with different APIs. | None |
| `CAP-14-D06` | Filesystem layout | **Use platform-native data/config/cache directories and architecture-neutral project-relative paths** | Reuse Windows paths and separators on every OS | Correct platform directories avoid permissions, cleanup and portability defects. | None |
| `CAP-14-D15` | Accessibility parity | **Repeat keyboard, screen-reader, scaling and contrast qualification on each OS** | Assume Windows accessibility results transfer unchanged | Assistive technology and webview/platform behavior differ by OS. | None |
...
|---|---|
| `packaging/linux` | AppImage/deb and signed metadata |
| `platform/linux` | XDG, Secret Service and process lifecycle |
| `tests/release/linux` | Ubuntu/DGX OS qualification |
...
|---|---|
| `LinuxReleaseArtifact` | Versioned LinuxReleaseArtifact state with stable identity, lifecycle/status, provenance and policy metadata appropriate to linux x86_64 and arm64 product qualification. |
| `DesktopIntegration` | Versioned DesktopIntegration state with stable identity, lifecycle/status, provenance and policy metadata appropriate to linux x86_64 and arm64 product qualification. |
| `LinuxCredentialReference` | Versioned LinuxCredentialReference state with stable identity, lifecycle/status, provenance and policy metadata appropriate to linux x86_64 and arm64 product qualification. |
| `DistributionProfile` | Versioned DistributionProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to linux x86_64 and arm64 product qualification. |
| `LinuxQualificationRun` | Versioned LinuxQualificationRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to linux x86_64 and arm64 product qualification. |
...
- `LinuxPackageBuilder.build/verify` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `LinuxSecretStore get/set/delete` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `LinuxReleaseGate.evaluate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** platform/build matrix.
- **Downstream:** DGX Spark runtime and portable project equivalence.
...
- Use XDG locations and desktop integration while keeping project locations user-governed.
- If Secret Service is unavailable, fail safely and explain supported remediation.
- Expose distribution/architecture/runtime compatibility in diagnostics.
...
- Packages and repositories/manifests are signed/checksummed.
- Do not fall back to plaintext secrets.
- Symlinks, executable bits and sandbox permissions are validated.
...
|---|---|---|
| FUSE/AppImage unavailable | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Secret Service daemon absent | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Wayland/X11/webview difference | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| ARM64 native dependency missing | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| Clean Ubuntu x64 and ARM64 install/package tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Desktop entry, MIME/file association and uninstall | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Offline workflows, updates, backup/restore, crash recovery | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Orca/keyboard/scaling and denied-secret behavior | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Supported distribution baseline explicit | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Native ARM64 execution without x86 emulation | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Package size/startup measured separately by format | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
platform/build matrix. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

DGX Spark runtime and portable project equivalence. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-14/index.html`; this slice page is `planning/review-site/CAP-14/CAP-14.S03.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

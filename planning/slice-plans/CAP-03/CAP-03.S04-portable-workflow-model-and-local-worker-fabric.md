...
slice_id: CAP-03.S04
title: Portable workflow model and local worker fabric
status: approved
wave: W1
priority: P0
deployment_profiles:
...
| Slice | `CAP-03.S04` - Portable workflow model and local worker fabric |
| Slice outcome | Long-running processes execute as durable, inspectable workflows instead of opaque UI calls. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-03.S04.T01`, `CAP-03.S04.T02`, `CAP-03.S04.T03` |
...
Run long local ingestion, parsing, indexing, extraction, drafting, and analysis reliably across application restarts while retaining human gates and later portability to hosted workflow infrastructure.
...
**Implementation thesis.** Define portable workflow-plan contracts and implement a SQLite-backed at-least-once local executor with explicit activities, attempts, checkpoints, artifacts, retries, cancellation, leases, and human tasks. Keep domain activities idempotent and compatible with a future Temporal adapter.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-03.S03.T02`, `CAP-02.S02.T03`.
- **Produces:** Long-running processes execute as durable, inspectable workflows instead of opaque UI calls.
...
- `planning/backlog.yaml` for `CAP-03.S04` and its task state/dependencies.
...
- `CAP-03.S03.T02` is complete or explicitly gated.
- `CAP-02.S02.T03` is complete or explicitly gated.
...
1. **Workflow definitions are versioned data/contracts, not arbitrary Python closures or UI state.**
2. **Use at-least-once activity execution with idempotency keys and durable checkpoints; never claim exactly-once side effects.**
3. **Separate workflow, step, job, attempt, artifact, and human-task identities/states.**
4. **Use SQLite queue tables plus atomic claim/lease/heartbeat and a supervised worker pool.**
5. **Cancellation is cooperative first, with bounded termination and explicit partial-artifact disposition.**
6. **Design activity signatures and history so a later server executor can implement the same semantics; do not emulate Temporal internals.**
...
- Workflow-plan JSON Schema and registry.
- Local queue repository, scheduler, worker supervisor, lease/heartbeat/retry engine, and checkpoint store.
- Activity SDK with idempotency, progress, cancellation, artifact, provenance, and resource-budget context.
- Task Center UI for queues, attempts, logs, human gates, cancellation, retry, and recovery.
...
- State machines cover pending, runnable, claimed, running, waiting-human, retry-scheduled, cancelling, cancelled, failed, succeeded, and superseded.
- Retry policy declares retryable error classes, max attempts, backoff/jitter, and non-retryable conditions.
- Artifacts are immutable references; accepted human decisions are never overwritten on replay.
- Progress is monotonic within an attempt and overall state is reconciled from durable history.
...
- Task Center shows what is running, why, governing project/intent, resources, progress confidence, current step, retry/cancel implications, and outputs.
- Human gates include evidence, options, consequences, and an explicit resume decision.
- App restart reconnects to durable operations rather than showing them as newly started.
...
- Activities receive capability-scoped services and declared network/model/file permissions.
- Do not allow workflow definitions to execute arbitrary shell commands.
- Worker logs are structured/redacted and governed by project sensitivity.
...
- Test crash before/after claim, after side effect/before completion, expired lease, worker loss, process restart, poison job, cancellation during parser/model call, and human-gate abandonment.
- Idempotency records and artifact hashes reconcile ambiguous completion.
- A quarantined job does not block unrelated project work.
...
### 9.1 `CAP-03.S04.T01` - Define workflow, step, job, attempt, artifact, and human-task contracts
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S04.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-03.S04.T02` - Implement the local durable queue and worker supervisor
...
**Risk / review gate:** `high` / `agent-review`  
**Verification profiles:** `service`, `data`, `e2e-local`

**Expected deliverables**
- SQLite-backed queue, leases, concurrency classes, heartbeat, retry, checkpoint, cancellation, and crash recovery.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S04.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-03.S04.T03` - Build task center, progress, cancellation, and human-gate UI
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S04.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
|---|---|
| Backlog profiles | `data`, `desktop`, `e2e-local`, `service` |
...
- Separate interactive, document, AI, and maintenance worker pools/limits.
- Use backpressure and admission control based on CPU, memory, GPU, disk, and project quotas.
- Queue polling/claiming should be efficient under idle conditions and thousands of jobs.
...
- Record queue depth, age, lease expiration, attempt duration, retry causes, cancellation latency, resource use, workflow/version, and artifact lineage.
- Every attempt carries trace/correlation and provenance activity identifiers.
...
- CAP-05 parsing, CAP-08 extraction, CAP-12 models, CAP-17 report integration, and CAP-18 drafting run through this fabric.
- CAP-03.S05 registers dependencies on workflow artifacts.
- Hosted W10 executor implements the same workflow-plan/activity contracts.
...
- Workflow definition upgrades do not mutate running histories; new runs use the new version unless an explicit compatible migration is implemented.
- Local executor schema migrations preserve runnable/recovery states.
...
- The promised outcome is demonstrable end to end: Long-running processes execute as durable, inspectable workflows instead of opaque UI calls.
...
- ADR: Local durable queue claim/lease algorithm.
- ADR: Workflow-plan and activity compatibility contract for hosted Temporal/equivalent.
- ADR: Worker resource-pool and cancellation policy.
...
|---|---|---|
| `TEMPORAL` | [Temporal Documentation](https://docs.temporal.io/) - Temporal | Compatibility target for hosted durable workflows. |
...
2. Confirm `CAP-03` is the active capability campaign and `CAP-03.S04` is the next eligible slice.
3. Claim the first READY task in order: `CAP-03.S04.T01`. Do not globally select work outside the capability.

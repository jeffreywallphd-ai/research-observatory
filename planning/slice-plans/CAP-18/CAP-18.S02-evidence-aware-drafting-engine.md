...
slice_id: CAP-18.S02
title: Evidence-aware drafting engine
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-18.S02` — Evidence-aware drafting engine |
| Slice outcome | Generated prose is section-specific, source-grounded, inspectable, and unable to conceal unsupported content. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-18.S02.T01`, `CAP-18.S02.T02`, `CAP-18.S02.T03` |
| Slice dependencies | `CAP-18.S01.T03`, `CAP-17.S06.T03` |
...
Generated prose is section-specific, source-grounded, inspectable, and unable to conceal unsupported content.
...
**Implementation thesis.** Generate small, inspectable draft candidates from deterministic section evidence packets and maintain a claim-level support map.
...
- Section-specific evidence packet builder.
- Paragraph/block candidate generation.
- Claim/evidence/interpretation map.
- Unsupported-content, citation and result-integrity controls.
...
- **Consumes:** `CAP-18.S01 draft/section state, CAP-09 literature evidence, CAP-17 verified results and CAP-16 claim plans` and formal dependencies `CAP-18.S01.T03`, `CAP-17.S06.T03`.
- **Produces:** Generated prose is section-specific, source-grounded, inspectable, and unable to conceal unsupported content.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-18.S01.T03`, `CAP-17.S06.T03`.
...
| `CAP-18-D06` | Evidence packet construction | **Build deterministic section-specific evidence packets from approved claims, literature, reports and researcher memos** | Let the model search the full project without an explicit packet | A bounded packet improves reproducibility, rights control and citation completeness. | None |
...
|---|---|
| `packages/core/drafting/evidence-packets` | deterministic packet assembly |
| `workers/manuscript-drafting` | schema-constrained candidate generation |
| `workers/manuscript-verification` | claim/source/result checks |
| `services/core-api/manuscripts/candidates` | candidate lifecycle and acceptance |
...
|---|---|
| `DraftEvidencePacket` | Versioned DraftEvidencePacket state with stable identity, lifecycle/status, provenance and policy metadata appropriate to evidence-aware drafting engine. |
| `DraftCandidate` | Versioned DraftCandidate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to evidence-aware drafting engine. |
| `CandidateClaim` | Versioned CandidateClaim state with stable identity, lifecycle/status, provenance and policy metadata appropriate to evidence-aware drafting engine. |
| `CandidateCitation` | Versioned CandidateCitation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to evidence-aware drafting engine. |
| `CandidateVerification` | Versioned CandidateVerification state with stable identity, lifecycle/status, provenance and policy metadata appropriate to evidence-aware drafting engine. |
| `UnsupportedContentFinding` | Versioned UnsupportedContentFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to evidence-aware drafting engine. |
...
- `Candidate prose is never canonical until accepted by a human-authorized action.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Every claim-bearing candidate has typed dependencies or an explicit unsupported/interpretive state.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Evidence packet manifests are immutable and reproducible.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-18.S01 draft/section state, CAP-09 literature evidence, CAP-17 verified results and CAP-16 claim plans.
- **Downstream:** CAP-18.S03/S04 manuscript-type drafting and CAP-18.S05 Studio.
...
- Show candidate beside evidence packet and verification findings.
- Offer accept/edit/reject/compare without hiding uncertainty.
...
- Enforce provider egress per source/report/manuscript policy.
- Exclude raw restricted text from logs and telemetry.
...
|---|---|---|
| model returns invalid block schema | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| citation does not entail claim | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| evidence packet exceeds context budget | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| user cancels drafting operation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | manuscript, evidence, ai |
...
| Review gate | `agent-review` |
| Verification profiles | manuscript, ai, evidence |
...
| Verification profiles | manuscript, ai, security-local |
...
|---|---|---|
| packet reproducibility/hash tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| claim-source entailment and citation completeness | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| unsupported-content denial | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| cancel/retry/restart with no duplicate acceptance | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| packet selection obeys token/evidence budget | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| candidate latency/cost reported by model profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| local fallback produces degraded but valid workflow | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-18.S01 draft/section state, CAP-09 literature evidence, CAP-17 verified results and CAP-16 claim plans. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-18.S03/S04 manuscript-type drafting and CAP-18.S05 Studio. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-18/index.html`; this slice page is `planning/review-site/CAP-18/CAP-18.S02.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

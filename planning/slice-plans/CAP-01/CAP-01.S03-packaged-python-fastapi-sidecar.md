...
slice_id: CAP-01.S03
title: Packaged Python/FastAPI sidecar
status: approved
wave: W1
priority: P0
deployment_profiles:
...
| Slice | `CAP-01.S03` - Packaged Python/FastAPI sidecar |
| Slice outcome | The desktop bundles and supervises a compatible local service with no user-managed Python installation. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-01.S03.T01`, `CAP-01.S03.T02`, `CAP-01.S03.T03` |
| Slice dependencies | `CAP-00.S03.T02`, `CAP-01.S01.T01` |
...
Provide a local analytical runtime that is invisible in normal use, diagnosable on failure, replaceable at the process boundary, and later portable to macOS/Linux.
...
**Implementation thesis.** Package the FastAPI modular monolith as a Tauri external binary. Use lifespan-managed resources, a machine-readable startup handshake, and OS process containment so the desktop owns exactly one supervised Core process tree.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-00.S03.T02`, `CAP-01.S01.T01`.
- **Produces:** The desktop bundles and supervises a compatible local service with no user-managed Python installation.
...
- `planning/backlog.yaml` for `CAP-01.S03` and its task state/dependencies.
...
- `CAP-01.S01.T01` is complete or explicitly gated.
...
1. **Use FastAPI/Pydantic as the local Core API and package it per target triple using PyInstaller onedir by default; benchmark onefile only as an optional distribution optimization.**
2. **Start the service on loopback with an OS-assigned ephemeral port and emit one bounded startup handshake over stdout or an inherited pipe.**
3. **Manage database pools, worker supervisor, temporary directories, and shutdown using FastAPI lifespan.**
4. **Use Windows Job Objects so desktop termination closes the complete sidecar process tree; design the supervisor interface for later macOS/Linux adapters.**
5. **Implement explicit health, readiness, version, and capability endpoints; a listening socket alone is not readiness.**
6. **Do not install or depend on a system Python environment.**
...
- `services/core-api` FastAPI composition root and domain-module routers.
- `services/core-api/bootstrap` environment validation, profile loading, and handshake output.
- `apps/desktop/src-tauri/runtime` process supervisor, job object adapter, logs, restart policy, and handshake parser.
- Platform packaging pipeline producing target-triple-named external binaries.
...
- Handshake includes protocol version, build ID, PID, bound host/port, nonce, capability set, database compatibility range, and startup diagnostics code.
- Exit codes are stable and classified: clean shutdown, configuration error, compatibility error, recoverable runtime failure, integrity/security failure.
- The supervisor owns one restart budget and never enters an unbounded crash loop.
- The service rejects non-loopback bind configuration in local profile.
...
- Normal startup exposes no terminal window.
- Slow startup shows named phases rather than a generic spinner.
- Recovery UI offers retry, open diagnostics, safe mode, repair prerequisites, and quit; it never asks users to run Python manually.
...
- Bundle only required Python modules and native libraries; generate an SBOM and scan the artifact.
- Set restrictive child environment variables and working directory; do not inherit secrets not required by Core.
- Redact launch tokens and research content from stdout/stderr logs.
- Verify the sidecar hash/signature as part of release qualification.
...
- Timeout startup, malformed handshake, early exit, crash after readiness, hung shutdown, and orphaned child processes are distinct tested cases.
- After a crash, preserve the project database and offer a bounded restart; run integrity checks before resuming writes when indicated.
- On version mismatch, fail closed with upgrade/repair guidance rather than attempting partial operation.
...
### 9.1 `CAP-01.S03.T01` - Create the modular FastAPI service skeleton
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S03.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-01.S03.T02` - Package the Python service as a Windows sidecar artifact
...
**Verification profiles:** `service`, `desktop`

**Expected deliverables**
- Repeatable sidecar build including pinned Python runtime/dependencies and required local helper binaries.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S03.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-01.S03.T03` - Implement sidecar lifecycle supervision in Tauri
...
**Risk / review gate:** `high` / `agent-review`  
**Verification profiles:** `desktop`, `service`, `e2e-local`

**Expected deliverables**
- Desktop-controlled start, health polling, graceful stop, crash detection, bounded restart, and log collection.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S03.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
|---|---|
| Backlog profiles | `desktop`, `e2e-local`, `service` |
...
- Median Core readiness target <=3 seconds after process spawn on the minimum Windows profile with no project open.
- Idle memory target is recorded and regression-bounded; startup must not eagerly load parser/ML frameworks.
- Shutdown target <=5 seconds before force termination, with durable work checkpointed first.
...
- Correlate desktop process-attempt ID with Core startup traces and support-bundle artifacts.
- Capture startup phase timings, exit reason, restart count, binary hash, and version; redact payload content.
...
- CAP-01.S04 authenticates and generates the API client after the handshake.
- CAP-02 supplies protected database and object-store initialization.
- CAP-03.S04 supplies durable workers, replacing bootstrap-only worker placeholders.
- CAP-14 adds platform packaging/supervision adapters without changing the Core contract.
...
- The sidecar handshake is versioned independently from domain API versions.
- Desktop and sidecar release artifacts are a tested compatibility unit; permit only explicitly declared compatibility ranges.
...
- The promised outcome is demonstrable end to end: The desktop bundles and supervises a compatible local service with no user-managed Python installation.
...
- ADR: Python sidecar packager and onedir/onefile decision.
- ADR: Cross-platform process-supervision interface.
...
| `FASTAPI_LIFESPAN` | [FastAPI Lifespan Events](https://fastapi.tiangolo.com/advanced/events/) - FastAPI | Deterministic service startup and shutdown. |
| `PYINSTALLER` | [PyInstaller Documentation](https://pyinstaller.org/en/stable/) - PyInstaller | Platform-specific packaging of Python sidecars. |
| `WINDOWS_JOBS` | [Windows Job Objects](https://learn.microsoft.com/en-us/windows/win32/procthread/job-objects) - Microsoft | Containment and shutdown of the Windows sidecar process tree. |
...
2. Confirm `CAP-01` is the active capability campaign and `CAP-01.S03` is the next eligible slice.
3. Claim the first READY task in order: `CAP-01.S03.T01`. Do not globally select work outside the capability.

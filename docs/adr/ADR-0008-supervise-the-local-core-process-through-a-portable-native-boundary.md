...
fixed local profile, starts without a terminal window, and assigns the process to
a Windows Job Object configured with `KILL_ON_JOB_CLOSE`. A single strict JSON
...
Concurrent start requests return the same active process. Graceful shutdown uses
the inherited control pipe and has a five-second force-termination boundary.
...
opaque codes only. The renderer can start, poll, retry, stop, and request redacted
...
- real packaged-sidecar supervision check for duplicate start, graceful stop,
  crash observation, PID replacement, and restart-budget exhaustion;

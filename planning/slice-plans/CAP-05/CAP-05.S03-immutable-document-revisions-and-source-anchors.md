...
| Slice | `CAP-05.S03` - Immutable document revisions and source anchors |
| Slice outcome | Every extracted passage and downstream assertion points to a specific immutable revision and stable location. |
| Wave / priority | `W2` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-05.S03.T01`, `CAP-05.S03.T02`, `CAP-05.S03.T03` |
| Slice dependencies | `CAP-05.S02.T03`, `CAP-02.S02.T03` |
...
Ensure that every extracted statement, claim, table, figure, correction, and manuscript citation can resolve to a specific immutable source representation and stable location.
...
**Implementation thesis.** Commit accepted normalized content as immutable DocumentRevisions and represent anchors with multiple complementary selectors: revision ID, structural node, page, normalized bounding region, text position, and text quote/context. Resolve with explicit confidence and mark broken dependencies stale.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-05.S02.T03`, `CAP-02.S02.T03`.
- **Produces:** Every extracted passage and downstream assertion points to a specific immutable revision and stable location.
...
- `planning/backlog.yaml` for `CAP-05.S03` and its task state/dependencies.
...
- `CAP-05.S02.T03` is complete or explicitly gated.
...
1. **Separate Document identity, DocumentSource, ParseAttempt, and immutable DocumentRevision.**
2. **A reparse/correction creates a new revision; prior revisions and anchors remain queryable.**
3. **Use revision-scoped stable IDs for structural nodes; never reuse IDs across different content.**
4. **Model anchors after W3C Web Annotation SpecificResource/Selector concepts using multiple selectors for resilience.**
5. **Coordinates use page number plus normalized [0,1] rectangles with declared origin/rotation; text selectors use normalized exact/prefix/suffix and position.**
6. **Anchor resolution returns exact, fallback, ambiguous, missing, or migrated-candidate status with confidence and evidence.**
...
- Document/revision structural schema and repositories.
- Revision commit validator and immutable persistence service.
- SourceAnchor schema, builder, normalizer, resolver, and migration candidate service.
- Anchor/context/citation-link APIs and broken-anchor dependency integration.
...
- DocumentRevision records source object, parse attempt, IR/schema, parser/config, content/structure hashes, accepted by/time, rights, and supersession relation.
- Anchor target always includes document revision and one or more selectors; no floating anchor to mutable current document.
- Resolution returns the selected source context plus which selector succeeded and any discrepancy.
- Broken/ambiguous resolution cannot be silently treated as verified evidence.
...
- Deep links visibly identify document revision and resolution confidence.
- When coordinates are unavailable, structured text fallback is explicit.
- Broken anchors show the last known source, reason, affected objects, and repair/migration actions.
...
- Anchor APIs enforce project/document rights and return only permitted context length.
- Quote selectors are protected research content and excluded from telemetry/support bundles by default.
- No direct decrypted file path is included in anchor/deep-link URIs.
...
- Test repeated text, normalization differences, rotated/cropped pages, missing coordinates, document revision replacement, project relocation, corrupt structural node, and concurrent reparse.
- Resolver never silently jumps to a low-confidence different passage.
- Anchor commit and dependent object commit occur atomically or through an idempotent two-phase staging record.
...
### 9.1 `CAP-05.S03.T01` - Persist normalized sections, blocks, sentences, and references
...
**Verification profiles:** `documents`, `data`

**Expected deliverables**
- Canonical document-revision schema and repositories for structural hierarchy, ordering, labels, text, and reference links.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S03.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-05.S03.T02` - Implement page, bounding-box, and text-span anchors
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S03.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-05.S03.T03` - Implement anchor resolution and citation-link APIs
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S03.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
4. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
5. Select affected unit/contract/integration checks under the workflow verification-breadth rule; record selected/deferred coverage. Full listed profiles remain slice/Wave coverage, not an automatic replay after every task edit. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
|---|---|
| Backlog profiles | `data`, `desktop`, `documents`, `service` |
...
- Index revision/node/page/text position and precompute normalized text structures.
- Resolve common structural/page anchors in <=100 ms local p95; quote search may be bounded/paged.
- Do not load full PDF bytes for text-only context resolution.
...
- Record resolution method/status/latency, selector disagreement, broken/ambiguous rates, migration acceptance, and downstream staleness.
- Track anchor coverage by parser/format/quality class.
...
- CAP-05.S04 viewer consumes anchors.
- CAP-05.S05 citation/table/figure objects use anchors.
- CAP-08 evidence and all later synthesis/drafting cite anchors.
- CAP-03.S05 marks dependents stale on broken/changed anchors.
...
- Anchor schema adds selectors without invalidating existing ones.
- Revision migration creates explicit candidate mappings reviewed before dependent objects adopt the new revision.
...
- The promised outcome is demonstrable end to end: Every extracted passage and downstream assertion points to a specific immutable revision and stable location.
...
- Binding detail: Multi-selector source-anchor schema and normalization rules.
- Binding detail: Document revision commit/acceptance semantics.
- Binding detail: Anchor resolution confidence and migration policy.
...
2. Confirm W2 is the approved active campaign and `CAP-05.S03` is dependency-eligible.
3. Claim only the next READY task in `CAP-05.S03` through taskctl; do not bypass Wave dependencies or create a capability lease.

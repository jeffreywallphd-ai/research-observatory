...
slice_id: CAP-10.S02
title: Independent adversarial novelty challenge
status: proposed
wave: W4
priority: P0
deployment_profiles:
...
| Slice | `CAP-10.S02` — Independent adversarial novelty challenge |
| Slice outcome | A separate workflow attempts to narrow or invalidate the proposed contribution rather than helping sell it. |
| Wave / priority | `W4` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-10.S02.T01`, `CAP-10.S02.T02`, `CAP-10.S02.T03` |
| Slice dependencies | `CAP-10.S01.T03`, `CAP-09.S06.T01` |
...
A separate workflow attempts to narrow or invalidate the proposed contribution rather than helping sell it.
...
**Implementation thesis.** Freeze the proponent’s concept and nearest-prior set, then launch an independently configured challenger whose objective is to defeat or narrow the novelty claim through alternative terminology, historical vocabulary, adjacent fields and non-journal sources.
...
- Terminology/historical/disciplinary expansion.
- Independent challenger orchestration, threat ranking and counterargument packet.
- Bounded novelty statement, residual uncertainty and human approval gate.

### 2.2 Explicit non-goals

- Helping the same agent write a persuasive introduction while challenging novelty.
- Claiming exhaustive global search.
- Automatically accepting or rejecting the research idea.
...
- **Consumes:** `CAP-10.S01.T03`, `CAP-09.S06.T01` and the handoffs in Section 13.
- **Produces:** A separate workflow attempts to narrow or invalidate the proposed contribution rather than helping sell it.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-10.S01.T03`, `CAP-09.S06.T01`.
...
|---|---|
| `services/core/research_observatory/modules/novelty/challenge.py` | Challenge aggregate, seal and state machine. |
| `services/core/research_observatory/modules/novelty/expansion.py` | Vocabulary/discipline/document branch generation. |
| `services/core/research_observatory/modules/novelty/threat_ranking.py` | Facet overlap, evidence and invalidation/narrowing explanations. |
| `packages/ui/novelty-audit/challenger` | Branch manifest, threat list, counterarguments and approval. |
| `packages/contracts/novelty-challenge` | Sealed input, search branch, threat and statement schemas. |
| `tests/benchmarks/novelty-challenge` | Known hidden prior work and adversarial cases. |
...
|---|---|
| `NoveltyChallenge` | sealed concept/prior set, challenger policy, state, cutoff and manifest |
| `ChallengeSearchBranch` | hypothesis, vocabulary/source scope, SearchRun IDs and findings |
| `NoveltyThreat` | work/argument, overlapping facets, defeat/narrow/qualify effect, evidence and rank |
| `BoundedNoveltyStatement` | approved wording, scope/cutoff, closest work, difference, limits and author |
| `ChallengeAdjudication` | unsupported/apparent/bounded/integrative/etc., rationale and follow-up |
...
- `ChallengeService.start(sealed_input, independent_policy)` validates separation and creates branch plan.
- `ChallengeOrchestrator.execute/resume/cancel` delegates only through recorded search/model tools.
- `ThreatRanker.rank(concept, candidates, comparisons)` preserves evidence and alternative interpretations.
- `NoveltyApproval.approve/reject/revise` is a human-authority command.
...
- Challenger panel shows planned/completed branches, databases/document types and what each sought to invalidate.
- Threat list sorts by defeat/narrowing potential but exposes all facets and evidence.
- Statement editor enforces required bounded sections and highlights prohibited overclaims.
- Stopping/saturation rationale is explicit; users can require more branches before approval.
...
- The sealed unpublished idea remains local unless each external branch/provider is permitted.
- Challenger tools are allowlisted, rate/cost bounded and cannot publish/contact external parties.
- Prompt injection in retrieved documents is inert content and cannot alter challenge instructions.
...
|---|---|
| Challenger shares generator state | Fail independence check and invalidate run. |
| Branch source unavailable | Record failure/coverage impact and require bounded limitation. |
| Challenge finds defeating work | Mark candidate unsupported/apparent or require concept revision; preserve all versions. |
| Long run canceled/restarted | Durable branch checkpoints and idempotent SearchRun linkage; resume remaining branches only. |
...
| Verification profiles | `novelty`, `ai`, `search` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S02.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `novelty`, `ai`, `service` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S02.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
3. Pin model, prompt/schema, runtime/provider and evaluation manifests. Enforce local schema validation, source/evidence closure, egress policy and deterministic fallback; treat model output as untrusted candidate state.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S02.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
3. Pin model, prompt/schema, runtime/provider and evaluation manifests. Enforce local schema validation, source/evidence closure, egress policy and deterministic fallback; treat model output as untrusted candidate state.
4. Exercise security, privacy, rights and research-integrity boundaries with malformed/untrusted inputs, authorization denial, confidential-content redaction and unsupported-state fixtures. Failure must leave canonical state unchanged or exactly recoverable.
5. Run the task verification profiles plus targeted unit, contract, integration and end-to-end tests. Capture named reports, manifests, performance evidence and changed-contract hashes against the reviewed commit.
...
|---|---|
| Branch planning | Default <= 20 branches with rationale before execution; human configurable. |
| Challenge run | Budgeted by sources/model/cost/time; progressive results and cancellation < 1 s local control. |
| Threat ranking | Top 100 candidates < 5 min with bounded passages and resume. |
| Independence | 100% runs record separate policy/model/prompt identity and sealed-input checksum. |
...
- branches/sources/document types
- threats by effect
- concepts defeated/narrowed/revised
- human agreement
- nearest-prior additions
- cost/latency/cancel/restart
...
- Consumes S01 concept and nearest-prior set plus CAP-06 SearchRun infrastructure.
- Provides challenge packet, bounded statement and adjudication to S03 dossier and manuscripts.
- Living monitor later reopens approved audits when threatening work arrives.
...
| Risk | Mitigation |
|---|---|
| Performative adversary | Independent configuration, hidden-prior benchmarks and human evaluation. |
| Unbounded cost/search | Preapproved branch/budget plan and saturation criteria. |
| Overclaim after clean search | Mandatory residual uncertainty and corpus limits. |

## 19. Required ADRs and human decisions

- ADR-CAP10-CHALLENGE: sealed-input independence, branch budget and novelty statement policy.

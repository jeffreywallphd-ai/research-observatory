...
slice_id: CAP-06.S04
title: Search Studio and transparent expansion
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Slice | `CAP-06.S04` — Search Studio and transparent expansion |
| Slice outcome | Researchers can iteratively broaden, narrow, branch, and compare searches without losing their reasoning history. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-06.S04.T01`, `CAP-06.S04.T02`, `CAP-06.S04.T03` |
| Slice dependencies | `CAP-06.S03.T03`, `CAP-04.S02.T03` |
...
Researchers can iteratively broaden, narrow, branch, and compare searches without losing their reasoning history.
...
**Implementation thesis.** Make search evolution a visible scholarly object: every edit, translation, branch, expansion and source failure is recorded, comparable and reversible rather than disappearing inside an agent conversation.
...
- Search Studio editor, source/index selection, preview, run/save/cancel and comparison.
- Versioned branch tree and database-specific translations with semantic-change warnings.
- Explicit citation, bibliographic-coupling and semantic expansion from seeds/clusters.

### 2.2 Explicit non-goals

- Corpus inclusion decisions (S06/CAP-04).
- Unbounded autonomous “research agent” web browsing.
- Pretending external database translations are semantically identical.
...
- **Consumes:** `CAP-06.S03.T03`, `CAP-04.S02.T03` and the handoffs in Section 13.
- **Produces:** Researchers can iteratively broaden, narrow, branch, and compare searches without losing their reasoning history.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-06.S03.T03`, `CAP-04.S02.T03`.
...
|---|---|
| `packages/ui/search-studio` | Editor, source chips, filter builder, branch tree, preview and result comparison. |
| `services/core/research_observatory/modules/corpus_search/branches.py` | SearchRun DAG and annotations. |
| `services/core/research_observatory/modules/corpus_search/translation.py` | Dialect adapters and semantic-loss report. |
| `services/core/research_observatory/modules/corpus_search/expansion.py` | Bounded citation/coupling/semantic operations. |
| `services/workers/connectors` | Cancelable source calls and graph traversal. |
| `tests/e2e/search-studio` | Draft/executed/branch/translation/expansion workflows. |
...
|---|---|
| `SearchBranch` | parent run, intent annotation, status and comparison baseline |
| `QueryTranslation` | source dialect, rendered query, unsupported/approximated clauses and version |
| `ExpansionOperation` | seed set, edge/similarity mode, depth/limit, preview, result and discovery paths |
| `SearchComparison` | added/removed/common records, result metrics and changed clauses |
...
- `SearchBranch.fork(parent_run, draft, rationale)` preserves immutable ancestry.
- `QueryTranslator.compile(ast, source_capabilities)` returns dialect plus loss report.
- `Expansion.preview/execute(spec, cancel_token)` uses identical bounded specification.
- UI routes and components conform to `search-studio.html` and adaptive workflow contracts.
...
- The selected project use case presents Search Studio as a numbered workflow step with previous/next guidance.
- Source chips show availability, rights/credential state and per-adapter errors.
- Branch comparison highlights query differences and result-set effects; no hidden auto-expansion.
- Keyboard shortcuts cover editor execution, cancellation, branch creation and result selection.
...
- External query execution follows connector rate, credential and network policy.
- Search text is private project content; remote sources receive only the explicit translated query after user policy/consent.
- Expansion cannot cross project/source rights or unbounded depth limits.
...
|---|---|
| External adapter partial failure | Complete healthy sources, show per-source error and allow retry without duplicating results. |
| Translation loss | Block or require explicit acknowledgment for material unsupported semantics. |
| Run cancellation | Stop connectors/traversal, retain partial diagnostic state but do not label run complete. |
| Crash during branch run | Resume durable operation or mark failed; parent/history remains intact. |
...
| Verification profiles | `desktop`, `search` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-06.S04.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-06.S04.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `search`, `graph`, `service` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-06.S04.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
3. Implement the approved Academic Minimal route and workflow state using shared components. Cover keyboard/focus, screen reader names, light/dark, loading, empty, offline, denied, stale, error and recovery states; update and approve the reference before any intentional UX divergence.
...
|---|---|
| Editor feedback | Parsing/validation response < 100 ms after debounce. |
| Branch tree | Interactive at 1,000 saved runs with virtualized history. |
| Expansion preview | First bounded estimate/results < 3 s for local graph; remote progress streamed. |
| Cancellation | UI acknowledgement < 200 ms; workers stop at next safe checkpoint < 2 s where provider supports it. |
...
- run/source success
- translation warning frequency
- branch depth and comparisons
- expansion yield/duplicates
- cancel/retry
...
- Consumes S03 SearchRun/hybrid retrieval and CAP-04 source adapters.
- Provides discovery paths and result sets to corpus membership and S06 screening.
- Exports PRISMA-S-ready search manifests through CAP-09.S06.
...
| Risk | Mitigation |
|---|---|
| Interface overload | Progressive disclosure; workflow-specific defaults; supporting tools remain accessible. |
| False translation equivalence | Loss report and source-specific rendered query are always retained. |
| Runaway traversal | Hard depth/count/time budgets and preview-first execution. |

...
slice_id: CAP-02.S04
title: Local secrets, profiles, and privacy controls
status: approved
wave: W1
priority: P0
deployment_profiles:
...
| Slice | `CAP-02.S04` - Local secrets, profiles, and privacy controls |
| Slice outcome | Credentials and policy-sensitive configuration are isolated from ordinary project content. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-02.S04.T01`, `CAP-02.S04.T02`, `CAP-02.S04.T03`, `CAP-02.S04.T04` |
| Slice dependencies | `CAP-02.S03.T02` |
...
Protect unpublished research, credentials, provider keys, and local canonical state while preserving account-optional offline use and usable recovery.
...
**Implementation thesis.** Use one profile vault root protected by the OS credential store on Windows, derive/wrap project database and object keys beneath it, support explicit application locking, and select SQLCipher as the baseline protected SQLite profile subject to licensing/build ADR.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-02.S03.T02`.
- **Produces:** Credentials and policy-sensitive configuration are isolated from ordinary project content.
...
- `planning/backlog.yaml` for `CAP-02.S04` and its task state/dependencies.
...
- `CAP-02.S03.T02` is complete or explicitly gated.
...
1. **Use Windows Credential Locker/DPAPI-backed storage for a random profile vault root; store provider credentials as encrypted records scoped by profile and provider.**
2. **Use distinct wrapped keys for project database, object store, and portable export; never reuse provider secrets as encryption keys.**
3. **Use SQLCipher with raw 256-bit keys, explicit compatibility parameters, integrity check, and rekey workflow as the leading protected-database implementation.**
4. **Provide an optional recovery passphrase/export key using Argon2id with stored parameters; do not silently weaken OS-backed protection.**
5. **Application lock clears active key material and authenticated sessions while leaving checkpointed workers in an explicitly permitted state.**
6. **Telemetry is off by default for research content and never includes document/draft/query payloads.**
...
- Profile vault and Windows credential-store adapter.
- Secret record service with scopes, rotation, expiry, and redacted diagnostics.
- Application lock/idle-lock coordinator.
- Privacy, retention, telemetry, secure-deletion, and provider-egress settings.
- Protected database adapter, SQLCipher build/packaging, key/rekey/recovery procedures, and benchmark.
...
- Secret access requires calling capability, project/profile scope, purpose, and audit context.
- Key material is returned through short-lived handles or callbacks rather than general DTOs.
- Privacy policy evaluation returns allow/deny/require-confirmation with a reason code.
- Protected database profile is declared in the project manifest and verified before first SQL operation.
...
- First-run setup explains local protection and recovery choices without requiring an online account.
- Privacy center shows local/remote model egress, telemetry, retention, support-bundle, and secure-deletion implications in plain language.
- Lock/unlock behavior is consistent and accessible; unsaved editor content is checkpointed before lock.
...
- Never store secrets in `.env`, logs, manifests, crash dumps, renderer storage, or command histories.
- Use constant-time verification where applicable, rate-limit unlock attempts, and avoid revealing whether a project/key exists.
- SQLCipher license, binaries, cryptographic parameters, and update compatibility are supply-chain controlled.
- Secure deletion is described as cryptographic erasure plus best-effort storage cleanup.
...
- Test missing/corrupt OS credential, wrong recovery passphrase, interrupted rekey, database key mismatch, profile relocation, and locked-session crash.
- Rekey uses backup/copy-on-write and verifies new key before retiring prior wrapped key.
- Losing all vault/recovery material is treated honestly as unrecoverable encrypted data.
...
### 9.1 `CAP-02.S04.T01` - Integrate Windows credential storage for secrets
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S04.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-02.S04.T02` - Implement local user profile and application-lock behavior
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S04.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
### 9.3 `CAP-02.S04.T03` - Create privacy, telemetry, retention, and secure-deletion settings
...
**Risk / review gate:** `high` / `privacy-review`  
**Verification profiles:** `desktop`, `security-local`
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S04.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
### 9.4 `CAP-02.S04.T04` - Select and implement the protected local project-database profile
...
**Risk / review gate:** `high` / `security-review`  
**Verification profiles:** `data`, `security-local`, `e2e-local`

**Expected deliverables**
- ADR-selected production protection for the SQLite project database using SQLCipher or an evaluated equivalent, with Windows credential-backed keys, migration, rekey, recovery, and performance evidence.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S04.T04`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
...
|---|---|
| Backlog profiles | `data`, `desktop`, `e2e-local`, `security-local` |
...
- Benchmark SQLCipher open, representative queries, migration, integrity check, and backup on minimum hardware.
- Avoid repeated KDF work by using random raw database keys wrapped by the profile vault, not a user passphrase for every open.
- Idle lock should not terminate durable jobs whose policy explicitly permits local continuation.
...
- Audit secret access by capability/purpose and record lock, unlock, key rotation, rekey, retention, and policy decisions without secret values.
- Expose cryptographic/profile versions and integrity status in diagnostics.
...
- CAP-02.S02 opens SQLite only through this protection profile.
- CAP-02.S03 uses the profile vault to wrap object keys.
- CAP-02.S05 uses a separate export/recovery envelope.
- CAP-12 model gateway consumes egress/provider-secret policy.
- CAP-14 adds Keychain and Linux Secret Service adapters under the same vault interface.
...
- Plaintext-to-protected conversion uses verified export into a new encrypted database and requires explicit user approval.
- Cryptographic parameter changes are versioned and supported through rekey/migration, not silent defaults.
...
- The promised outcome is demonstrable end to end: Credentials and policy-sensitive configuration are isolated from ordinary project content.
...
- ADR REQUIRED: SQLCipher licensing, build, and compatibility profile.
- ADR: Profile-vault hierarchy and recovery-key policy.
- ADR: Application-lock behavior for active local jobs.
...
|---|---|---|
| `CREDENTIAL_LOCKER` | [Credential Locker for Windows Apps](https://learn.microsoft.com/en-us/windows/apps/develop/security/credential-locker) - Microsoft | OS-backed local secret storage for Windows. |
| `SQLCIPHER` | [SQLCipher API](https://www.zetetic.net/sqlcipher/sqlcipher-api/) - Zetetic | Encrypted SQLite profile, keying, rekeying, and integrity checks. |
...
2. Confirm `CAP-02` is the active capability campaign and `CAP-02.S04` is the next eligible slice.
3. Claim the first READY task in order: `CAP-02.S04.T01`. Do not globally select work outside the capability.

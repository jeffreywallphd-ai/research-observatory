...
slice_id: CAP-09.S06
title: Reproducibility packages and scholarly exports
status: proposed
wave: W4
priority: P0
deployment_profiles:
...
| Slice | `CAP-09.S06` — Reproducibility packages and scholarly exports |
| Slice outcome | A project can produce a rights-aware record of corpus, searches, schemas, models, decisions, analysis, and outputs. |
| Wave / priority | `W4` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-09.S06.T01`, `CAP-09.S06.T02`, `CAP-09.S06.T03` |
| Slice dependencies | `CAP-09.S05.T03`, `CAP-04.S04.T02` |
...
A project can produce a rights-aware record of corpus, searches, schemas, models, decisions, analysis, and outputs.
...
**Implementation thesis.** Export a verifiable research object rather than a loose set of files: freeze a rights-filtered snapshot, describe it with portable metadata, validate every payload checksum and preserve the exact search, corpus, schema, model, decision and analysis manifests.
...
- Reproducibility manifest and immutable snapshot boundary.
- PRISMA-compatible flow and search/review appendices.
- DOCX/Markdown/CSV/JSON-LD/graph outputs bundled with integrity metadata.

### 2.2 Explicit non-goals

- Redistributing restricted full text.
- Promising bitwise reproducibility when external providers/models are unavailable.
- Using one proprietary export as the canonical project representation.
...
- **Consumes:** `CAP-09.S05.T03`, `CAP-04.S04.T02` and the handoffs in Section 13.
- **Produces:** A project can produce a rights-aware record of corpus, searches, schemas, models, decisions, analysis, and outputs.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-09.S05.T03`, `CAP-04.S04.T02`.
...
|---|---|---|---|---|
| **Package model** | Use RO-Crate JSON-LD to describe entities/activities/files and BagIt-compatible payload manifests/checksums for transfer integrity. | Custom ZIP with a README only. | The combination supplies portable research-object semantics and deterministic payload validation. | [RO-Crate 1.3 Specification](https://www.researchobject.org/ro-crate/specification.html) |
| **Snapshot boundary** | Freeze canonical revision/event high-water marks, corpus membership, rights decisions, schemas, indexes/models/prompt manifests and approved outputs; derived indexes are referenced or rebuilt, not silently copied as authority. | Export whatever is current as each file is written. | A single transaction-like snapshot boundary is necessary for coherent reproduction. | [PROV-O: The PROV Ontology](https://www.w3.org/TR/prov-o/) |
| **Rights filtering** | Compute an export policy per file/field/passage; replace restricted content with metadata/checksum/locator when redistribution is not permitted. | Include all local project content because the recipient may have access. | Access on the source machine does not grant redistribution rights. | [RFC 8493 - The BagIt File Packaging Format](https://www.rfc-editor.org/rfc/rfc8493.html) |
...
|---|---|
| `services/core/research_observatory/modules/exports/snapshot.py` | Freeze and validate project snapshot boundary. |
| `services/core/research_observatory/modules/exports/ro_crate.py` | RO-Crate metadata graph. |
| `services/core/research_observatory/modules/exports/bagit.py` | Payload manifests, checksums and verification. |
| `services/core/research_observatory/modules/exports/prisma.py` | Flow, search and screening appendices. |
| `services/core/research_observatory/modules/exports/renderers` | DOCX, Markdown, CSV, JSON-LD and graph adapters. |
| `packages/ui/synthesis-studio/export` | Preview, rights exclusions, build/verify and disclosure UI. |
...
|---|---|
| `ReproducibilitySnapshot` | project/event/corpus/schema/model/decision/output boundaries and checksum |
| `ExportPolicyDecision` | artifact/field, classification, include/metadata-only/omit, reason and authority |
| `ResearchObjectCrate` | RO-Crate metadata graph and profiles |
| `PayloadManifest` | paths, sizes, media types and cryptographic checksums |
| `ExportVerificationReport` | schema, checksum, link, rights and replay-readiness findings |
...
- `SnapshotService.freeze(project, scope)` returns immutable snapshot or exact unresolved dependencies.
- `ExportPlanner.preview(snapshot, recipient_policy, formats)` reports contents and exclusions before build.
- `ResearchPackageBuilder.build(plan, cancel_token)` writes staging area then atomically finalizes archive.
- `ResearchPackageVerifier.verify(path)` operates independently and produces machine/human reports.
...
- Export preview explains which sources/passages are included, metadata-only or omitted and why.
- Users choose reproducibility, evidence-table, graph or publication bundle profiles without losing manifest detail.
- Progress, cancellation, resume/rebuild and checksum verification are visible.
- Generated disclosure summarizes AI models, human validation, corpus cutoff and known reproduction limits.
...
- Destination path is user-authorized; symlink/path traversal and archive extraction hazards are tested.
- Secrets, credentials, decrypted temporary paths and private telemetry are prohibited from packages.
- Restricted content policy is reevaluated at build time, not assumed from preview.
...
|---|---|
| Snapshot dependency unresolved | Block build and list stale/missing approvals; optional explicitly partial package is labeled. |
| Disk full/cancel | Remove or retain marked staging safely; never expose partial final archive. |
| Checksum/schema failure | Fail verification and quarantine artifact; no success state. |
| Recipient cannot access locator | Package remains valid metadata-only and documents requirement without embedding restricted content. |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S06.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `evidence`, `search`, `graph` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S06.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `graph`, `evidence`, `security-local` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S06.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Define the versioned portable schema and invariants first. Validate examples and negative fixtures; keep provider, database, model, graph, and UI framework types behind adapters.
...
|---|---|
| Snapshot | Reference 100,000-work project boundary < 30 s after DB indexes. |
| Streaming export | Memory bounded < 1 GiB; throughput >= 50 MB/s for local permitted payloads. |
| Verification | Checksum speed within 20% of platform sequential-read baseline. |
| Determinism | Metadata and small derived files reproducible byte-for-byte for identical snapshot/tool versions where formats permit. |
...
- package builds/cancel/failure
- bytes/files by policy
- rights exclusions
- verification findings
- build/verify duration
- recipient reopen/import success
...
- Consumes search/screening/evidence/graph/synthesis and provenance manifests.
- Provides portable artifacts to manuscript, reviewer, university and archival workflows.
- Import/round-trip fixtures protect macOS/Linux and later hosted compatibility.
...
| Risk | Mitigation |
|---|---|
| Rights breach | Field-level policy preview and build-time reevaluation. |
| False reproducibility | Explicit environmental/provider limits and snapshot hashes. |
| Format drift | Versioned profiles, schema validation and round-trip regression corpus. |

## 19. Required ADRs and human decisions

- ADR-CAP09-EXPORT: RO-Crate/BagIt package profile, rights filters and snapshot boundary.

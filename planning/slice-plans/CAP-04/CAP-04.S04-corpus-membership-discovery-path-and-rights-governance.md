...
slice_id: CAP-04.S04
title: Corpus membership, discovery path, and rights governance
status: proposed
wave: W2
priority: P0
deployment_profiles:
...
| Slice | `CAP-04.S04` - Corpus membership, discovery path, and rights governance |
| Slice outcome | Corpus state is a deliberate scholarly decision with complete acquisition and inclusion provenance. |
| Wave / priority | `W2` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-04.S04.T01`, `CAP-04.S04.T02`, `CAP-04.S04.T03` |
| Slice dependencies | `CAP-04.S03.T03`, `CAP-03.S02.T03` |
...
Make corpus inclusion, exclusion, discovery, access rights, and source coverage explicit so evidence claims can be reproduced and bounded.
...
**Implementation thesis.** Represent project corpus membership as versioned decisions over canonical works/versions, preserve every discovery path and screening reason, and evaluate a structured permitted-use policy at acquisition, viewing, model egress, collaboration, drafting, export, and backup.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-04.S03.T03`, `CAP-03.S02.T03`.
- **Produces:** Corpus state is a deliberate scholarly decision with complete acquisition and inclusion provenance.
...
- `planning/backlog.yaml` for `CAP-04.S04` and its task state/dependencies.
...
- `CAP-04.S03.T03` is complete or explicitly gated.
- `CAP-03.S02.T03` is complete or explicitly gated.
...
1. **Corpus membership is project-scoped and versioned; canonical records can exist without inclusion.**
2. **Separate discovered, candidate, included, excluded, awaiting-decision, unavailable, and withdrawn states with reasons and decision provenance.**
3. **Discovery paths are immutable edges from search/import/citation/recommendation/manual sources with exact run/query/record provenance.**
4. **Rights metadata separates license assertion, access/entitlement source, copy location, permitted actions, confidence, and expiry.**
5. **Policy decisions are action-specific and conservative when metadata is absent/conflicting.**
6. **Source-overlap and bias reports operate on discovery/source records, not only deduplicated canonical counts.**
...
- CorpusItem aggregate and membership decision service.
- DiscoveryPath/SourceCoverage graph and reporting.
- RightsAssertion, Entitlement, PermittedUse policy model and evaluator.
- Corpus provenance, overlap, missingness, language/venue/source diagnostic UI.
...
- Membership decisions include criterion/rule version, reason code, actor/model, evidence, time, and superseding decision.
- Permitted-use evaluation returns allow/deny/unknown/require-confirmation plus governing assertions and expiry.
- Every corpus export carries corpus snapshot ID and rights-filter manifest.
...
- Corpus Canvas shows canonical item, versions, inclusion state, discovery paths, source availability, and rights status.
- Coverage diagnostics explain database/language/year/venue/full-text limitations and allow alternative boundary comparisons.
- Unknown rights are not presented as forbidden or permitted without explanation.
...
- Enforce rights in service APIs; UI hiding alone is insufficient.
- Entitlement credentials and institutional tokens remain scoped secrets.
- Exports, backups, model calls, and collaboration re-evaluate action-specific policy.
...
- Test conflicting licenses, expired entitlement, multiple copies with different terms, membership reversal, source record deletion, and missing rights data.
- Policy changes mark affected actions/artifacts for review through dependency graph.
- Coverage reports retain snapshot/query versions for reproducibility.
...
### 9.1 `CAP-04.S04.T01` - Model corpus item states, reasons, and discovery paths
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S04.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-04.S04.T02` - Implement rights, license, entitlement, and permitted-use metadata
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S04.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-04.S04.T03` - Build corpus provenance and source-overlap reports
...
**Verification profiles:** `desktop`, `service`, `search`

**Expected deliverables**
- Reports of source contribution, duplicate overlap, missing identifiers, OA/full-text status, years, venues, disciplines, languages, and discovery routes.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S04.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
|---|---|
| Backlog profiles | `data`, `desktop`, `search`, `security-local`, `service` |
...
- Precompute project/source overlap summaries incrementally while retaining exact drill-down.
- Index corpus state/reason, rights action, source, search run, and snapshot.
...
- Track membership transitions, exclusion reasons, rights decisions/unknowns, denied attempts, coverage/missingness, source overlap, and report snapshot IDs.
...
- CAP-05 acquisition consumes permitted-use decisions.
- CAP-06 screening updates membership decisions.
- CAP-08 extraction/CAP-18 drafting consume rights-filtered evidence.
- CAP-11 monitoring creates new discovery paths into existing corpus authority.
...
- New rights fields/actions are additive and default to unknown until evaluated.
- Policy rule versions are retained with prior decisions; recalculation creates new decisions.
...
- The promised outcome is demonstrable end to end: Corpus state is a deliberate scholarly decision with complete acquisition and inclusion provenance.
...
- Binding detail: Rights/permitted-use vocabulary and default-unknown policy.
- Binding detail: Corpus snapshot/version semantics.
- Binding detail: Discovery path and source-overlap graph representation.
...
2. Confirm W2 is the approved active campaign and `CAP-04.S04` is dependency-eligible.
3. Claim only the next READY task in `CAP-04.S04` through taskctl; do not bypass Wave dependencies or create a capability lease.

...
## R04 value-to-identity closure

R03 established a separately versioned guidance identity but tested only substitution of the advertised hash. The generated client compared that field with the approved constant without deriving the hash from the received values, so a caller could retain the valid identity while changing one example, evidence-type list, novelty standard, autonomy level, stopping-condition list, or warning. The root cause was treating an authenticated identity field as proof of its unbound payload at the client trust boundary.

R04 reconstructs the complete guidance document from every decoded profile, serializes it with the same recursively sorted, compact, ASCII-safe JSON rules used by Core, computes SHA-256 in the synchronous generated decoder, and requires the computed value to equal the advertised approved hash. The first regression changes `systematic-review.defaultStoppingConditions` while retaining both advertised guidance fields and must return `null`; a positive test uses the exact fourteen-profile guidance values and proves the derived hash is accepted. This is a bounded contract-validation correction: persistence, activation witness, Core values, approved experience, and closed F01/F02 behavior remain unchanged. Focused generated-contract checks lead; service, desktop, native, architecture, planning, and exact-range checks replay only where the changed generated boundary can affect them. Performance and the full cross-capability matrix remain deferred to W1 exit.

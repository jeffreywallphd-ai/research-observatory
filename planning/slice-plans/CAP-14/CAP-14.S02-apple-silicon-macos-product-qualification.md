...
slice_id: CAP-14.S02
title: Apple Silicon macOS product qualification
status: proposed
wave: W6
priority: P0
deployment_profiles:
...
| Slice | `CAP-14.S02` — Apple Silicon macOS product qualification |
| Slice outcome | The local product is production-ready on supported Apple Silicon macOS versions. |
| Wave / priority | `W6` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `macos-arm64` |
| Backlog tasks | `CAP-14.S02.T01`, `CAP-14.S02.T02`, `CAP-14.S02.T03` |
...
The local product is production-ready on supported Apple Silicon macOS versions.
...
**Implementation thesis.** Qualify Apple Silicon macOS as a first-class local edition with Apple trust-chain, Keychain, platform directories and assistive-technology behavior fully tested.
...
- Keychain and macos directory/process adapters.
- Complete macos install/update/offline/accessibility/recovery.
...
- **Produces:** The local product is production-ready on supported Apple Silicon macOS versions.
...
|---|---|---|---|---|
| `CAP-14-D03` | macOS distribution | **Use Developer ID signing, hardened runtime, notarization and stapled DMG/PKG artifacts** | Distribute an unsigned application bundle | Gatekeeper-compatible distribution is required for a production macOS release. | ADR-MACOS-PACKAGING |
...
|---|---|
| `packaging/macos` | universal policy with arm64 artifacts, DMG/PKG and notarization |
| `platform/macos` | Keychain, directories and lifecycle |
| `tests/release/macos` | full workflow qualification |
...
|---|---|
| `MacReleaseArtifact` | Versioned MacReleaseArtifact state with stable identity, lifecycle/status, provenance and policy metadata appropriate to apple silicon macos product qualification. |
| `NotarizationEvidence` | Versioned NotarizationEvidence state with stable identity, lifecycle/status, provenance and policy metadata appropriate to apple silicon macos product qualification. |
| `MacCredentialReference` | Versioned MacCredentialReference state with stable identity, lifecycle/status, provenance and policy metadata appropriate to apple silicon macos product qualification. |
| `MacQualificationRun` | Versioned MacQualificationRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to apple silicon macos product qualification. |
...
- `MacSigner.sign/notarize/staple` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `MacSecretStore get/set/delete` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `MacReleaseGate.evaluate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Downstream:** cross-platform project equivalence and G6.
...
- Follow native macOS expectations for menus, file dialogs, shortcuts and permissions while preserving the Academic Minimal shell.
- Explain Gatekeeper, permissions and update failures with exact recovery steps.
- VoiceOver and keyboard navigation cover primary workflows and dense research surfaces.
...
- Developer ID signatures, hardened runtime and secure timestamps are mandatory.
- Secrets use Keychain access control; logs never expose credential values.
- Entitlements are minimal and reviewed per sidecar/process.
...
|---|---|---|
| Notarization warning or failure | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Quarantine/Gatekeeper blocks sidecar | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Keychain denied/locked | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Update/rollback crosses incompatible project migration | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| Signature/notary/staple verification on clean macOS | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Install, launch, update, rollback and uninstall | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Offline literature workflow with local models/fallbacks | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| VoiceOver, scaling, backup/restore and crash recovery | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Startup and workflow performance measured on declared Apple Silicon profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No Rosetta dependency for supported arm64 release | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Package and update size disclosed | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
cross-platform project equivalence and G6. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-14/index.html`; this slice page is `planning/review-site/CAP-14/CAP-14.S02.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

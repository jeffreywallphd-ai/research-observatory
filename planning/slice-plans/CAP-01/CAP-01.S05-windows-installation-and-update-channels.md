...
slice_id: CAP-01.S05
title: Windows installation and update channels
status: proposed
wave: W5
priority: P0
deployment_profiles:
...
| Slice | `CAP-01.S05` - Windows installation and update channels |
| Slice outcome | The PC/lab application can be installed, upgraded, repaired, and removed predictably by individuals or lab administrators. |
| Wave / priority | `W5` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-01.S05.T01`, `CAP-01.S05.T02`, `CAP-01.S05.T03` |
| Slice dependencies | `CAP-01.S04.T03`, `CAP-00.S05.T03` |
...
Deliver a Windows desktop product that researchers and laboratories can install, update, repair, and manage without developer tooling while preserving local projects and research continuity.
...
**Implementation thesis.** Produce signed per-user NSIS and per-machine MSI packages, signed update metadata with stable/beta channels, and documented silent deployment policy. Treat desktop, sidecar, migrations, and UI reference as one qualified release unit.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-01.S04.T03`, `CAP-00.S05.T03`.
- **Produces:** The PC/lab application can be installed, upgraded, repaired, and removed predictably by individuals or lab administrators.
...
- `planning/backlog.yaml` for `CAP-01.S05` and its task state/dependencies.
...
- `CAP-01.S04.T03` is complete or explicitly gated.
- `CAP-00.S05.T03` is complete or explicitly gated.
...
1. **Offer per-user NSIS as the default individual install and MSI as the managed lab/institution option.**
2. **Use code signing for executables/installers and signed Tauri updater manifests; fail closed on invalid signatures or rollback attempts.**
3. **Separate stable and beta channels and require explicit user/admin enrollment in beta.**
4. **Never place user projects inside the installation directory and never delete them during uninstall or repair.**
5. **Support silent install/uninstall, install directory policy, update policy, and diagnostics collection through documented switches/administrative settings.**
6. **Stage updates, verify compatibility and free space, checkpoint current work, and provide rollback/repair paths.**
...
- Windows installer configuration, signing pipeline, and clean VM tests.
- Release metadata service/files, channel manifests, signature verification, and update UI.
- Administrative deployment guide and machine-readable policy file.
- Upgrade/repair/uninstall integration tests over prior supported versions.
...
- Release manifest binds desktop, sidecar, schema compatibility, UI reference, channel, minimum OS, artifact hashes, and signatures.
- Silent deployment returns stable exit codes and writes redacted deployment logs.
- Update deferral, mandatory security update, and channel policies are explicit and testable.
...
- Update UI explains size, channel, restart requirement, compatibility, and release notes.
- Researchers can postpone normal updates while no active workflow is at a non-checkpointable stage.
- Repair and uninstall language explicitly states that project data is retained unless the user separately requests secure deletion.
...
- Signing keys remain outside CI runners except through protected signing infrastructure.
- Updater accepts HTTPS plus signature verification and rejects downgrade/rollback metadata outside approved recovery.
- Installer custom actions are minimized and run with least privilege.
...
- Test interrupted download, corrupted package, invalid signature, insufficient disk, locked files, failed sidecar migration, and restart after update.
- A failed update leaves the prior version runnable or routes to repair without altering project content.
- Managed installs can disable self-update and use administrator-controlled distribution.
...
### 9.1 `CAP-01.S05.T01` - Create per-user and per-machine Windows installers
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S05.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-01.S05.T02` - Implement signed update manifests and release channels
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S05.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
### 9.3 `CAP-01.S05.T03` - Support silent lab deployment and policy configuration
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S05.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
...
|---|---|
| Backlog profiles | `desktop`, `e2e-local`, `security-local` |
...
- Installer and updater artifact size budgets are recorded and regression-checked.
- Delta update support is optional only if it preserves signature and rollback guarantees.
- First launch after upgrade performs bounded compatibility checks with visible progress.
...
- Record install/update version, channel, stage, safe error code, and outcome; no project paths/content in general telemetry.
- Retain signed release manifests and qualification reports as release evidence.
...
- CAP-02 migration/backup behavior is invoked before incompatible project changes.
- CAP-14 extends the same release-manifest model to macOS and Linux.
- CAP-00 release automation supplies SBOM, signing, provenance, and artifact retention.
...
- Support at least the previous production release for direct upgrade unless an explicit bridge release is documented.
- Database/project compatibility is not inferred from application version alone; use explicit compatibility ranges.
...
- The promised outcome is demonstrable end to end: The PC/lab application can be installed, upgraded, repaired, and removed predictably by individuals or lab administrators.
...
- ADR: Windows installer channel strategy and code-signing service.
- ADR: Update rollback and support-window policy.
...
|---|---|---|
| `TAURI_UPDATER` | [Tauri 2 Updater Plugin](https://v2.tauri.app/plugin/updater/) - Tauri | Signed desktop update metadata and release channels. |
| `TAURI_WINDOWS` | [Tauri 2 Windows Installer](https://v2.tauri.app/distribute/windows-installer/) - Tauri | NSIS/MSI packaging and Windows installer behavior. |
...
2. Confirm `CAP-01` is the active capability campaign and `CAP-01.S05` is the next eligible slice.
3. Claim the first READY task in order: `CAP-01.S05.T01`. Do not globally select work outside the capability.

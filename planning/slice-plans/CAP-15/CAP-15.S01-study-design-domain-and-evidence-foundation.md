...
slice_id: CAP-15.S01
title: Study-design domain and evidence foundation
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-15.S01` — Study-design domain and evidence foundation |
| Slice outcome | Study designs are first-class, versioned, source-grounded research objects. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-15.S01.T01`, `CAP-15.S01.T02`, `CAP-15.S01.T03` |
| Slice dependencies | `CAP-10.S03.T03`, `CAP-09.S05.T03` |
...
Study designs are first-class, versioned, source-grounded research objects.
...
**Implementation thesis.** Make empirical designs durable, evidence-linked scholarly objects so every recommendation, assumption and revision can be compared, challenged and propagated into later protocols and manuscripts.
...
- Canonical study-design domain contracts.
- Evidence/opportunity linkage for design elements.
- Versioning, comparison and staleness propagation.
...
- **Consumes:** `CAP-10 opportunity dossiers, CAP-09 synthesis and research intent` and formal dependencies `CAP-10.S03.T03`, `CAP-09.S05.T03`.
- **Produces:** Study designs are first-class, versioned, source-grounded research objects.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-10.S03.T03`, `CAP-09.S05.T03`.
...
|---|---|---|---|---|
| `CAP-15-D01` | Study object | **Represent StudyDesign as a versioned canonical object linked to intent, opportunity, evidence and decisions** | Generate a disposable prose protocol from a chat | Structured, versioned design state supports comparison, staleness and downstream manuscripts. | ADR-STUDY-DESIGN-DOMAIN |
...
| `CAP-15-D03` | Question logic | **Separate purposes, questions, propositions/hypotheses where appropriate, mechanisms, estimands/interpretive aims and inference limits** | Treat every research question as a statistical hypothesis | Different research logics require different validation and reporting structures. | None |
...
|---|---|
| `modules/study_design/domain` | design, elements and revisions |
| `modules/study_design/evidence_links` | source and rationale closure |
| `modules/study_design/staleness` | dependency and impact rules |
...
|---|---|
| `StudyDesign` | Versioned StudyDesign state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design domain and evidence foundation. |
| `DesignRevision` | Versioned DesignRevision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design domain and evidence foundation. |
| `DesignElement` | Versioned DesignElement state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design domain and evidence foundation. |
| `DesignEvidenceLink` | Versioned DesignEvidenceLink state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design domain and evidence foundation. |
| `DesignAlternative` | Versioned DesignAlternative state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design domain and evidence foundation. |
| `DesignImpact` | Versioned DesignImpact state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design domain and evidence foundation. |
...
- `StudyDesignService.create/revise/compare/approve` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `DesignEvidenceService.link/verify` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `DesignStalenessService.assess` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-10 opportunity dossiers, CAP-09 synthesis and research intent.
- **Downstream:** design alternatives, manuscript blueprint and result integration.
...
- Study Design Studio distinguishes evidence, inference, convention, researcher preference and unresolved decisions.
- Users can compare alternatives and revisions side by side.
- Changed literature or opportunity assumptions identify affected elements without rewriting approved protocols.
...
- Unpublished designs remain local/private by default.
- Evidence links inherit rights and access restrictions.
- No AI-generated element becomes approved without human adjudication.
...
|---|---|---|
| Evidence removed or becomes stale | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Schema/method pack changes | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Two alternatives share inconsistent construct definitions | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Approved protocol revised without rationale | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | study-design, data |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | study-design, evidence, graph |
...
|---|---|---|
| Domain invariants and revision history | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Evidence closure and access-denied links | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Staleness propagation from literature/opportunity changes | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Round-trip structured export/import | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Large designs remain responsive through incremental projections | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Every accepted design element has traceable status/rationale | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No destructive in-place rewrite of approved revision | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-10 opportunity dossiers, CAP-09 synthesis and research intent. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

design alternatives, manuscript blueprint and result integration. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-15/index.html`; this slice page is `planning/review-site/CAP-15/CAP-15.S01.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

...
slice_id: CAP-14.S05
title: Cross-platform project compatibility and recovery
status: proposed
wave: W6
priority: P0
deployment_profiles:
...
| Slice | `CAP-14.S05` — Cross-platform project compatibility and recovery |
| Slice outcome | Projects and analytical results remain portable and semantically identical across qualified desktop operating systems. |
| Wave / priority | `W6` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64`, `macos-arm64`, `linux-x64`, `linux-arm64` |
| Backlog tasks | `CAP-14.S05.T01`, `CAP-14.S05.T02`, `CAP-14.S05.T03` |
| Slice dependencies | `CAP-14.S02.T03`, `CAP-14.S03.T03` |
...
Projects and analytical results remain portable and semantically identical across qualified desktop operating systems.
...
**Implementation thesis.** Guarantee that canonical projects, evidence and decisions move among operating systems unchanged by normalizing path hazards and rebuilding only explicitly derivative artifacts.
...
- Architecture-neutral project/bundle format.
- Path, filename, permission and migration safety.
- Cross-platform transfer/recovery/semantic-equivalence suite.
...
- **Consumes:** `macOS/Linux qualification and CAP-02 portability` and formal dependencies `CAP-14.S02.T03`, `CAP-14.S03.T03`.
- **Produces:** Projects and analytical results remain portable and semantically identical across qualified desktop operating systems.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-14.S02.T03`, `CAP-14.S03.T03`.
...
|---|---|---|---|---|
| `CAP-14-D12` | Project interchange | **Keep canonical project structures architecture-neutral; treat indexes and model caches as rebuildable derivatives** | Serialize native paths, accelerator state and index binaries as canonical | Projects must move among OS targets without lossy conversion. | None |
| `CAP-14-D13` | Portable package | **Use checksummed BagIt payloads plus RO-Crate metadata and PROV links for transfers** | Zip the project directory without a manifest | Portable packaging should detect corruption and preserve research context. | None |
| `CAP-14-D14` | Path safety | **Detect case collisions, invalid names, symlinks, permissions and length limits before transfer** | Let the destination filesystem resolve conflicts | Cross-platform filename rules can otherwise overwrite or hide project objects. | None |
...
|---|---|
| `modules/portability` | project/bundle manifests |
| `modules/filesystem_compat` | collision and path validation |
| `tests/portability` | round-robin OS transfer fixtures |
...
|---|---|
| `PortableProjectManifest` | Versioned PortableProjectManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform project compatibility and recovery. |
| `PathCompatibilityFinding` | Versioned PathCompatibilityFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform project compatibility and recovery. |
| `TransferRun` | Versioned TransferRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform project compatibility and recovery. |
| `RebuildPlan` | Versioned RebuildPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform project compatibility and recovery. |
| `SemanticEquivalenceReport` | Versioned SemanticEquivalenceReport state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform project compatibility and recovery. |
...
- `ProjectPortability.inspect/export/import` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `PathCompatibility.validate/normalize` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `SemanticEquivalence.compare` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** macOS/Linux qualification and CAP-02 portability.
- **Downstream:** G6 cross-platform gate and later hosted import.
...
- Preflight lists incompatible names, symlinks, permissions and rebuildable caches before transfer.
- Users can choose safe renaming only for noncanonical presentation names; identifiers remain stable.
- After import, a summary distinguishes preserved canonical state from rebuilt derivatives.
...
- Reject unsafe symlinks/path traversal and executable surprises.
- Transfer packages are checksummed and rights-aware.
- Private content remains encrypted according to destination policy.
...
|---|---|---|
| Case-insensitive collision | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Path too long/invalid on destination | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Transfer interrupted or hash mismatch | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Native vector/model cache cannot be reused | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | desktop-cross-platform, data |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | desktop-cross-platform, data, security-local |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | desktop-cross-platform, e2e-local |
...
|---|---|---|
| Windows→macOS→Linux→Windows round trip | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Canonical SQLite/object/provenance hash/semantic comparison | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| BagIt checksum and RO-Crate metadata validation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Recovery after partial import and derivative rebuild | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Preflight scales linearly within project profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Canonical transfer does not require model/index payloads | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Rebuild time estimated and visible | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
macOS/Linux qualification and CAP-02 portability. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

G6 cross-platform gate and later hosted import. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-14/index.html`; this slice page is `planning/review-site/CAP-14/CAP-14.S05.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

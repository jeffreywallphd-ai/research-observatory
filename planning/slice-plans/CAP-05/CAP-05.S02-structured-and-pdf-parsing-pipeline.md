...
slice_id: CAP-05.S02
title: Structured and PDF parsing pipeline
status: proposed
wave: W2
priority: P0
deployment_profiles:
...
| Slice | `CAP-05.S02` - Structured and PDF parsing pipeline |
| Slice outcome | A replaceable local parser pipeline produces normalized document structure with retained originals and quality signals. |
| Wave / priority | `W2` / `P0` |
...
Convert lawful scholarly documents into a common inspectable structure while preserving original bytes, source locations, uncertainty, and the ability to replace parsing technology.
...
**Implementation thesis.** Define a parser-neutral Document IR and deterministic selection policy. Prefer native JATS/TEI/XML/HTML adapters, use a pinned Docling process for local PDF parsing, retain raw parser outputs and quality signals, and isolate parser crashes/resource use.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-05.S01.T03`, `CAP-03.S04.T02`.
- **Produces:** A replaceable local parser pipeline produces normalized document structure with retained originals and quality signals.
...
- `planning/backlog.yaml` for `CAP-05.S02` and its task state/dependencies.
...
- `CAP-05.S01.T03` is complete or explicitly gated.
- `CAP-03.S04.T02` is complete or explicitly gated.
...
4. **Run Docling in an isolated worker process with pinned model/package versions, resource/time limits, no network, and OCR disabled by default.**
...
- Parser interface, registry, selection policy, and normalized Document IR schemas.
- Secure JATS/TEI/XML/HTML adapters with golden fixtures.
- Docling adapter process, page rendering, model asset packaging, fallback extractor, and benchmark harness.
- Staged parse result validator and quality report.
...
- IR includes document metadata, ordered pages/regions/blocks/sections/paragraphs/sentences/lists/footnotes/references/citation markers/tables/figures plus original text/offset/coordinates where available.
- Every node has stable staged ID, type, order, text/content reference, parent, source locator, confidence, warnings, and parser evidence.
- Parser selection result records candidate formats/parsers, chosen parser/version/config, rationale, and fallback path.
- Partial parse output is never canonical after terminal failure.
...
- Parser worker receives read-only source stream and isolated output staging with CPU/memory/time limits.
- Quarantine crashes or anomalous resource behavior; no document text in general crash logs.
...
- Test malformed XML, XXE payload, namespace variants, malformed PDF, encrypted/password PDF, scanned PDF, huge page/canvas, parser crash, timeout, OOM, cancellation, and restart.
- Parse attempts are immutable and resumable at the workflow level; a new attempt never overwrites prior raw output.
- Fallback output is independently tagged and cannot satisfy downstream quality gates silently.
...
### 9.1 `CAP-05.S02.T01` - Define parser interface, document IR, and parser-selection policy
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S02.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-05.S02.T02` - Implement native JATS/TEI/XML/HTML parsing
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S02.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-05.S02.T03` - Integrate the local Docling-based PDF parser with fallback
...
**Risk / review gate:** `high` / `agent-review`  
**Verification profiles:** `documents`, `e2e-local`

**Expected deliverables**
- Packaged Docling-based local PDF parser adapter, resource limits, page rendering, OCR-disabled-by-default policy, fallback text extraction, and quality report.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S02.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
4. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
|---|---|
| Backlog profiles | `documents`, `e2e-local` |
...
- Record selected parser/config/model versions, phase timings, pages, nodes, warnings, quality dimensions, resource peaks, fallback use, and failure class.
- Retain benchmark corpus/version and regression thresholds.
...
- Consumes CAP-05.S01 source objects and CAP-03.S04 workflows.
- CAP-05.S03 validates and commits immutable normalized revisions/anchors.
- CAP-05.S05 enriches references/citations/tables/figures from IR.
- CAP-05.S06 evaluates quality and corrections.
- Hosted deployment may use GROBID plus Docling behind the same parser port.
...
- Parser/IR schema upgrades create new parse attempts and document revisions after explicit acceptance.
- IR versioning and adapter conformance fixtures protect downstream modules from parser-specific structures.
...
- The promised outcome is demonstrable end to end: A replaceable local parser pipeline produces normalized document structure with retained originals and quality signals.
...
- ADR: Document IR schema and parser selection policy.
- ADR: Docling pinned version/model packaging and resource limits.
- ADR: OCR activation and degraded fallback policy.
...
| `JATS` | [Journal Article Tag Suite](https://jats.nlm.nih.gov/) - NLM | Native scholarly article parsing. |
| `TEI` | [TEI Guidelines](https://tei-c.org/guidelines/) - TEI Consortium | Native structured text parsing. |
| `OWASP_XXE` | [OWASP XXE Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html) - OWASP | Secure XML parsing. |
...
2. Confirm `CAP-05` is the active capability campaign and `CAP-05.S02` is the next eligible slice.
3. Claim the first READY task in order: `CAP-05.S02.T01`. Do not globally select work outside the capability.

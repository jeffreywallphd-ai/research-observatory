...
slice_id: CAP-04.S01
title: Reference-library and file imports
status: proposed
wave: W2
priority: P0
deployment_profiles:
...
| Slice | `CAP-04.S01` - Reference-library and file imports |
| Slice outcome | Researchers can import existing bibliographies with preview, mapping, validation, and repeatable merge behavior. |
| Wave / priority | `W2` / `P0` |
...
Allow researchers to bring existing bibliographies and structured records into the observatory without losing source fidelity, creating duplicates, or concealing mapping decisions.
...
**Implementation thesis.** Implement format-specific tolerant parsers into one raw-preserving import IR, preview all transformations and conflicts, and commit through idempotent manifests with deterministic source-record keys and full provenance.
...
- **Produces:** Researchers can import existing bibliographies with preview, mapping, validation, and repeatable merge behavior.
...
- `planning/backlog.yaml` for `CAP-04.S01` and its task state/dependencies.
...
1. **Support RIS, BibTeX, CSL JSON, DOI lists, and governed structured CSV through format-specific adapters into a common ImportRecord IR.**
2. **Preserve original bytes/record text, source filename, encoding, row/record number, parser version, warnings, and unknown fields.**
3. **Separate parse, normalize, preview/map, reconcile, and commit stages; preview never mutates the corpus.**
4. **Use deterministic import source keys and manifest/content hashes so rerunning the same import is idempotent.**
5. **Never silently discard malformed records; valid records may proceed only with a complete error/warning report.**
6. **Allow user mapping profiles for CSV but version and include them in the import manifest.**
...
- Import adapter interfaces and parsers for each baseline format.
- Import IR, validation/warning taxonomy, and normalization preview.
- Desktop Import/Reconciliation workspace with field mapping, conflict grouping, sample raw view, and inclusion controls.
- Idempotent commit service and import manifest/export.
...
- ImportRecord separates raw value, normalized candidate, confidence, source location, and accepted mapped value.
- Commit manifest records source hash, parser/mapping versions, record decisions, created/linked IDs, warnings, actor, and transaction outcome.
- A rerun with identical manifest inputs returns the prior commit result or a no-op comparison.

**Early identity handoff.** `CAP-04.S01.T03` commits canonical source-record and
import-manifest identities through the existing Core repository/provenance ports.
It may link to an already established work/version through the shared contract,
but does not create a second reconciliation engine. New ambiguous work/version
resolution belongs to `CAP-04.S03`; before then the UI reports imported source
records awaiting reconciliation, not an invented resolved corpus. Preserve source
IDs when later reconciliation appends work/version links. Idempotent re-import
does not depend on `CAP-04.S03` being implemented or produce duplicate source records.
...
- Constrain file sizes/record counts, validate encoding, and parse without arbitrary code execution or unsafe BibTeX extensions.
- Do not expose local paths in exported manifests.
...
- Test malformed/truncated files, mixed encoding, duplicate columns, huge fields, parser timeout, commit conflict, and restart during commit.
- Parsing and preview are restartable from immutable source objects.
- Commit is atomic for canonical state/provenance; partial adapter output cannot create half-imported corpus records.
...
### 9.1 `CAP-04.S01.T01` - Implement RIS, BibTeX, CSL JSON, DOI-list, and structured CSV parsers
...
### 9.2 `CAP-04.S01.T02` - Create import preview, mapping, and conflict UI
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S01.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
### 9.3 `CAP-04.S01.T03` - Implement idempotent import commits and import manifests
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-04.S01.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
- Stream large RIS/BibTeX/CSV inputs and paginate preview.
- Set explicit record/field length limits with actionable override policy.
- Benchmark at 100k metadata records without unbounded memory.
...
- Record parser/mapping versions, record counts, warning/error classes, preview/commit duration, dedupe candidates, and idempotent replays.
- Never log raw bibliographic content by default.
...
- CAP-04.S03 reconciles normalized candidates to canonical works/versions.
- CAP-04.S04 records discovery path, membership, and rights.
- CAP-05 attaches full text to resulting work versions.
- Zotero synchronization can later reuse import IR and manifests.
...
- Parser/mapping upgrades do not rewrite prior import manifests; re-import comparison creates a new manifest.
- Import IR schema additions are backward-compatible and preserve raw source access.
...
- The promised outcome is demonstrable end to end: Researchers can import existing bibliographies with preview, mapping, validation, and repeatable merge behavior.
...
- Binding detail: CSV mapping-profile and formula-injection policy.
...
|---|---|---|
| `CSL_JSON` | [Citation Style Language Data Schema](https://github.com/citation-style-language/schema/blob/master/csl-data.json) - Citation Style Language | Portable bibliographic records. |
| `ZOTERO_API` | [Zotero Web API v3 Basics](https://www.zotero.org/support/dev/web_api/v3/basics) - Zotero | Reference-library interoperability. |
| `OWASP_UPLOAD` | [OWASP File Upload Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html) - OWASP | Untrusted file intake and quarantine. |
...
2. Confirm W2 is the approved active campaign and `CAP-04.S01` is dependency-eligible.
3. Claim only the next READY task in `CAP-04.S01` through taskctl; do not bypass Wave dependencies or create a capability lease.

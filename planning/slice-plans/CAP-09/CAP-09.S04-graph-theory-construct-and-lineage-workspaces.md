...
slice_id: CAP-09.S04
title: Graph, theory, construct, and lineage workspaces
status: proposed
wave: W4
priority: P0
deployment_profiles:
...
| Slice | `CAP-09.S04` — Graph, theory, construct, and lineage workspaces |
| Slice outcome | Complex field structures are navigable through task-specific views rather than one undifferentiated network. |
| Wave / priority | `W4` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-09.S04.T01`, `CAP-09.S04.T02`, `CAP-09.S04.T03` |
| Slice dependencies | `CAP-09.S01.T03`, `CAP-06.S05.T01` |
...
Complex field structures are navigable through task-specific views rather than one undifferentiated network.
...
**Implementation thesis.** Turn the graph into task-focused scholarly views—claim, theory, construct, citation, method and temporal—using progressive disclosure and synchronized source inspection rather than exposing an undifferentiated network hairball.
...
- Scalable claim/argument graph explorer.
- Theory, construct-definition and operationalization maps.
- Citation, semantic, method and temporal presets with lineage.

### 2.2 Explicit non-goals

- Rendering an entire corpus graph at once.
- Using visual proximity as evidence.
- Replacing accessible tabular/tree views with canvas-only interaction.
...
- **Consumes:** `CAP-09.S01.T03`, `CAP-06.S05.T01` and the handoffs in Section 13.
- **Produces:** Complex field structures are navigable through task-specific views rather than one undifferentiated network.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-09.S01.T03`, `CAP-06.S05.T01`.
...
|---|---|
| `packages/ui/graph-explorer` | Shared accessible graph canvas, table/tree alternative and details drawer. |
| `packages/ui/claim-graph` | Claim/argument task presets. |
| `packages/ui/theory-map` | Theory, construct, definition, measure and lineage presets. |
| `services/core/research_observatory/modules/scholarly_graph/views.py` | Authorized subgraph and aggregate view specifications. |
| `packages/contracts/graph-view` | View spec, layout, expansion and selection schemas. |
| `tests/e2e/graph-workspaces` | Navigation, source inspection, accessibility and scale fixtures. |
...
|---|---|
| `GraphViewSpec` | root/filter/relation/depth/limit/layout/encoding/snapshot and owner |
| `GraphViewResult` | nodes, edges, aggregates, truncation, explanations and manifest |
| `LayoutArtifact` | algorithm, parameters, positions, viewport and checksum |
| `MapPreset` | claim/theory/construct/citation/semantic/method/temporal semantics |
| `GraphSelection` | selected entity/assertion and synchronized source/details context |
...
- `GraphViewQuery.execute(spec, authorization)` returns bounded result plus truncation summary.
- `GraphExpansion.preview/apply` records the exact relation/filter/depth added.
- `LayoutAdapter.layout(result, preset, seed)` is deterministic when supported and stores parameters.
- Source/details drawer uses CAP-05 anchors and S01 assertion history.
...
- Claim Graph and Theory Map match approved HTML routes and adaptive workflow placement.
- Keyboard users can traverse the equivalent hierarchical/table list, open details and expand relations.
- Legends distinguish direct evidence, inferred, disputed, stale and adjudicated relations using icon/text plus color.
- Large views summarize hidden clusters and offer filtered expansion rather than silently dropping content.
...
- Core returns only authorized nodes/edges and safe labels; client cannot expand by arbitrary IDs outside project.
- Export respects rights and excludes restricted passage text.
- Layouts and screenshots of sensitive projects follow export/clipboard policy.
...
|---|---|
| Layout worker fails | Return accessible table/tree and allow retry/alternate layout. |
| View exceeds cap | Return deterministic truncation/aggregation report and refinement suggestions. |
| Selected edge becomes stale | Keep selection, show stale reason and current alternatives. |
| Source unavailable | Show assertion metadata and repair route without fabricating passage. |
...
| Verification profiles | `desktop`, `graph` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S04.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `desktop`, `graph`, `evidence` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S04.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `desktop`, `graph`, `search` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S04.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the approved Academic Minimal route and workflow state using shared components. Cover keyboard/focus, screen reader names, light/dark, loading, empty, offline, denied, stale, error and recovery states; update and approve the reference before any intentional UX divergence.
...
|---|---|
| Interactive view | Default <= 500 nodes/1,500 edges; initial usable render < 2 s on reference desktop. |
| Expansion | p95 < 1 s for bounded one-hop authorized expansion. |
| Pan/zoom | Target 60 fps for default cap; degrade labels/edges predictably below 30 fps. |
| Accessible alternative | Table/tree first page < 500 ms and complete paginated access. |
...
- Consumes S01 graph projection, S02 entities/arguments and S03 comparison relations.
- Provides saved map snapshots to S05 synthesis, CAP-10 opportunity dossiers and manuscripts.
- Preserves stable view contracts for macOS/Linux and later hosted deployments.
...
| Risk | Mitigation |
|---|---|
| Graph hairball | Strict caps, presets and explicit expansion. |
| Visual overclaim | Evidence drawer, legends and textual explanation. |
| Accessibility gap | Equivalent table/tree workflow and automated keyboard/ARIA tests. |

## 19. Required ADRs and human decisions

- ADR-CAP09-GRAPH-UX: Cytoscape adapter, view caps and accessible equivalent.
...
- **Cytoscape.js Documentation** — Interactive graph visualization, selectors, layouts, serialization and graph interaction.  

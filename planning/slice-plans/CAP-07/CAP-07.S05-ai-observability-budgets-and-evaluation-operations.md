...
slice_id: CAP-07.S05
title: AI observability, budgets, and evaluation operations
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Slice | `CAP-07.S05` — AI observability, budgets, and evaluation operations |
| Slice outcome | Model use is measurable by scholarly task, project, provider, quality, latency, and cost without exposing research content. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-07.S05.T01`, `CAP-07.S05.T02`, `CAP-07.S05.T03` |
| Slice dependencies | `CAP-07.S04.T03`, `CAP-03.S03.T03` |
...
Model use is measurable by scholarly task, project, provider, quality, latency, and cost without exposing research content.
...
**Implementation thesis.** Operate AI as an evaluated, budgeted and observable dependency: collect redacted technical traces, reconcile usage, compare models on versioned research tasks and block upgrades that degrade evidence quality.
...
- Redacted OpenTelemetry-compatible model tracing and usage accounting.
- Project budgets, quotas, caching, cancellation and resource governance.
- Evaluation registry/dashboard and controlled upgrade workflow.

### 2.2 Explicit non-goals

- Logging raw confidential prompts by default.
- Treating LLM-as-judge as the sole quality measure.
- Auto-upgrading models on mutable provider aliases.
...
- **Consumes:** `CAP-07.S04.T03`, `CAP-03.S03.T03` and the handoffs in Section 13.
- **Produces:** Model use is measurable by scholarly task, project, provider, quality, latency, and cost without exposing research content.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-07.S04.T03`, `CAP-03.S03.T03`.
...
|---|---|
| `services/core/research_observatory/evaluation` | Dataset registry, run definitions, metrics, comparisons and promotion decisions. |
| `services/core/research_observatory/modules/model_gateway/budgets.py` | Project quotas, reservations, reconciliation and cancellation. |
| `services/core/research_observatory/modules/model_gateway/cache.py` | Policy-aware content-addressed result cache. |
| `services/core/research_observatory/adapters/telemetry` | OTel spans/metrics and local sink. |
| `packages/ui/model-center/evaluations` | Cost/quality/latency dashboards and upgrade review. |
| `tests/evaluations` | Held-out extraction, verification, retrieval, synthesis and novelty fixtures. |
...
|---|---|
| `ModelCallTrace` | trace/span IDs, task/route/model, timing, usage, validation and redaction status |
| `BudgetPolicy` | project/task/provider currency/token/time/resource limits and approval thresholds |
| `EvaluationDatasetVersion` | task, cases, licenses, split, provenance and contamination controls |
| `EvaluationRun` | models/prompts/schemas, environment, metrics, errors and artifacts |
| `ModelPromotionDecision` | baseline/candidate comparison, thresholds, approver and rollback |
...
- `BudgetManager.reserve/reconcile/cancel` wraps every model call.
- `EvaluationRunner.run(matrix)` uses the same gateway contracts as production.
- `ModelRegistry` only marks a version approved after a promotion decision.
- Telemetry and durable project provenance share trace IDs but have separate retention/content rules.
...
- Project settings show budget remaining, expected run cost and cancellation effects.
- Model Center compares accuracy/citation/schema/calibration/cost/latency by task and domain.
- Upgrade workflow shows regressions and keeps prior approved version available.
- Users can export an AI use/disclosure manifest without telemetry secrets.
...
- Redaction tests fail CI if content or credentials enter default traces.
- Evaluation datasets retain rights and confidentiality labels; provider routes obey them.
- Budgets prevent malicious or accidental unbounded agent loops.
...
|---|---|
| Telemetry sink down | Do not block local scholarly work; buffer bounded metadata and emit health warning. |
| Budget exhausted | Cancel/deny before next call and preserve partial workflow state. |
| Evaluation infrastructure failure | No promotion; retain current approved models. |
| Cache mismatch/corruption | Verify manifest/content hash, discard and recompute. |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-07.S05.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `ai`, `service`, `desktop` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-07.S05.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Verification profiles | `ai`, `desktop` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-07.S05.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Define the versioned portable schema and invariants first. Validate examples and negative fixtures; keep provider, database, model, graph, and UI framework types behind adapters.
3. Pin model, prompt/schema, runtime/provider and evaluation manifests. Enforce local schema validation, source/evidence closure, egress policy and deterministic fallback; treat model output as untrusted candidate state.
4. Exercise security, privacy, rights and research-integrity boundaries with malformed/untrusted inputs, authorization denial, confidential-content redaction and unsupported-state fixtures. Failure must leave canonical state unchanged or exactly recoverable.
5. Run the task verification profiles plus targeted unit, contract, integration and end-to-end tests. Capture named reports, manifests, performance evidence and changed-contract hashes against the reviewed commit.
...
|---|---|
| Telemetry overhead | < 5% latency and CPU overhead for ordinary calls. |
| Accounting completeness | 100% of completed/failed attempts have reconciled usage or explicit unknown status. |
| Core evaluation | Deterministic/stub suite < 15 min; live qualification scheduled/manual with fixed budget. |
| Cancellation | Budget or user cancel prevents new calls immediately and propagates active cancellation. |
...
- cost/tokens/time by project/task/model
- validation/citation/accuracy metrics
- fallback/cache
- budget denial
...
- Evaluates CAP-06 retrieval/reranking and every CAP-08–10 model-dependent workflow.
- Supplies model disclosures and manifests to CAP-09 exports and later manuscripts.
- CAP-14 adds platform/hardware evaluation dimensions.
...
| Risk | Mitigation |
|---|---|
| Evaluation circularity | Use objective checks, expert samples, cross-model judges only as secondary and external replication. |
| Benchmark overfitting | Held-out/versioned sets and contamination tracking. |
| Observability privacy | Content-free defaults and project-governed artifacts. |

## 19. Required ADRs and human decisions

- ADR-CAP07-EVAL: initial task metrics, datasets, promotion thresholds and telemetry retention.

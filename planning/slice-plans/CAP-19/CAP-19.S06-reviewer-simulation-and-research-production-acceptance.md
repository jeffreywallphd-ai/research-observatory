...
slice_id: CAP-19.S06
title: Reviewer simulation and research-production acceptance
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-19.S06` — Reviewer simulation and research-production acceptance |
| Slice outcome | Extended simulated review and revision are production-ready and complete the G8 local research lifecycle. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-19.S06.T01`, `CAP-19.S06.T02`, `CAP-19.S06.T03` |
| Slice dependencies | `CAP-19.S05.T03`, `CAP-18.S06.T03` |
...
Extended simulated review and revision are production-ready and complete the G8 local research lifecycle.
...
**Implementation thesis.** Accept reviewer simulation only when generated and uploaded drafts, multiple research traditions, privacy, calibration and full revision/response workflows pass expert evaluation.
...
- Reviewer simulation and revision & response ui completion.
- Generated and uploaded draft evaluation.
- Expert reviewer benchmark and calibration.
- G8 end-to-end research-production release evidence.
...
- **Consumes:** `all CAP-19 slices and complete CAP-16-18 research-production chain` and formal dependencies `CAP-19.S05.T03`, `CAP-18.S06.T03`.
- **Produces:** Extended simulated review and revision are production-ready and complete the G8 local research lifecycle.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-19.S05.T03`, `CAP-18.S06.T03`.
...
| `CAP-19-D16` | Revision plan | **Map each accepted comment to stable blocks, actions, owner, status, evidence and change sets** | Revise prose directly from reviewer text without a plan | A governed plan preserves author authority and enables selective review. | None |
| `CAP-19-D17` | Response artifact | **Generate an editable point-by-point response/rebuttal linked to comments and manuscript diffs** | Produce a generic cover letter summary | Traceable responses support real editorial workflows and accountability. | None |
| `CAP-19-D18` | Follow-up review | **Re-run only relevant reviewer roles/criteria against the revised snapshot and prior response** | Repeat the entire panel blindly after every edit | Selective re-review reduces cost and focuses verification on resolved issues. | None |
...
|---|---|
| `tests/acceptance/review` | multi-type review/revision suite |
| `benchmarks/reviewer` | expert report corpus and seeded defects |
| `artifacts/gates/G8-review` | capability acceptance bundle |
...
|---|---|
| `ReviewerAcceptanceRun` | Versioned ReviewerAcceptanceRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer simulation and research-production acceptance. |
| `ReviewerBenchmarkResult` | Versioned ReviewerBenchmarkResult state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer simulation and research-production acceptance. |
| `ReviewGateDecision` | Versioned ReviewGateDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer simulation and research-production acceptance. |
...
- `Simulation is labeled developmental and cannot claim acceptance prediction.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Every accepted reviewer finding is evidence/criterion traceable.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Human authors decide revisions and final submission.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** all CAP-19 slices and complete CAP-16-18 research-production chain.
- **Downstream:** G8 publication-development release and later living-monitor updates.
...
- Test panel setup, reports, synthesis, revision and response end to end.
- Include keyboard/screen-reader/light/dark and high-comment-volume workflows.
...
- Audit local/private defaults and explicit remote permission.
- Use authorized benchmark manuscripts and protect reviewer data.
...
|---|---|---|
| reviewer misses seeded critical issue | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| panel invents venue rule | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| meta-review erases dissent | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| revision resolves comment without changing manuscript | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | reviewer, desktop |
...
| Verification profiles | reviewer, e2e-local, security-local |
...
|---|---|---|
| expert recall/precision/actionability and overreach metrics | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| generated and external draft coverage | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| privacy/egress red-team | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| independent editorial/methods/theory/critical review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| panel runtime/cost declared by profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| no unresolved critical/high integrity/privacy issue | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| calibration reported by role and manuscript type | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
all CAP-19 slices and complete CAP-16-18 research-production chain. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

G8 publication-development release and later living-monitor updates. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-19/index.html`; this slice page is `planning/review-site/CAP-19/CAP-19.S06.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

...
slice_id: CAP-17.S03
title: Study-plan and result reconciliation
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-17.S03` — Study-plan and result reconciliation |
| Slice outcome | Actual study conduct and results are distinguished from plans and verified before authoring. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-17.S03.T01`, `CAP-17.S03.T02`, `CAP-17.S03.T03` |
| Slice dependencies | `CAP-17.S02.T03`, `CAP-15.S05.T03` |
...
Actual study conduct and results are distinguished from plans and verified before authoring.
...
**Implementation thesis.** Reconcile what was planned, what was conducted and what was reported without rewriting history or inferring misconduct.
...
- Studydesign-to-studyrun comparison.
- Missing/inconsistent/duplicate/unsupported claim detection.
- Neutral deviation ledger and materiality.
- Human verification and adjudication workflow.
...
- **Consumes:** `CAP-15 approved design/protocol and CAP-17.S02 candidates` and formal dependencies `CAP-17.S02.T03`, `CAP-15.S05.T03`.
- **Produces:** Actual study conduct and results are distinguished from plans and verified before authoring.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-17.S02.T03`, `CAP-15.S05.T03`.
...
|---|---|---|---|---|
| `CAP-17-D12` | Plan-versus-actual reconciliation | **Compare approved design/protocol with reported conduct and preserve both states** | Rewrite the original study design to match the report | Transparent deviations are essential for methods/results drafting and review. | None |
| `CAP-17-D13` | Deviation handling | **Create a neutral deviation ledger with materiality, explanation, consequence and human adjudication** | Automatically label deviations as misconduct or errors | The system should surface discrepancies without making unsupported normative judgments. | None |
| `CAP-17-D14` | Verification model | **Use independent machine checks plus sampled/full human verification based on consequence and confidence** | Promote extracted results directly to verified | Study-result evidence is consequential and requires explicit promotion. | None |
...
|---|---|
| `packages/core/results/reconciliation` | plan/actual/report comparison engine |
| `services/core-api/results/adjudication` | verification and deviation decisions |
| `packages/ui/results` | comparison and adjudication components |
...
|---|---|
| `StudyRun` | Versioned StudyRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-plan and result reconciliation. |
| `DesignExecutionMapping` | Versioned DesignExecutionMapping state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-plan and result reconciliation. |
| `StudyDeviation` | Versioned StudyDeviation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-plan and result reconciliation. |
| `ResultVerification` | Versioned ResultVerification state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-plan and result reconciliation. |
| `ResultAdjudication` | Versioned ResultAdjudication state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-plan and result reconciliation. |
| `UnsupportedReportClaim` | Versioned UnsupportedReportClaim state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-plan and result reconciliation. |
...
- `Approved design remains immutable and actual conduct is a separate revisioned object.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Deviation finding is descriptive; misconduct/ethics determinations remain external human authority.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Only verified/adjudicated results may support manuscript result claims.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-15 approved design/protocol and CAP-17.S02 candidates.
- **Downstream:** CAP-17.S04 verified graph and CAP-18 methods/results drafting.
...
- Display planned, actual and reported fields side by side.
- Provide evidence, consequence and recommendation for each discrepancy.
...
- Restrict sensitive deviations and participant/operational details to authorized users.
- Audit all promotions and overrides.
...
|---|---|---|
| report omits actual conduct details | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| same result appears in conflicting tables | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| user rejects machine reconciliation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| source correction invalidates prior adjudication | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | results, study-design |
...
| Verification profiles | results, evidence |
...
|---|---|---|
| known plan/actual deviation fixtures | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| duplicate and contradiction detection precision review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| human adjudication persistence/audit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| staleness after report correction | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| comparison completes interactively for representative protocol/report | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| bulk discrepancies are pageable/filterable | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| no automatic high-consequence promotion | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-15 approved design/protocol and CAP-17.S02 candidates. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-17.S04 verified graph and CAP-18 methods/results drafting. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-17/index.html`; this slice page is `planning/review-site/CAP-17/CAP-17.S03.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

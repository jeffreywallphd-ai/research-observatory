...
slice_id: CAP-09.S03
title: Comparability sets and contradiction candidates
status: proposed
wave: W4
priority: P0
deployment_profiles:
...
| Slice | `CAP-09.S03` — Comparability sets and contradiction candidates |
| Slice outcome | Studies are normalized into defensible comparison sets before support, contradiction, or boundary inference. |
| Wave / priority | `W4` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-09.S03.T01`, `CAP-09.S03.T02`, `CAP-09.S03.T03` |
| Slice dependencies | `CAP-09.S02.T03`, `CAP-08.S05.T03` |
...
Studies are normalized into defensible comparison sets before support, contradiction, or boundary inference.
...
**Implementation thesis.** Determine whether studies are comparable before interpreting agreement or disagreement, using mode-specific hard gates and explainable similarity rather than letting semantic proximity manufacture contradictions.
...
- Mode-specific comparability dimensions and rule packs.
- Comparable-set clustering, pair scoring and explanation.
- Support, contradiction, qualification and boundary-condition candidate detection.

### 2.2 Explicit non-goals

- Declaring scientific truth from NLI output.
- Comparing claims that differ materially in construct, level, population or outcome.
- One universal comparability formula across all research modes.
...
- **Consumes:** `CAP-09.S02.T03`, `CAP-08.S05.T03` and the handoffs in Section 13.
- **Produces:** Studies are normalized into defensible comparison sets before support, contradiction, or boundary inference.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-09.S02.T03`, `CAP-08.S05.T03`.
...
|---|---|---|---|---|
| **Two-stage comparability** | Apply explicit incompatibility gates first, then score remaining pairs on construct definition, unit/level, population, context, time, method, measure, outcome and intervention/exposure. | Cluster only by embedding similarity or citation graph. | Hard semantic/methodological mismatches must be visible before softer similarity signals. | [Fact or Fiction: Verifying Scientific Claims](https://aclanthology.org/2020.emnlp-main.609/) |
| **Mode packs** | Systematic, theory, technical, critical and other modes version their dimensions, required fields, missingness treatment and thresholds. | Global weights hidden in the model. | What counts as comparable depends on the intended scholarly inference. | [PRISMA 2020 Statement](https://doi.org/10.1136/bmj.n71) |
| **Relation detection** | Generate typed candidates from normalized claims and evidence with explanatory contrasting dimensions; require human confirmation for promoted contradiction/boundary assertions. | Promote model labels directly to graph edges. | NLI/stance is a signal; context and measurement differences can explain apparent conflict. | [Fact or Fiction: Verifying Scientific Claims](https://aclanthology.org/2020.emnlp-main.609/) |
...
|---|---|
| `services/core/research_observatory/modules/comparability/rules.py` | Versioned dimension/gate packs. |
| `services/core/research_observatory/modules/comparability/engine.py` | Pair features, gates, scores and explanations. |
| `services/core/research_observatory/modules/comparability/clustering.py` | Deterministic set formation and diagnostics. |
| `services/core/research_observatory/modules/comparability/relation_detector.py` | Support/contradiction/qualification/boundary candidates. |
| `packages/ui/graph/comparability` | Pair explanation and promotion review. |
| `tests/benchmarks/comparability` | Gold comparable/noncomparable/qualified claim pairs. |
...
|---|---|
| `ComparabilityRulePack` | mode, dimensions, hard gates, missingness, weights, thresholds and version |
| `StudyComparisonProfile` | normalized values plus source evidence for each dimension |
| `ComparabilityAssessment` | pair, gate outcomes, features, score, explanation, assessor and status |
| `ComparisonSet` | membership, formation policy, centroid/summary and unresolved exclusions |
| `ClaimRelationAssessment` | candidate type, claim pair, comparability, evidence and alternative explanation |
...
- `ComparabilityEngine.assess(a, b, rule_pack)` is deterministic for fixed profiles/policy.
- `ComparisonSetBuilder.build(candidate_pool, policy)` records all exclusions and reasons.
- `ClaimRelationDetector.propose(set_snapshot, model_policy)` consumes only comparable pairs.
- `ComparisonReview.confirm/qualify/reject` emits typed graph assertion or preserves candidate.
...
- Users see why two studies are or are not comparable dimension by dimension.
- Missing fields are shown as unknown rather than silently imputed.
- Contradiction candidates foreground alternative explanations such as measure/context/time differences.
- Threshold and mode changes can be previewed as membership diffs before recalculation.
...
- Sensitive populations/contexts are access-filtered before pair generation and UI display.
- Remote model comparison is disabled for confidential passages unless explicitly approved.
- Large pair jobs enforce quotas and cancellation to prevent local resource exhaustion.
...
|---|---|
| Required dimension unknown | Apply pack policy: exclude, provisional or route to coding; never treat unknown as match. |
| Combinatorial explosion | Block/prune by hard facets, bounded candidate generation and resumable batches. |
| Model relation disagreement | Retain per-model/human alternatives; do not average into a fact. |
| Rule pack changes | Version outputs, mark dependent sets/relations/syntheses stale and offer differential rebuild. |
...
| Verification profiles | `graph`, `evidence` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S03.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `graph`, `search`, `ai` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S03.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-09.S03.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
|---|---|
| Pair feature evaluation | >= 10,000 deterministic pairs/s for cached normalized profiles. |
| Set build | 100,000 studies with blocking completes < 15 min on reference PC; no all-pairs path. |
| Candidate review query | p95 < 500 ms for filtered queues. |
| Recalculation | Checkpoint and resume after cancellation/restart with no duplicate assessments. |
...
- gold comparability precision/recall
- gate exclusion reasons
- set stability across versions
- candidate precision by relation type
- human confirmation/correction
- pair workload avoided by blocking
...
- Consumes normalized evidence/entity/claim records from CAP-08/S02.
- Produces comparison sets and relation candidates for S04 maps, S05 synthesis and CAP-10 detectors.
- Later empirical result integration reuses rule-pack architecture with separate evidence classes.
...
| Risk | Mitigation |
|---|---|
| False contradiction | Hard comparability gates, explanatory dimensions and human promotion. |
| Rules encode one epistemology | Mode/domain packs, editable assumptions and versioned alternatives. |
| Pair scaling | Blocking indexes and explicit budgets. |

## 19. Required ADRs and human decisions

- ADR-CAP09-COMPARABILITY: dimension packs, hard gates and promotion policy.

...
slice_id: CAP-10.S05
title: Critical and hermeneutic research support
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-10.S05` — Critical and hermeneutic research support |
| Slice outcome | The system surfaces evidence-linked candidate assumptions and alternative framings without replacing interpretive authority. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-10.S05.T01`, `CAP-10.S05.T02`, `CAP-10.S05.T03` |
| Slice dependencies | `CAP-10.S03.T02`, `CAP-08.S01.T03` |
...
The system surfaces evidence-linked candidate assumptions and alternative framings without replacing interpretive authority.
...
**Implementation thesis.** Support critical and hermeneutic scholarship by offering evidence-linked candidate readings, competing interpretations and iterative memo/search lineage while keeping final interpretation and normative claims under researcher authority.
...
- Editable critical ontology pack and candidate extraction.
- Competing-reading/problematization workspace.
- Hermeneutic search-read-interpret cycles and memo lineage.

### 2.2 Explicit non-goals

- Automating critical theory or declaring hidden assumptions as facts.
- Forcing all critical traditions into one ontology.
- Treating hermeneutic inquiry as a one-pass systematic review.
...
- **Consumes:** `CAP-10.S03.T02`, `CAP-08.S01.T03` and the handoffs in Section 13.
- **Produces:** The system surfaces evidence-linked candidate assumptions and alternative framings without replacing interpretive authority.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-10.S03.T02`, `CAP-08.S01.T03`.
...
|---|---|
| `services/core/research_observatory/modules/critical/ontology_pack.py` | Core and extension pack contracts. |
| `services/core/research_observatory/modules/critical/candidates.py` | Passage-grounded candidate readings and alternatives. |
| `services/core/research_observatory/modules/hermeneutic/cycles.py` | Search-read-interpret cycle and evolving question state. |
| `services/core/research_observatory/modules/memos` | Researcher-owned memo revisions and links. |
| `packages/ui/critical-lens` | Competing readings, evidence and adjudication. |
| `packages/ui/research-notebook` | Cycle timeline, memos, question/schema changes and next search. |
...
|---|---|
| `CriticalLensPack` | tradition/domain, concepts, prompts, examples, exclusions and version |
| `CriticalReadingCandidate` | passage, lens, interpretation, alternatives, uncertainty and status |
| `InterpretiveAdjudication` | accepted/rejected/qualified/unresolved reading and researcher rationale |
| `HermeneuticCycle` | starting question, searches, readings, memos, changed understanding and next moves |
| `ResearchMemo` | researcher-authored immutable revisions, links, visibility and AI-assistance disclosure |
...
- `CriticalCandidateService.propose(passages, lens_pack, policy)` returns multiple evidence-linked readings.
- `InterpretationCommand.adjudicate` requires researcher role and preserves alternatives.
- `HermeneuticCycleService.open/record_reading/record_shift/close` links all activities.
- `MemoService.create/revise/link` never silently overwrites researcher prose.
...
- Critical Lens shows passage, candidate interpretation, counter-reading, affected stakeholders and open questions side by side.
- Users can compare different theoretical lenses without selecting one as default truth.
- Research Notebook visualizes cycles and how questions/terms/corpus/schema changed.
- The workflow encourages returning to Search Studio or source reading with the current memo/context.
...
- Memos and unpublished interpretations are private by default; team visibility is explicit.
- AI may summarize/link memos only within approved policy and cannot overwrite them.
- Sensitive stakeholder/harm annotations inherit source/project classification and export limits.
...
|---|---|
| Thin evidence | Candidate is labeled speculative and cannot be accepted without passage/context review. |
| Lens pack updated | Preserve prior readings, mark candidates for optional reconsideration, never rewrite researcher adjudication. |
| AI homogenizes alternatives | Require diverse/counter-reading prompt/evaluation and allow lens-specific human templates. |
| Interrupted cycle | Persist every search/read/memo transition; resume from explicit current understanding. |
...
| Review gate | `critical-method-review` |
| Verification profiles | `evidence`, `novelty` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S05.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S05.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `interpretive-review` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S05.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
3. Use the declared representative corpus and gold queries to measure recall, ranking quality, bias/coverage and latency. Record ablations and make degraded/fallback behavior user-visible.
...
|---|---|
| Candidate generation | Bounded selected passages; default <= 5 alternatives per passage and explicit model budget. |
| Workspace | Load current passage/readings/memos < 500 ms cached. |
| Cycle history | Paginated timeline p95 < 500 ms for 10,000 events. |
| Researcher authority | 100% accepted interpretations include named researcher decision/rationale. |
...
- candidate/alternative/adjudication counts
- lens-pack usage
- researcher corrections
- cycles/search-return/read/memo activity
- unresolved plurality
- agency/trust evaluation
...
- Consumes CAP-05 passages, CAP-08 evidence/ontologies and CAP-06 search.
- Provides accepted interpretations and memos to S03 dossiers, CAP-16 critical blueprints and CAP-18 drafting.
- Does not convert interpretations into empirical evidence records.
...
| Risk | Mitigation |
|---|---|
| Critical overreach | Candidate/provocation status and human interpretive authority. |
| Ontology lock-in | Versioned packs, competing coding and original-language preservation. |
| Researcher de-skilling | Source-first workflow and memo ownership, not auto-conclusions. |

## 19. Required ADRs and human decisions

- ADR-CAP10-INTERPRETIVE: critical candidate status, ontology extension and hermeneutic cycle semantics.
...
- **Generating Research Questions Through Problematization** — Assumption-challenging research-question methodology.  
  https://doi.org/10.5465/amr.2009.0188
- **A Hermeneutic Approach for Conducting Literature Reviews and Literature Searches** — Iterative search-reading-interpretation cycles and evolving understanding.  

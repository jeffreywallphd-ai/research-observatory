...
slice_id: CAP-18.S01
title: Manuscript project and section workflow
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-18.S01` — Manuscript project and section workflow |
| Slice outcome | Drafts are durable, versioned scholarly objects with human ownership and selective recalculation. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-18.S01.T01`, `CAP-18.S01.T02`, `CAP-18.S01.T03` |
...
Drafts are durable, versioned scholarly objects with human ownership and selective recalculation.
...
**Implementation thesis.** Create a stable manuscript project and section workflow whose canonical state is independent of editor framework and output format.
...
- Manuscript project, document, section, block and author-ownership state.
- Structured editor adapter and stable block ids.
- Section readiness, locks, review/acceptance and human gates.
- Dependency and selective-redrafting model.
...
- **Consumes:** `CAP-16 approved blueprint and CAP-03 workflow/dependency fabric` and formal dependencies `CAP-16.S05.T03`.
- **Produces:** Drafts are durable, versioned scholarly objects with human ownership and selective recalculation.
...
| `CAP-18-D02` | Canonical draft representation | **Persist a versioned semantic block tree with stable IDs and separate rendered exports** | Persist DOCX or HTML as the sole canonical manuscript | A portable domain model supports selective redrafting, review links and multiple exports. | None |
| `CAP-18-D03` | Collaboration strategy | **Keep local revision history canonical and add Yjs as an optional hosted collaboration adapter later** | Make a CRDT the canonical state for all editions immediately | The PC/lab product does not need collaboration complexity, while the adapter preserves a later path. | None |
| `CAP-18-D04` | Section workflow | **Use typed readiness, locks, authorship, review and acceptance states per section** | Allow any agent to rewrite any section at any time | Consequence-aware section gates preserve researcher ownership and reproducibility. | None |
...
|---|---|
| `packages/contracts/manuscript-draft` | portable draft schemas |
| `packages/core/manuscript-draft` | aggregate and section workflow |
| `apps/desktop/manuscript-editor` | ProseMirror adapter |
| `services/core-api/manuscripts/drafts` | commands, queries and locks |
...
|---|---|
| `ManuscriptDraft` | Versioned ManuscriptDraft state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `DraftRevision` | Versioned DraftRevision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `SectionDraft` | Versioned SectionDraft state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `ContentBlock` | Versioned ContentBlock state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `AuthorOwnership` | Versioned AuthorOwnership state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `SectionGate` | Versioned SectionGate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `ChangeSet` | Versioned ChangeSet state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
| `DraftDependency` | Versioned DraftDependency state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript project and section workflow. |
...
- `Editor transactions map to domain commands and cannot bypass gates.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Stable block IDs survive edit/reorder and appear in review/export sidecars.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Accepted researcher text is never overwritten without a visible change set.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-16 approved blueprint and CAP-03 workflow/dependency fabric.
- **Downstream:** CAP-18.S02 drafting, CAP-18.S05 publication exports and CAP-19 review snapshots.
...
- Use `manuscript-studio.html` as governed reference.
- Show section readiness, ownership, stale state and next workflow action.
...
- Protect local drafts and autosave revisions.
- Sanitize pasted HTML and untrusted embedded content.
...
|---|---|---|
| editor transaction conflicts with section lock | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| autosave interrupted | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| schema migration cannot represent an old block | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| dependency change stales accepted text | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| ProseMirror-domain round-trip | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| revision/undo/change-set tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| crash/restart autosave recovery | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| selective stale propagation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| typing and local transactions remain perceptually immediate | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| large manuscript opens under approved target | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| revision compaction preserves complete audit history | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-16 approved blueprint and CAP-03 workflow/dependency fabric. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-18.S02 drafting, CAP-18.S05 publication exports and CAP-19 review snapshots. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-18/index.html`; this slice page is `planning/review-site/CAP-18/CAP-18.S01.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

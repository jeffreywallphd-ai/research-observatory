...
slice_id: CAP-17.S01
title: Private technical-report and study-artifact intake
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-17.S01` — Private technical-report and study-artifact intake |
| Slice outcome | Unpublished study results enter through a confidential, rights-aware, versioned channel. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-17.S01.T01`, `CAP-17.S01.T02`, `CAP-17.S01.T03` |
...
Unpublished study results enter through a confidential, rights-aware, versioned channel.
...
**Implementation thesis.** Treat uploaded reports and study artifacts as confidential, immutable research evidence with explicit custody, rights and revision lineage.
...
- Private artifact intake and study association.
- Artifact classification, rights, egress, retention and access policy.
- Immutable revision/correction/supersession lineage.
- Content-addressed storage and malware/untrusted-content checks.
...
- **Consumes:** `CAP-02 protected storage, CAP-05 document acquisition/parsing and CAP-15 StudyDesign identity` and formal dependencies `CAP-15.S01.T03`.
- **Produces:** Unpublished study results enter through a confidential, rights-aware, versioned channel.
...
|---|---|---|---|---|
| `CAP-17-D01` | Artifact custody | **Store technical reports and study artifacts in the project private artifact vault by default** | Upload all reports to a remote model/provider during intake | Unpublished results are highly sensitive and local-first custody preserves the existing trust boundary. | ADR-RESULT-ARTIFACT-CUSTODY |
...
| `CAP-17-D03` | Revision lineage | **Use immutable artifact revisions with correction and supersession relationships** | Overwrite prior reports when a corrected file is uploaded | Study-result provenance and manuscript staleness require complete version history. | None |
| `CAP-17-D04` | Confidentiality and rights | **Apply artifact classification, project access, egress policy, retention and export controls at intake** | Rely on folder location and user memory | Policy must travel with every derivative record and external call. | None |
...
|---|---|
| `services/core-api/study-artifacts` | intake and association commands |
| `packages/core/results/artifacts` | custody and lineage domain |
| `workers/artifact-intake` | format detection, hashing and safe staging |
| `packages/storage/artifact-vault` | encrypted project-scoped object storage |
...
|---|---|
| `StudyArtifact` | Versioned StudyArtifact state with stable identity, lifecycle/status, provenance and policy metadata appropriate to private technical-report and study-artifact intake. |
| `TechnicalReport` | Versioned TechnicalReport state with stable identity, lifecycle/status, provenance and policy metadata appropriate to private technical-report and study-artifact intake. |
| `ArtifactRevision` | Versioned ArtifactRevision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to private technical-report and study-artifact intake. |
| `ArtifactClassification` | Versioned ArtifactClassification state with stable identity, lifecycle/status, provenance and policy metadata appropriate to private technical-report and study-artifact intake. |
| `ArtifactPolicy` | Versioned ArtifactPolicy state with stable identity, lifecycle/status, provenance and policy metadata appropriate to private technical-report and study-artifact intake. |
| `SupersessionRelation` | Versioned SupersessionRelation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to private technical-report and study-artifact intake. |
...
- `A file hash/revision is immutable after acceptance.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Every derivative retains artifact/revision and policy identifiers.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `No external provider call occurs without explicit egress decision.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-02 protected storage, CAP-05 document acquisition/parsing and CAP-15 StudyDesign identity.
- **Downstream:** CAP-17.S02 parsing/extraction and CAP-17.S05 workspace.
...
- Use `technical-reports.html` intake and version history surfaces.
- Display classification, study link, parsing status and allowed egress before processing.
...
- Quarantine uploads, validate type/size, scan where available and never execute embedded content.
- Enforce project authorization and protected local storage.
...
|---|---|---|
| malicious or malformed upload | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| duplicate artifact with conflicting metadata | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| correction uploaded while parsing is active | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| policy denies requested model/provider | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | results, documents, security-local |
...
| Verification profiles | results, security-local |
...
|---|---|---|
| hash/idempotent intake | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| rights/egress denial paths | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| revision/supersession graph integrity | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| crash/restart during staging and commit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| large report intake streams without loading whole file in memory | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| duplicate detection completes within local profile budget | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| temporary staging is quota-bound and cleaned | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-02 protected storage, CAP-05 document acquisition/parsing and CAP-15 StudyDesign identity. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-17.S02 parsing/extraction and CAP-17.S05 workspace. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-17/index.html`; this slice page is `planning/review-site/CAP-17/CAP-17.S01.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

...
slice_id: CAP-14.S01
title: Platform abstraction and build matrix
status: proposed
wave: W6
priority: P0
deployment_profiles:
...
| Slice | `CAP-14.S01` — Platform abstraction and build matrix |
| Slice outcome | One codebase builds and reports its capabilities consistently across qualified desktop operating systems. |
| Wave / priority | `W6` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `platform-neutral`, `macos-arm64`, `linux-x64`, `linux-arm64` |
| Backlog tasks | `CAP-14.S01.T01`, `CAP-14.S01.T02`, `CAP-14.S01.T03` |
...
One codebase builds and reports its capabilities consistently across qualified desktop operating systems.
...
**Implementation thesis.** Establish a cross-platform build and adapter contract before porting so macOS and Linux qualification extends the Windows product rather than creating divergent implementations.
...
- Platform capability/adapter inventory.
- Four-target ci/build matrix.
- Target-specific sidecar/native dependency manifests.
...
- **Consumes:** `G5 Windows release contracts` and formal dependencies `CAP-11.S06.T03`.
- **Produces:** One codebase builds and reports its capabilities consistently across qualified desktop operating systems.
...
|---|---|---|---|---|
| `CAP-14-D01` | Codebase strategy | **Use one shared desktop/core codebase with narrow platform adapter ports and no feature forks** | Maintain independent Windows, macOS and Linux implementations | Shared semantics and tests are essential for project portability and maintainability. | ADR-DESKTOP-PLATFORM-PORTS |
| `CAP-14-D02` | Qualification matrix | **Qualify Windows x64, Apple Silicon macOS, Linux x86_64 and Linux ARM64 as explicit targets** | Claim generic cross-platform support from framework compatibility | Native dependencies, sidecars, credential stores and installers require target-specific evidence. | None |
| `CAP-14-D07` | Sidecar packaging | **Build and sign target-triple-specific Rust/Python/native sidecars from pinned manifests** | Download executable dependencies at first launch | Offline operation and supply-chain control require known local artifacts. | None |
| `CAP-14-D08` | Python runtime | **Bundle a reproducible target-specific Python runtime and wheel set** | Depend on the user’s system Python | Scientific dependencies and ABI compatibility cannot be delegated to arbitrary environments. | None |
...
|---|---|
| `packages/platform` | filesystem, secrets, process, shell and accelerator ports |
| `build/matrix` | Windows x64, macOS arm64, Linux x64/arm64 |
| `build/native-manifest` | sidecars, wheels and libraries per target |
...
|---|---|
| `PlatformTarget` | Versioned PlatformTarget state with stable identity, lifecycle/status, provenance and policy metadata appropriate to platform abstraction and build matrix. |
| `PlatformCapability` | Versioned PlatformCapability state with stable identity, lifecycle/status, provenance and policy metadata appropriate to platform abstraction and build matrix. |
| `NativeArtifact` | Versioned NativeArtifact state with stable identity, lifecycle/status, provenance and policy metadata appropriate to platform abstraction and build matrix. |
| `BuildManifest` | Versioned BuildManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to platform abstraction and build matrix. |
| `CompatibilityFinding` | Versioned CompatibilityFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to platform abstraction and build matrix. |
...
- `PlatformServices.resolve(target)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `NativeArtifactRegistry.require(target,version)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `BuildMatrix.qualify(revision,target)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** G5 Windows release contracts.
- **Downstream:** macOS/Linux qualification and portable runtime.
...
- Platform limitations are reported in product language, not raw native errors.
- Feature availability does not silently change navigation or scholarly semantics.
- Help and diagnostics identify OS, architecture and selected runtime backends.
...
- Every target artifact is pinned, hashed and eventually signed/notarized.
- No first-run executable download is required for core offline operation.
- Platform adapters validate paths, shell actions and external process arguments.
...
|---|---|---|
| Native wheel/sidecar missing for one architecture | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Framework reports support but packaged runtime fails | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Platform adapter leaks an OS-specific path into canonical state | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| CI target uses wrong architecture artifact | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | desktop-cross-platform, architecture |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | desktop-cross-platform, foundation |
...
|---|---|---|
| Adapter contract tests on all targets | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Clean target builds from pinned manifests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Native dependency/license inventory | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Unsupported target produces explicit diagnostic rather than partial install | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Shared-code percentage and fork count monitored | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Build/release time bounded by target parallelism | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Target artifact size disclosed and optimized without runtime downloads | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
G5 Windows release contracts. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

macOS/Linux qualification and portable runtime. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-14/index.html`; this slice page is `planning/review-site/CAP-14/CAP-14.S01.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

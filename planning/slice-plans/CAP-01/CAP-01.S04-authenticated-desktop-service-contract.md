...
slice_id: CAP-01.S04
title: Authenticated desktop-service contract
status: proposed
wave: W1
priority: P0
deployment_profiles:
...
| Slice | `CAP-01.S04` - Authenticated desktop-service contract |
| Slice outcome | Local IPC is private, versioned, cancellable, and observable. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-01.S04.T01`, `CAP-01.S04.T02`, `CAP-01.S04.T03` |
| Slice dependencies | `CAP-01.S03.T03` |
...
Create a narrow, authenticated, versioned seam between the desktop experience and durable research services so deployment can later move from local to institutional without rewriting the UI.
...
**Implementation thesis.** Bind Core to loopback only, authenticate every request with an ephemeral per-launch token, publish OpenAPI 3.1 contracts, generate the TypeScript client, and standardize errors, optimistic concurrency, idempotency, progress, and diagnostics.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-01.S03.T03`.
- **Produces:** Local IPC is private, versioned, cancellable, and observable.
...
- `planning/backlog.yaml` for `CAP-01.S04` and its task state/dependencies.
...
- `CAP-01.S03.T03` is complete or explicitly gated.
...
1. **Generate a cryptographically random 256-bit bearer token at each launch and transfer it only through the inherited startup channel; never persist it.**
2. **Bind to 127.0.0.1 and ::1 only, select an ephemeral port, and validate Host/Origin expectations.**
3. **Use OpenAPI 3.1 as the source of HTTP contract truth and generate a checked-in or build-generated TypeScript client with drift tests.**
4. **Represent API failures using RFC 9457 problem details with stable error codes and safe user guidance.**
5. **Use idempotency keys for commands, ETags/If-Match for mutable resources, trace IDs, and SSE for progress/event streams.**
6. **Keep support bundles opt-in, previewable, redacted, and free of research content by default.**
...
- Core authentication middleware and loopback binding policy.
- Version/capability handshake endpoint and generated client pipeline.
- Problem-details library, idempotency store, concurrency middleware, and SSE client.
- Desktop diagnostics and support-bundle workspace.
...
- Every operation declares authentication, idempotency, concurrency, offline, and cancellation semantics.
- Client generation fails CI when OpenAPI output differs from committed/generated contract artifacts.
- Problem details include type, title, status, stable code, trace ID, retryability, and safe remediation; sensitive internals are excluded.
- SSE events carry operation ID, monotonically increasing sequence, state, progress, optional artifact reference, and terminal disposition.
...
- Diagnostics shows desktop/Core versions, compatibility, process state, local storage status, and redacted check results.
- Users can inspect exactly what a support bundle contains before export.
- Lost SSE connections reconnect from the last event ID and the UI reconciles with authoritative operation state.
...
- Reject unauthenticated, wrong-token, non-loopback, unexpected-host, and cross-origin requests.
- Apply request size limits and schema validation at the boundary.
- Do not expose interactive API docs in production local builds unless gated by a developer profile.
- Rotate the token on every Core restart.
...
- A client generated for an incompatible API fails during handshake with explicit upgrade guidance.
- Repeated command submission returns the original result when the idempotency key and payload hash match; conflicts are explicit when payload differs.
- Diagnostics collection continues around failed checks and records unavailable sections rather than failing the entire bundle.
...
### 9.1 `CAP-01.S04.T01` - Implement loopback authentication and endpoint binding controls
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S04.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-01.S04.T02` - Generate and consume versioned API contracts
...
**Verification profiles:** `desktop`, `service`

**Expected deliverables**
- OpenAPI-derived TypeScript client, error envelope, pagination, job-status, cancellation, and compatibility rules.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S04.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-01.S04.T03` - Build desktop diagnostics and support-bundle collection
...
**Risk / review gate:** `medium` / `agent-review`  
**Verification profiles:** `desktop`, `service`, `security-local`

**Expected deliverables**
- Diagnostics page for component versions, health, storage paths, resource use, recent failures, and redacted support export.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S04.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
...
|---|---|
| Backlog profiles | `desktop`, `security-local`, `service` |
...
- Handshake and health requests target <=100 ms local p95 after readiness.
- Generated client adds no runtime schema duplication beyond boundary validation.
- SSE backpressure and event retention are bounded.
...
- Trace IDs cross renderer, Tauri command, Core request, workflow operation, and support bundle.
- Record auth failures and contract mismatches without logging tokens or payloads.
...
- All later desktop workspaces depend on this contract and must not bypass it with direct database access.
- CAP-03.S04 provides durable operation state for progress APIs.
- Hosted deployments reuse domain/API contracts but replace launch-token authentication with institutional/cloud identity.
...
- Additive API changes remain compatible within a declared range; breaking changes require a new major contract and coordinated desktop/Core release.
- Generated client artifacts carry source OpenAPI hash and generator version.
...
- The promised outcome is demonstrable end to end: Local IPC is private, versioned, cancellable, and observable.
...
- ADR: OpenAPI client generator and checked-in/generated artifact policy.
- ADR: Local SSE versus WebSocket progress transport.
...
|---|---|---|
| `FASTAPI_CLIENT` | [FastAPI - Generate Clients](https://fastapi.tiangolo.com/advanced/generate-clients/) - FastAPI | OpenAPI-derived TypeScript clients and contract drift control. |
| `OPENAPI` | [OpenAPI Specification 3.1.1](https://spec.openapis.org/oas/v3.1.1.html) - OpenAPI Initiative | Versioned HTTP API contracts. |
| `RFC9457` | [RFC 9457 - Problem Details for HTTP APIs](https://www.rfc-editor.org/rfc/rfc9457.html) - IETF | Machine-readable and actionable API failure envelopes. |
...
2. Confirm `CAP-01` is the active capability campaign and `CAP-01.S04` is the next eligible slice.
3. Claim the first READY task in order: `CAP-01.S04.T01`. Do not globally select work outside the capability.

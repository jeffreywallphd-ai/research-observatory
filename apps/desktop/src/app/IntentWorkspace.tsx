...
export function IntentWorkspace({
  ...
  if (!availability.available) {
    return <section className="intent-workspace ro-page-region" aria-labelledby="intent-title"><div className="page-header"><Typography id="intent-title" as="h1" variant="page-title">Research Intent Contract</Typography></div><Notification tone="warning" title="Research intent unavailable">{availability.message}</Notification></section>;
  }

  return (
    <section className="intent-workspace ro-page-region" aria-labelledby="intent-title" data-intent-workspace="true">
      ...
      {!workspace ? <p role="status">{busy === "load" ? "Loading the local intent ledger…" : "Research intent could not be loaded."}</p> : (
        <form className="intent-form ro-form" onSubmit={save}>
          <Panel title="Purpose and intended contribution">
            ...
            <div className="intent-three-column ro-grid">
          ...
          <Panel title="Primary use case and guided workflow">
            ...
            <div className="intent-guidance ro-notice">
          ...
          <Panel title="Save or accept intent revision">
            ...
            {showHistory ? <ol className="intent-history ro-stack">{workspace.history.map((item) => <li key={item.revisionId}>Revision {item.revision} · {item.primaryUseCase} · {item.unresolvedDecisionCount} unresolved · <code>{item.revisionContentHash.slice(0, 20)}…</code></li>)}</ol> : null}
          </Panel>
        </form>
      )}
    </section>

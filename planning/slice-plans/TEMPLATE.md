...
> **Implementation gate.** Before implementation, the capability decision packet must be decision-complete and approved, this plan must be approved, required ADRs/reference changes must be approved, and `python tools/planctl.py ready CAP-XX --require-approved` must pass. The planning agent must create this plan from the canonical template if it is missing.

...
slice_id: CAP-11.S03
title: Offline, privacy, and local security acceptance
status: proposed
wave: W5
priority: P0
deployment_profiles:
...
| Slice | `CAP-11.S03` — Offline, privacy, and local security acceptance |
| Slice outcome | The local edition has verified no-account/offline behavior and a reviewed threat/control baseline. |
| Wave / priority | `W5` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-11.S03.T01`, `CAP-11.S03.T02`, `CAP-11.S03.T03` |
| Slice dependencies | `CAP-11.S02.T03`, `CAP-07.S03.T03`, `CAP-09.S06.T03` |
...
The local edition has verified no-account/offline behavior and a reviewed threat/control baseline.
...
**Implementation thesis.** Prove the local edition can complete core work with the network blocked and that every egress, deletion, rights and update path matches a reviewed local threat model.
...
- Air-gapped end-to-end acceptance.
- Local threat model and attack fixtures.
- Privacy, rights, deletion and ai disclosure review.
...
- **Consumes:** `CAP-11 recovery plus CAP-07 provider policy and CAP-09 exports` and formal dependencies `CAP-11.S02.T03`, `CAP-07.S03.T03`, `CAP-09.S06.T03`.
- **Produces:** The local edition has verified no-account/offline behavior and a reviewed threat/control baseline.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-11.S02.T03`, `CAP-07.S03.T03`, `CAP-09.S06.T03`.
...
|---|---|---|---|---|
| `CAP-11-D07` | Offline baseline | **Treat no-account, network-blocked operation as a release-gated core product path** | Treat offline operation as best-effort degraded behavior | Local-first privacy and PC/lab use are core product commitments. | None |
| `CAP-11-D08` | Local security acceptance | **Maintain a threat model and attack-fixture suite for IPC, documents, plugins, models, paths, logs and updates** | Rely on framework defaults and antivirus | The desktop processes untrusted scholarly and model artifacts across several native boundaries. | None |
| `CAP-11-D09` | Privacy and telemetry | **Default telemetry off/local; require visible egress preview, rights checks and deletion evidence** | Enable usage/content telemetry by default | Unpublished research and source content require conservative disclosure controls. | None |
...
|---|---|
| `tests/e2e/offline` | network-denied research workflows |
| `security/threat-model` | assets, boundaries and abuse cases |
| `modules/privacy` | egress inventory, deletion and disclosure evidence |
...
|---|---|
| `ThreatScenario` | Versioned ThreatScenario state with stable identity, lifecycle/status, provenance and policy metadata appropriate to offline, privacy, and local security acceptance. |
| `EgressDecision` | Versioned EgressDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to offline, privacy, and local security acceptance. |
| `DeletionRun` | Versioned DeletionRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to offline, privacy, and local security acceptance. |
| `DisclosureManifest` | Versioned DisclosureManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to offline, privacy, and local security acceptance. |
| `SecurityFinding` | Versioned SecurityFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to offline, privacy, and local security acceptance. |
...
- `NetworkPolicy.deny_all_except_loopback` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `EgressGuard.preview/authorize` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `DeletionService.plan/execute/verify` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-11 recovery plus CAP-07 provider policy and CAP-09 exports.
- **Downstream:** G5 privacy/security gate and lab support.
...
- Offline status is visible but does not dominate normal local operation.
- Online-only capabilities explain why they are unavailable and offer local alternatives.
- Deletion and provider egress use preview-and-confirm flows with verifiable completion.
...
- Loopback IPC uses authentication and origin checks.
- Documents, archives, plugins and model files are untrusted inputs.
- Telemetry is disabled or local by default and support export is opt-in.
...
|---|---|---|
| Unexpected DNS or provider call in offline mode | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Prompt-injection content attempts tool or egress actions | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Deletion misses derivative cache or backup reference | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Unsigned or tampered update/model artifact | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | security-local |
...
| Verification profiles | security-local, e2e-local |
...
|---|---|---|
| Network sandbox shows zero unauthorized outbound traffic | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Path traversal, archive bomb, malicious PDF and model fixtures | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Rights-denied view/export/cache tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Deletion inventory proves all governed copies handled | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Offline workflows complete within normal local performance envelopes | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Security tests run deterministically in CI/release qualification | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No raw source text or unpublished idea appears in diagnostics by default | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-11 recovery plus CAP-07 provider policy and CAP-09 exports. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

G5 privacy/security gate and lab support. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-11/index.html`; this slice page is `planning/review-site/CAP-11/CAP-11.S03.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

...
slice_id: CAP-18.S03
title: Empirical manuscript drafting
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-18.S03` — Empirical manuscript drafting |
| Slice outcome | The platform can draft a complete empirical article from verified study and literature evidence. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-18.S03.T01`, `CAP-18.S03.T02`, `CAP-18.S03.T03` |
| Slice dependencies | `CAP-18.S02.T03`, `CAP-17.S05.T03` |
...
The platform can draft a complete empirical article from verified study and literature evidence.
...
**Implementation thesis.** Draft empirical methods, results and discussion from approved design, actual conduct and verified outcomes with numerical and interpretive integrity.
...
- Methods drafting from planned and actual study state.
- Results drafting from verified result records and approved tables/figures.
- Discussion drafting across supported/null/mixed/contradictory outcomes.
- Limitations, deviations, robustness and alternative explanations.
...
- **Consumes:** `CAP-15 StudyDesign, CAP-17 verified StudyRun/results and CAP-16 empirical blueprint` and formal dependencies `CAP-18.S02.T03`, `CAP-17.S05.T03`.
- **Produces:** The platform can draft a complete empirical article from verified study and literature evidence.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-18.S02.T03`, `CAP-17.S05.T03`.
...
|---|---|---|---|---|
| `CAP-18-D08` | Methods drafting | **Draft methods from approved design plus verified actual-conduct/deviation records** | Draft methods from the original proposal only | Published methods must reflect what was actually done and disclose material deviations. | None |
...
| `CAP-18-D10` | Discussion drafting | **Integrate verified outcomes with literature, mechanisms, alternatives, boundaries and contribution under null/mixed findings** | Produce a generic positive-results discussion | A rigorous discussion must reflect actual outcome patterns and rival interpretations. | None |
...
|---|---|
| `packages/core/drafting/empirical` | empirical section policies |
| `workers/manuscript-drafting/empirical` | section generators |
| `workers/manuscript-verification/empirical` | number, table and result validators |
...
|---|---|
| `MethodsDraftCandidate` | Versioned MethodsDraftCandidate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical manuscript drafting. |
| `ResultsDraftCandidate` | Versioned ResultsDraftCandidate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical manuscript drafting. |
| `DiscussionDraftCandidate` | Versioned DiscussionDraftCandidate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical manuscript drafting. |
| `NumericalClaim` | Versioned NumericalClaim state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical manuscript drafting. |
| `OutcomeInterpretation` | Versioned OutcomeInterpretation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical manuscript drafting. |
| `DeviationDisclosure` | Versioned DeviationDisclosure state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical manuscript drafting. |
...
- `Methods reflect verified actual conduct and disclose material differences from plan.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Results contain no number/direction/significance not present in verified result state.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Discussion distinguishes evidence, interpretation and speculation.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-15 StudyDesign, CAP-17 verified StudyRun/results and CAP-16 empirical blueprint.
- **Downstream:** CAP-18.S05 complete manuscript and CAP-19 methods/evidence review.
...
- Show plan-versus-actual context in methods review.
- Link each numerical sentence to result record/table cell.
- Present alternative discussion framings and null/mixed contribution value.
...
- Protect participant/confidential study details.
- Apply disclosure/aggregation rules before draft packets.
...
|---|---|---|
| inconsistent sample sizes across results | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| table and prose disagree | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| planned analysis absent from actual report | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| discussion overgeneralizes beyond design | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | manuscript, results, study-design |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | manuscript, results, evidence |
...
|---|---|---|
| numerical exactness and rounding rules | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| method deviation completeness | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| outcome-pattern coverage | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| expert methods review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| section draft operations remain bounded/restartable | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| large result matrices use selected packets | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| verification catches every seeded numerical mutation | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-15 StudyDesign, CAP-17 verified StudyRun/results and CAP-16 empirical blueprint. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-18.S05 complete manuscript and CAP-19 methods/evidence review. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-18/index.html`; this slice page is `planning/review-site/CAP-18/CAP-18.S03.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

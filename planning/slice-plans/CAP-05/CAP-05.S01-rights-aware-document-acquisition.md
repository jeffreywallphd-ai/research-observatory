...
slice_id: CAP-05.S01
title: Rights-aware document acquisition
status: proposed
wave: W2
priority: P0
deployment_profiles:
...
| Slice | `CAP-05.S01` - Rights-aware document acquisition |
| Slice outcome | Full-text acquisition is explicit, resumable, checksum-verified, and governed by permitted use. |
| Wave / priority | `W2` / `P0` |
...
Acquire full text lawfully and reproducibly while keeping metadata-only scholarship useful when content cannot be obtained.
...
**Implementation thesis.** Treat local attachment and remote open-access retrieval as explicit durable acquisition jobs. Validate source, rights, redirects, MIME, size, checksum, and work-version association before committing an immutable document source object.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-04.S04.T02`, `CAP-02.S03.T03`.
- **Produces:** Full-text acquisition is explicit, resumable, checksum-verified, and governed by permitted use.
...
- `planning/backlog.yaml` for `CAP-05.S01` and its task state/dependencies.
...
- `CAP-04.S04.T02` is complete or explicitly gated.
- `CAP-02.S03.T03` is complete or explicitly gated.
...
1. **Support PDF, JATS, TEI, XML, HTML, DOCX, and plain text as intake formats, but classify parser suitability separately from declared file extension.**
2. **Hash and stage every local attachment before association; duplicate bytes reuse object storage while preserving separate acquisition/source assertions.**
3. **Remote acquisition consumes approved location candidates and action-specific rights policy; it is not a generic URL downloader.**
4. **Resolve/validate DNS and every redirect, allow only HTTPS by default, and block private, loopback, link-local, metadata, and file destinations.**
5. **Validate content type, magic bytes, extension, response size, decompression limits, checksum, and license/source metadata.**
6. **Preserve alternative copies and unavailable/entitlement-required states instead of overwriting or fabricating full-text availability.**
...
- Local attachment intake and work/version association service.
- Safe remote acquisition client and OA location selector.
- Acquisition queue/workflow, resumable staging, checksum verification, and object-store commit.
- Conflict/copy comparison and entitlement placeholder UI.
...
- AcquisitionSource records method, URI/path reference, provider, rights assertion, retrieved/attached time, expected/actual media type, size, checksum, and source work-version association.
- Acquisition job state includes candidate, authorized, downloading/copying, validating, committed, unavailable, denied, failed, cancelled, and quarantined.
- Document source commit is atomic with object reference, provenance, and rights metadata.
...
- Quarantine suspicious/mismatched files and never open them in the renderer.
- Do not follow authentication redirects or transmit institutional/provider credentials to unapproved hosts.
- Store originals encrypted and expose only controlled streams.
...
- Test interrupted local copy/download, disk full, duplicate byte content, wrong checksum, redirect loop, DNS rebinding, content spoofing, zip bomb, password protection, oversized response, and cancellation.
- Resume only when source semantics and server range support remain safe; otherwise restart from a clean staging object.
- Stale staging is cleaned through leases and recovery manifests.
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S01.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-05.S01.T02` - Implement open-access location selection and download
...
**Verification profiles:** `documents`, `security-local`

**Expected deliverables**
- Acquisition from approved OA locations with redirect controls, MIME validation, license capture, checksum, and retry.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S01.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
### 9.3 `CAP-05.S01.T03` - Create acquisition queue, conflict, and entitlement placeholders
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-05.S01.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
|---|---|
| Backlog profiles | `desktop`, `documents`, `security-local`, `service` |
...
- Stream all inputs with bounded memory and incremental hash/encryption.
- Use configurable but conservative file/response limits and display estimated size when available.
- Limit parallel downloads and respect provider policies.
...
- Record acquisition source/provider, policy result, redirects as hosts only, byte count, duration, checksum outcome, copy classification, and failure code.
- Do not log signed URLs, credentials, local paths, or document content.
...
- Consumes CAP-04.S03 work/version identity and CAP-04.S04 rights policy.
- Commits bytes through CAP-02.S03 and jobs through CAP-03.S04.
- CAP-05.S02 parses committed immutable sources.
- CAP-05.S04 enforces viewer actions from acquisition/rights assertions.
...
- Acquisition records are immutable assertions; improved metadata creates a new assertion/reconciliation, not silent rewrite.
- New intake formats add adapters without changing committed source-object identity.
...
- The promised outcome is demonstrable end to end: Full-text acquisition is explicit, resumable, checksum-verified, and governed by permitted use.
...
- ADR: Safe acquisition URL/destination policy and MIME/magic validation library.
- ADR: Local file quarantine and password-protected document policy.
...
2. Confirm `CAP-05` is the active capability campaign and `CAP-05.S01` is the next eligible slice.
3. Claim the first READY task in order: `CAP-05.S01.T01`. Do not globally select work outside the capability.

...
capability_plan: planning/capability-plans/CAP-08.md
planning_gate: capability-decision-complete
slice_id: CAP-08.S01
title: Core ontology and schema-pack registry
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Capability objective | Transform full text into source-grounded, mode-sensitive evidence records that distinguish observation, machine extraction, inference, verification, dispute, adjudication, and staleness. |
| Slice | `CAP-08.S01` — Core ontology and schema-pack registry |
| Slice outcome | A small stable scholarly core can be extended by domain and method packs under explicit version governance. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-08.S01.T01`, `CAP-08.S01.T02`, `CAP-08.S01.T03` |
| Slice dependencies | `CAP-03.S01.T03`, `CAP-07.S04.T02` |
| Capability decision packet | `planning/capability-plans/CAP-08.md` — must be approved and decision-complete |
| Approved experience | `RO-UI-ACADEMIC-MINIMAL-1.3`; relevant pages: schema-manager.html, evidence-matrix.html, document-reader.html, audit-lineage.html |
| Approval state | `PROPOSED` / human approval pending |

## 1. Purpose and contribution to the larger vision

A small stable scholarly core can be extended by domain and method packs under explicit version governance.

This slice advances the capability objective: **Transform full text into source-grounded, mode-sensitive evidence records that distinguish observation, machine extraction, inference, verification, dispute, adjudication, and staleness.** It is designed as one production vertical inside a long-running capability campaign, not as an isolated technical experiment. The implementation must preserve the platform’s evidence-before-prose rule, source and decision provenance, bounded uncertainty, researcher authority, local-first privacy, cross-platform ports, and the distinction between canonical scholarly state and rebuildable analytical derivatives.

**Implementation thesis.** Establish a compact interoperable ontology core and versioned disciplinary schema packs so evidence can be compared across projects without forcing every scholarly tradition into one fixed vocabulary.
...
- Stable core entity/relation/status vocabulary.
- Extraction-schema authoring, validation and semantic constraints.
- Pack import/export/fork/version comparison and controlled migration.

### 2.2 Explicit non-goals

- Encoding every theory or discipline in the core.
- Allowing schema packs to redefine immutable evidence/provenance semantics.
- Automatically declaring two author terms equivalent.
...
- **Consumes:** `CAP-03.S01.T03`, `CAP-07.S04.T02` and the handoffs in Section 13.
- **Produces:** A small stable scholarly core can be extended by domain and method packs under explicit version governance.
...
4. Approved capability decision packet `planning/capability-plans/CAP-08.md`.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-03.S01.T03`, `CAP-07.S04.T02`.
...
|---|---|---|---|---|
| **Core scope** | Core covers work/document/passage, claim/evidence, theory/construct/method/context, provenance/status/decision and opportunity references; domain detail lives in packs. | Large universal scholarly ontology. | A small core protects interoperability while limiting ontology reification. | [PROV-O: The PROV Ontology](https://www.w3.org/TR/prov-o/) |
| **Schema format** | JSON Schema 2020-12 for record shape plus a constrained semantic-rule registry and JSON-LD context for export. | Free-form JSON or OWL-only authoring. | JSON Schema is implementable across desktop/server; JSON-LD provides linked-data interchange without making RDF the local runtime. | [JSON Schema Draft 2020-12](https://json-schema.org/draft/2020-12) |
| **Evolution** | Immutable pack versions, explicit compatible/breaking diff, project pinning, fork lineage and migration preview. | Edit schema in place and reinterpret old records. | Evidence must retain the schema under which it was produced and reviewed. | [PROV-O: The PROV Ontology](https://www.w3.org/TR/prov-o/) |
...
|---|---|
| `services/core/research_observatory/modules/evidence_graph/ontology.py` | Core vocabulary, identifiers and invariants. |
| `services/core/research_observatory/modules/evidence_graph/schema_registry.py` | Pack versions, validation, fork/import/export and compatibility. |
| `packages/contracts/ontology` | JSON Schemas, JSON-LD contexts and semantic rule definitions. |
| `packages/ui/schema-manager` | Authoring, diff, preview, validation and project activation. |
| `services/workers/schema_migration` | Preview/revalidate/migrate derived records. |
| `tests/fixtures/schema-packs` | Core, IS theory, technical ML and critical packs. |
...
|---|---|
| `OntologyTerm` | stable ID, label, definition, synonyms, scope, status and source |
| `RelationType` | domain/range, direction, evidence/status rules and inverse |
| `SchemaPackVersion` | pack ID/version, JSON Schema, context, terms, rules, dependencies, license and hash |
| `ProjectSchemaBinding` | project, pack version, local extension/fork and activation state |
| `SchemaCompatibilityReport` | added/removed/changed fields, impact and migration requirements |
...
- `SchemaRegistry.validate(pack)` returns structural and semantic diagnostics.
- `EvidenceSchema.validate(record, pack_version)` never resolves “latest” implicitly.
- `SchemaRegistry.compare/fork/import/export` preserves identity and lineage.
- `ImpactGraph.preview_schema_change` lists evidence/extractions/syntheses requiring revalidation.
...
- Schema Manager separates core read-only terms from editable/forkable project packs.
- Authoring provides field type, cardinality, “not reported” behavior, evidence requirement and examples.
- Diff view highlights breaking semantics—not only JSON shape—and requires review before activation.
- Workflow presets recommend packs but users can inspect and reject them.
...
- Pack import is data-only, schema-limited and cannot execute code or load remote contexts at validation time.
- Licensing/source metadata travels with imported packs.
- Sensitive labels/fields receive data-class annotations used by egress/export policy.
...
|---|---|
| Invalid or cyclic pack dependency | Reject activation with exact dependency path. |
| Breaking update | Keep current pin; produce migration/re-extraction plan and require approval. |
| Unknown term | Preserve original wording and allow unresolved candidate; never force nearest term. |
| Corrupt imported pack | Quarantine, show hash/source and retain active version. |
...
| Verification profiles | `evidence`, `graph` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S01.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `desktop`, `evidence` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S01.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `evidence`, `service` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S01.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Define the versioned portable schema and invariants first. Validate examples and negative fixtures; keep provider, database, model, graph, and UI framework types behind adapters.
3. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
5. Build from an immutable snapshot into a staging destination; apply rights filters, produce checksums/manifests, verify independently, then atomically finalize. Test cancellation, disk failure and round trip.
...
|---|---|
| Record validation | p95 < 20 ms for ordinary evidence records. |
| Pack load | < 500 ms for 2,000 terms/200 fields after cache. |
| Version comparison | < 2 s for representative packs. |
| Migration preview | 100,000 evidence records impact-scanned < 60 s using indexed bindings. |
...
- pack/fork/version use
- breaking changes
- unresolved terms
- migration/revalidation counts
- schema-related disputes
...
- Consumes CAP-03 identifiers/provenance and CAP-05 source anchors.
- Supplies schemas to all CAP-08 extraction/evidence and CAP-09 graph relations.
- Critical pack extension is added in CAP-10.S05 without changing core authority.
...
| Risk | Mitigation |
|---|---|
| Ontology lock-in | Small core, forkable packs, original-language preservation and competing mappings. |
| Schema proliferation | Dependency/version catalog, governance and reuse recommendations. |
| Semantic breaking change hidden as compatible | Human review of definitions/rules and golden-record revalidation. |

## 19. Required ADRs and human decisions

- ADR-CAP08-ONTOLOGY: core vocabulary boundary, pack format and semantic-rule subset.
...
1. Run `python tools/planctl.py ready CAP-08 --require-approved` and `python tools/taskctl.py validate`.
2. Confirm the active campaign is `CAP-08`, this is the current slice, and the approved packet/plan commits match the campaign record.

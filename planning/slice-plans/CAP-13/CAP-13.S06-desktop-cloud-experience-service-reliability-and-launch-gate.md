...
slice_id: CAP-13.S06
title: Desktop-cloud experience, service reliability, and launch gate
status: proposed
wave: W11
priority: P3
deployment_profiles:
...
| Slice | `CAP-13.S06` — Desktop-cloud experience, service reliability, and launch gate |
| Slice outcome | Cloud customers use the canonical desktop with clear synchronization, reliability, support, and service-status behavior. |
| Wave / priority | `W11` / `P3` |
| Deployment profiles | `CLD` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-13.S06.T01`, `CAP-13.S06.T02`, `CAP-13.S06.T03` |
| Slice dependencies | `CAP-13.S04.T03`, `CAP-13.S05.T03` |
...
Cloud customers use the canonical desktop with clear synchronization, reliability, support, and service-status behavior.
...
**Implementation thesis.** Qualify the desktop-cloud product through explicit SLOs, canary/rollback discipline, transparent status and an end-to-end launch gate spanning scholarly, cost and isolation outcomes.
...
- Desktop cloud connection and status experience.
- Slo/error-budget/canary release operations.
- Complete saas launch acceptance.
...
- **Consumes:** `all CAP-13 slices` and formal dependencies `CAP-13.S04.T03`, `CAP-13.S05.T03`.
- **Produces:** Cloud customers use the canonical desktop with clear synchronization, reliability, support, and service-status behavior.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-13.S04.T03`, `CAP-13.S05.T03`.
...
|---|---|---|---|---|
| `CAP-13-D16` | Service reliability | **Publish SLOs, error budgets, canary releases and automated rollback** | Use uptime dashboards without release/error-budget policy | Reliability objectives must govern change velocity and incident response. | None |
| `CAP-13-D18` | Launch gate | **Require tenant-isolation, recovery, cost, security and complete desktop-cloud workflow evidence** | Launch after infrastructure and billing are connected | SaaS readiness is an end-to-end product and governance claim. | None |
...
|---|---|
| `packages/client-gateway/cloud` | region/tier/status behavior |
| `ops/reliability` | SLOs, canaries and rollback |
| `release/g11` | SaaS gate evidence and launch runbooks |
...
|---|---|
| `ServiceLevelObjective` | Versioned ServiceLevelObjective state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop-cloud experience, service reliability, and launch gate. |
| `ErrorBudget` | Versioned ErrorBudget state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop-cloud experience, service reliability, and launch gate. |
| `Canary` | Versioned Canary state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop-cloud experience, service reliability, and launch gate. |
| `Rollback` | Versioned Rollback state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop-cloud experience, service reliability, and launch gate. |
| `TenantStatus` | Versioned TenantStatus state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop-cloud experience, service reliability, and launch gate. |
| `CloudLaunchGate` | Versioned CloudLaunchGate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop-cloud experience, service reliability, and launch gate. |
...
- `CloudStatusService.status/incidents` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ReleaseController.canary/promote/rollback` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `LaunchGate.evaluate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** all CAP-13 slices.
- **Downstream:** managed cloud production and later portfolio monitoring.
...
- Cloud projects retain the same adaptive research workflows and supporting tools.
- Status, quota, cost and provider degradation are visible in user-relevant terms.
- Any new cloud administration/billing page requires governed design-reference approval first.
...
- Canary promotion verifies signed artifacts and isolation controls.
- Status endpoints reveal no tenant/content metadata.
- Launch credentials and production changes require explicit authorized release roles.
...
|---|---|---|
| Canary causes error-budget burn | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Region outage during active desktop work | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Desktop/server compatibility drift | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Billing/entitlement outage during launch | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | desktop, cloud |
...
|---|---|---|
| Complete local-like workflow in shared and isolated tiers | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| SLO/error-budget and rollback drills | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Cross-region/tenant isolation, backup and cost reconciliation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Independent security, privacy, operations and product review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Published SLO and support tiers backed by measured evidence | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Rollback within declared objective | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No critical/high unresolved launch blocker | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
all CAP-13 slices. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

managed cloud production and later portfolio monitoring. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-13/index.html`; this slice page is `planning/review-site/CAP-13/CAP-13.S06.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

...
slice_id: CAP-16.S02
title: Empirical article blueprints
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-16.S02` — Empirical article blueprints |
| Slice outcome | Empirical conference and journal skeletons are complete, adaptable, and linked to protocol/result requirements. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-16.S02.T01`, `CAP-16.S02.T02`, `CAP-16.S02.T03` |
| Slice dependencies | `CAP-16.S01.T03`, `CAP-15.S05.T02` |
...
Empirical conference and journal skeletons are complete, adaptable, and linked to protocol/result requirements.
...
**Implementation thesis.** Construct empirical blueprints from method-aware reporting requirements and live study-design/result readiness rather than a generic IMRaD shell.
...
- Journal and conference empirical structures.
- Method-family packs for quantitative, qualitative, mixed, computational, field and design-science work.
- Study-design and result requirement mappings.
- Section claim, evidence, disclosure, table/figure and word-budget plans.
...
- **Consumes:** `CAP-15 approved StudyDesign/Protocol and CAP-16.S01 manuscript/profile contracts` and formal dependencies `CAP-16.S01.T03`, `CAP-15.S05.T02`.
- **Produces:** Empirical conference and journal skeletons are complete, adaptable, and linked to protocol/result requirements.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-16.S01.T03`, `CAP-15.S05.T02`.
...
|---|---|---|---|---|
| `CAP-16-D07` | Empirical blueprint basis | **Drive empirical patterns from study design, verified conduct/results requirements and method/reporting profiles** | Use a universal IMRaD outline without method-specific requirements | Empirical completeness depends on design family, actual conduct and reporting standards. | None |
| `CAP-16-D10` | Section readiness | **Give every section typed inputs, required decisions, evidence expectations, completion state and blockers** | Use one manuscript-wide completion percentage | Section-level readiness enables safe drafting and selective recalculation. | None |
...
| `CAP-16-D12` | Claim planning | **Link planned contribution and section claims to evidence/result/argument requirements before drafting** | Generate claims only while writing prose | Claim plans are the bridge between evidence, blueprint, drafting and review. | None |
...
| `CAP-16-D16` | Drafting gate | **Prohibit generated manuscript prose until the blueprint and claim plan are approved** | Allow drafting from an incomplete outline | The blueprint is the human-approved architecture and prevents prose from silently determining the argument. | None |
...
|---|---|
| `packages/core/manuscript/empirical` | empirical architecture patterns |
| `packages/method-packs/manuscript` | method-specific section requirements |
| `services/core-api/manuscripts/empirical` | blueprint generation and readiness |
| `packages/contracts/manuscript/empirical` | protocol/result requirement links |
...
|---|---|
| `EmpiricalBlueprintPattern` | Versioned EmpiricalBlueprintPattern state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical article blueprints. |
| `MethodReportingProfile` | Versioned MethodReportingProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical article blueprints. |
| `SectionRequirement` | Versioned SectionRequirement state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical article blueprints. |
| `ResultRequirement` | Versioned ResultRequirement state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical article blueprints. |
| `MethodsConductRequirement` | Versioned MethodsConductRequirement state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical article blueprints. |
| `DisclosureRequirement` | Versioned DisclosureRequirement state with stable identity, lifecycle/status, provenance and policy metadata appropriate to empirical article blueprints. |
...
- `Methods blueprint distinguishes planned design from verified actual conduct.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Results sections cannot be marked ready without CAP-17 verified result coverage.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Method packs extend the core without changing stable manuscript semantics.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-15 approved StudyDesign/Protocol and CAP-16.S01 manuscript/profile contracts.
- **Downstream:** CAP-16.S05 Blueprint workspace and CAP-18 empirical drafting.
...
- Show missing upstream study-design and result requirements next to affected sections.
- Offer conference/journal variants with visible tradeoffs and researcher-controlled compression.
...
- Do not surface confidential protocol/result details beyond project authorization.
- Instrument and reporting-guideline rights metadata remain attached.
...
|---|---|---|
| no applicable method pack | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| contradictory study design and venue requirement | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| conference compression removes required integrity disclosure | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| result-dependent section requested before verified results exist | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | manuscript, study-design |
...
|---|---|---|
| golden blueprints for each supported empirical method family | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| reporting-guideline requirement coverage | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| conference/journal transformation diff tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| readiness-blocking tests for missing design/results | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| blueprint generation remains interactive on minimum local profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| no method pack may increase core schema validation beyond accepted budget | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| word-budget solver produces a feasible or explicit infeasible result | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-15 approved StudyDesign/Protocol and CAP-16.S01 manuscript/profile contracts. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-16.S05 Blueprint workspace and CAP-18 empirical drafting. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-16/index.html`; this slice page is `planning/review-site/CAP-16/CAP-16.S02.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

...
slice_id: CAP-10.S07
title: Living monitor and impact-aware research memory
status: proposed
wave: W9
priority: P1
deployment_profiles:
...
| Slice | `CAP-10.S07` — Living monitor and impact-aware research memory |
| Slice outcome | New literature is evaluated as a change to existing claims, syntheses, and opportunity assessments rather than as a generic alert. |
| Wave / priority | `W9` / `P1` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-10.S07.T01`, `CAP-10.S07.T02`, `CAP-10.S07.T03` |
| Slice dependencies | `CAP-10.S03.T03`, `CAP-09.S06.T01` |
...
New literature is evaluated as a change to existing claims, syntheses, and opportunity assessments rather than as a generic alert.
...
**Implementation thesis.** Monitor saved scholarly scopes as differential, provenance-rich retrievals and assess how new works may affect accepted claims, syntheses and opportunities, while requiring human review before corpus membership or prior conclusions change.
...
- Saved monitors and differential source retrieval with cursors/windows.
- New-paper triage and impact-candidate analysis.
- Living-review change reports, stale propagation and reassessment workflow.

### 2.2 Explicit non-goals

- Automatically including/excluding papers.
- Automatically revising synthesis or novelty conclusions.
- Continuous uncontrolled polling of every source.
...
- **Consumes:** `CAP-10.S03.T03`, `CAP-09.S06.T01` and the handoffs in Section 13.
- **Produces:** New literature is evaluated as a change to existing claims, syntheses, and opportunity assessments rather than as a generic alert.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-10.S03.T03`, `CAP-09.S06.T01`.
...
|---|---|---|---|---|
| **Monitor definition** | Bind a monitor to versioned queries/search branches, sources, schedule, cursor/window strategy, dedupe policy, budget and target project objects. | Keyword alert with no search/version provenance. | Living evidence requires knowing exactly what changed relative to a known search state. | [PRISMA-S: An Extension to the PRISMA Statement for Reporting Literature Searches](https://doi.org/10.1186/s13643-020-01542-z) |
| **Membership authority** | New records enter an inbox/candidate state and follow normal reconciliation/screening; monitor never changes corpus membership directly. | Auto-add high-relevance results. | Researchers retain inclusion authority and active-learning errors remain auditable. | [ASReview Documentation](https://asreview.readthedocs.io/en/latest/) |
| **Impact analysis** | Generate evidence-linked “may affect” candidates for claims, comparison sets, syntheses, dossiers and designs; human confirms impact before recalculation/revision. | Automatically rewrite affected outputs. | New evidence relevance and substantive impact are consequential scholarly judgments. | [PROV-O: The PROV Ontology](https://www.w3.org/TR/prov-o/) |
...
|---|---|
| `services/core/research_observatory/modules/monitoring/definitions.py` | Monitor scope, schedule, cursor and targets. |
| `services/workers/monitoring/source_poll.py` | Rate-limited differential connector jobs. |
| `services/core/research_observatory/modules/monitoring/triage.py` | Reconciliation, relevance and screening inbox. |
| `services/core/research_observatory/modules/monitoring/impact.py` | Dependency/semantic/claim impact candidates. |
| `packages/ui/living-monitor` | Monitor health, new evidence, change report and reassessment. |
| `tests/e2e/living-monitor` | Cursor overlap, deletion/correction, outage, triage and impact flows. |
...
|---|---|
| `SavedMonitor` | scope, source/query versions, cadence, budget, cursor strategy and targets |
| `MonitorRun` | window/cursors, source responses, records, dedupe, errors and manifest |
| `NewEvidenceCandidate` | canonical work/version, discovery path, relevance and triage state |
| `ImpactCandidate` | new evidence, affected object, proposed effect, explanation, status and reviewer |
| `LivingChangeReport` | period, additions/corrections/retractions, confirmed impacts, unresolved items and decisions |
...
- `MonitorScheduler.due/enqueue` uses durable local workflow and backoff.
- `SourceMonitor.fetch_delta(definition, cursor/window)` records request/response cursors and overlap.
- `ImpactAnalyzer.propose(new_evidence, dependency_graph, policy)` emits candidates only.
- `ReassessmentCommand.confirm/decline` triggers scoped staleness/recalculation through CAP-03.
...
- Living Monitor separates source health, new records, screening/triage, impact candidates and confirmed change reports.
- Users see which prior query/source/version produced each new record and whether it is correction/retraction/new work.
- Impact cards open the new passage and affected claim/synthesis/dossier side by side.
- Schedules, network/offline state and rate limits are visible and configurable.
...
- Credentials remain in OS secret storage and source rate/licensing rules are enforced.
- Private project targets/queries are not sent beyond configured scholarly source endpoints.
- Notifications redact confidential titles/ideas according to desktop privacy settings.
...
|---|---|
| Cursor lost/source semantics changed | Use overlapping date/window recovery, dedupe and explicit potential-gap warning. |
| Offline/rate limited | Backoff, retain next-run state and allow manual retry; no busy polling. |
| Record corrected/retracted | Create version/status event and high-priority impact candidate. |
| Impact analysis fails | New record remains in triage; no output is marked unaffected automatically. |
...
| Verification profiles | `search`, `service` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S07.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `graph`, `novelty`, `evidence` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S07.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `desktop`, `graph`, `e2e-local` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S07.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
|---|---|
| Scheduler overhead | < 1% idle CPU; no background model execution without due job. |
| Delta retrieval | Source-configured page/rate/budget and cancellation; checkpoint each page. |
| Triage | 1,000 new records prioritized < 5 min after metadata indexes. |
| Impact analysis | Progressive, bounded top affected objects; resumable and cost-visible. |
...
- monitor due/run/success/lag
- source cursor/records/dedup/errors
- new/relevant/included/excluded
- impact proposed/confirmed/declined
- claims/dossiers made stale
- change report completion
...
- Consumes CAP-04 connectors, CAP-06 searches/screening and CAP-03 dependencies.
- Updates CAP-09 syntheses and CAP-10 audits/dossiers only after confirmed impact.
- Supports later manuscript/result monitoring and university scheduled execution without changing domain semantics.
...
| Risk | Mitigation |
|---|---|
| Silent retrieval gap | Overlap windows, source-health diagnostics and bounded completeness claims. |
| Alert fatigue | Triage/ranking, digest cadence and workflow-specific impact filters. |
| Automatic conclusion drift | Candidate-only impact and human reassessment authority. |

## 19. Required ADRs and human decisions

- ADR-CAP10-MONITOR: differential cursor/window, impact candidate and reassessment authority.
...
- **OpenAlex API Documentation** — Scholarly metadata, citations, cursor paging and source monitoring.  
  https://docs.openalex.org/
- **Crossref REST API** — DOI metadata, updates, licenses and cursor-based retrieval.  
  https://www.crossref.org/documentation/retrieve-metadata/rest-api/
- **Semantic Scholar Academic Graph API** — Citation graph, recommendations and paper metadata.  

...
slice_id: CAP-19.S02
title: Extended independent reviewer panel
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-19.S02` — Extended independent reviewer panel |
| Slice outcome | Generated or uploaded drafts receive complementary substantive, methodological, theoretical, critical, and integrity reviews. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-19.S02.T01`, `CAP-19.S02.T02`, `CAP-19.S02.T03` |
| Slice dependencies | `CAP-19.S01.T03` |
...
Generated or uploaded drafts receive complementary substantive, methodological, theoretical, critical, and integrity reviews.
...
**Implementation thesis.** Run a genuinely role-diverse independent reviewer panel whose critiques are evidence-linked, method-aware and reproducible.
...
- Editorial/contribution/scope reviewer.
- Methods/analysis/validity/ethics/reproducibility reviewer.
- Theory/critical/domain/evidence-integrity reviewers.
- Role/model diversity and independent execution.
...
- **Consumes:** `CAP-19.S01 protocol/snapshot and CAP-18/CAP-17 evidence` and formal dependencies `CAP-19.S01.T03`.
- **Produces:** Generated or uploaded drafts receive complementary substantive, methodological, theoretical, critical, and integrity reviews.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-19.S01.T03`.
...
| `CAP-19-D06` | Editorial reviewer | **Include a scope, contribution, positioning, coherence and venue-fit role** | Ask every reviewer to judge every criterion | Role specialization produces more focused and auditable assessments. | None |
| `CAP-19-D07` | Methods reviewer | **Use method-family-aware analysis, validity, ethics and reproducibility criteria** | Use a generic statistical checklist for all papers | Quantitative, qualitative, mixed, computational and design-science manuscripts require different review logic. | None |
| `CAP-19-D08` | Theory and critical reviewer | **Provide separate theory/argument and critical/reflexive reviewer roles** | Score both with an empirical causal-validity rubric | These traditions require different standards of contribution and evidence. | None |
| `CAP-19-D09` | Evidence-integrity reviewer | **Audit citations, source entailment, result records, tables/figures and unsupported claims** | Rely on reviewer impressions of citation quality | Evidence-grounded review is a key differentiator and must be independently checked. | None |
| `CAP-19-D10` | Ethics and reproducibility reviewer | **Inspect disclosure, data/code/protocol availability, rights, human-subject and reproducibility claims without making institutional determinations** | Automatically declare ethical compliance | The system can flag evidence and missingness but cannot replace authorized ethics or editorial judgment. | None |
...
|---|---|
| `workers/reviewer/editorial` | fit/contribution/coherence role |
| `workers/reviewer/methods` | method-pack-aware review |
| `workers/reviewer/theory-critical` | argument/reflexivity review |
| `workers/reviewer/evidence` | citation/result integrity review |
| `services/core-api/reviews/panel` | parallel isolated orchestration |
...
|---|---|
| `ReviewerRun` | Versioned ReviewerRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to extended independent reviewer panel. |
| `ReviewerReportCandidate` | Versioned ReviewerReportCandidate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to extended independent reviewer panel. |
| `ReviewerFinding` | Versioned ReviewerFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to extended independent reviewer panel. |
| `ReviewerEvidenceReference` | Versioned ReviewerEvidenceReference state with stable identity, lifecycle/status, provenance and policy metadata appropriate to extended independent reviewer panel. |
| `ReviewerConfigurationManifest` | Versioned ReviewerConfigurationManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to extended independent reviewer panel. |
...
- `Each report is generated without peer-report context.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Comments identify criterion, location, evidence and confidence.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `No role makes acceptance predictions or institutional ethics determinations.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-19.S01 protocol/snapshot and CAP-18/CAP-17 evidence.
- **Downstream:** CAP-19.S04 reports/meta-review and S05 revision planning.
...
- Stream independent reviewer completion without exposing reports to other agents.
- Allow researchers to inspect role criteria and evidence for every finding.
...
- Enforce allowed-context manifest per reviewer.
- Provider requests and cached output honor confidentiality policy.
...
|---|---|---|
| reviewer times out | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| role produces out-of-scope comments | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| model repeats another reviewer due to shared cache | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| evidence citation unavailable | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | reviewer, ai, novelty |
...
| Review gate | `agent-review` |
| Verification profiles | reviewer, ai, study-design, results |
...
|---|---|---|
| isolation/correlation tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| criterion coverage and out-of-scope rate | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| evidence-link validity | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| method-specific expert benchmark | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| parallelism respects local/provider resource budgets | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| per-role token/cost ceilings | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| partial panel can resume without rerunning complete reports | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-19.S01 protocol/snapshot and CAP-18/CAP-17 evidence. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-19.S04 reports/meta-review and S05 revision planning. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-19/index.html`; this slice page is `planning/review-site/CAP-19/CAP-19.S02.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

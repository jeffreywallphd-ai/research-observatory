...
slice_id: CAP-15.S05
title: Study Design Studio and protocol exports
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-15.S05` — Study Design Studio and protocol exports |
| Slice outcome | Researchers can produce a source-grounded, reviewable empirical protocol and analysis plan. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-15.S05.T01`, `CAP-15.S05.T02`, `CAP-15.S05.T03` |
| Slice dependencies | `CAP-15.S04.T03` |
...
Researchers can produce a source-grounded, reviewable empirical protocol and analysis plan.
...
**Implementation thesis.** Provide one source-first Study Design Studio that supports inspect–contest–adjudicate work and produces reviewable protocol, analysis-plan and preregistration-ready artifacts.
...
- Guided study design studio workspace.
- Docx/markdown/structured protocol exports.
- Completeness, contradiction and integrity audit.
...
- **Consumes:** `complete design/analysis/governance plan and approved UI reference` and formal dependencies `CAP-15.S04.T03`.
- **Produces:** Researchers can produce a source-grounded, reviewable empirical protocol and analysis plan.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-15.S04.T03`.
...
|---|---|---|---|---|
| `CAP-15-D17` | Reporting guidance | **Select method/venue reporting guidance from verified profiles such as JARS/EQUATOR and retain version/source** | Embed one static checklist for all studies | Reporting requirements vary by study design and venue. | None |
...
|---|---|
| `packages/ui/study-design` | adaptive workflow and comparison surfaces |
| `services/exports/study-design` | human and machine artifacts |
| `modules/study_design/integrity` | rule/model assisted audit |
...
|---|---|
| `DesignWorkspaceState` | Versioned DesignWorkspaceState state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study design studio and protocol exports. |
| `ProtocolExport` | Versioned ProtocolExport state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study design studio and protocol exports. |
| `ReportingGuidelineProfile` | Versioned ReportingGuidelineProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study design studio and protocol exports. |
| `IntegrityFinding` | Versioned IntegrityFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study design studio and protocol exports. |
| `CompletenessState` | Versioned CompletenessState state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study design studio and protocol exports. |
...
- `StudyDesignWorkspace.load/save/adjudicate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ProtocolExporter.render(format,profile)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `DesignIntegrityAudit.run` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** complete design/analysis/governance plan and approved UI reference.
- **Downstream:** production acceptance, CAP-16 manuscript blueprint and CAP-17 results.
...
- Follow the empirical-study-design workflow stages and always show next expected output.
- Decision options and recommendation appear near the top with evidence and unresolved status.
- Exports preview source links, inferred content, missing items and disclosure language.
...
- Document generation escapes untrusted content and carries rights/disclosure metadata.
- Venue/registry templates are used only when verified and versioned.
- Audit findings are recommendations; researchers resolve or explicitly accept them.
...
|---|---|---|
| Template/profile missing or outdated | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Export loses evidence link or revision | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Audit flags false contradiction across method traditions | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| User changes design during export | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | study-design, desktop |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | study-design, manuscript |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | study-design, ai, evidence |
...
|---|---|---|
| UI conformance to approved Study Design Studio prototype | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Export schema/format round trips and citation/evidence audit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Completeness/contradiction gold cases | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Accessibility, cancellation, restart and stale-reference behavior | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Interactive editing responsive for representative protocol | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Export deterministic for same revision/profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Audit time and model cost bounded with local fallback | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
complete design/analysis/governance plan and approved UI reference. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

production acceptance, CAP-16 manuscript blueprint and CAP-17 results. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-15/index.html`; this slice page is `planning/review-site/CAP-15/CAP-15.S05.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

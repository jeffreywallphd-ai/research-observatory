...
slice_id: CAP-12.S04
title: Team collaboration and scholarly adjudication
status: proposed
wave: W10
priority: P2
deployment_profiles:
...
| Slice | `CAP-12.S04` — Team collaboration and scholarly adjudication |
| Slice outcome | Research groups can share projects, assign work, compare decisions, discuss disputes, and retain scholarly plurality. |
| Wave / priority | `W10` / `P2` |
| Deployment profiles | `UNI` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-12.S04.T01`, `CAP-12.S04.T02`, `CAP-12.S04.T03` |
| Slice dependencies | `CAP-12.S03.T03`, `CAP-08.S06.T02` |
...
Research groups can share projects, assign work, compare decisions, discuss disputes, and retain scholarly plurality.
...
**Implementation thesis.** Support team research as versioned scholarly collaboration with explicit conflict, disagreement and adjudication rather than shared mutable documents.
...
- Project members and work assignments.
- Optimistic concurrency and collaborative events.
- Review, discussion and adjudication preserving plurality.
...
- **Consumes:** `institutional identity/isolation` and formal dependencies `CAP-12.S03.T03`, `CAP-08.S06.T02`.
- **Produces:** Research groups can share projects, assign work, compare decisions, discuss disputes, and retain scholarly plurality.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-12.S03.T03`, `CAP-08.S06.T02`.
...
|---|---|---|---|---|
| `CAP-12-D08` | Collaboration semantics | **Use immutable revisions, optimistic concurrency and explicit conflict/adjudication objects** | Use last-writer-wins updates | Scholarly disagreement and simultaneous review cannot be safely collapsed. | None |
...
|---|---|
| `modules/collaboration` | membership, assignment and discussion |
| `modules/revisions` | optimistic concurrency and merge proposals |
| `packages/ui/collaboration` | review queues and adjudication surfaces |
...
|---|---|
| `ProjectMembership` | Versioned ProjectMembership state with stable identity, lifecycle/status, provenance and policy metadata appropriate to team collaboration and scholarly adjudication. |
| `Assignment` | Versioned Assignment state with stable identity, lifecycle/status, provenance and policy metadata appropriate to team collaboration and scholarly adjudication. |
| `DiscussionThread` | Versioned DiscussionThread state with stable identity, lifecycle/status, provenance and policy metadata appropriate to team collaboration and scholarly adjudication. |
| `Conflict` | Versioned Conflict state with stable identity, lifecycle/status, provenance and policy metadata appropriate to team collaboration and scholarly adjudication. |
| `Adjudication` | Versioned Adjudication state with stable identity, lifecycle/status, provenance and policy metadata appropriate to team collaboration and scholarly adjudication. |
| `PresenceEvent` | Versioned PresenceEvent state with stable identity, lifecycle/status, provenance and policy metadata appropriate to team collaboration and scholarly adjudication. |
...
- `CollaborationService.assign/comment/review` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `RevisionService.commit(expected_version)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `AdjudicationService.record(decision,rationale,dissent)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** institutional identity/isolation.
- **Downstream:** institutional pilot and future cloud collaboration.
...
- Show who changed what and preserve a direct route to source evidence.
- Conflicts become reviewable objects with alternatives, not silent overwrite errors.
- Unresolved scholarly plurality can remain an accepted state.
...
- Project membership and role checks apply to every collaboration event and attachment.
- Notifications contain minimal sensitive content.
- Audit retains author identity and cannot be rewritten by project admins.
...
|---|---|---|
| Concurrent edits to same evidence/claim | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Member removed while task is assigned | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Notification delivery failure | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Offline cached revision submitted after project changed | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | server, desktop |
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | server, desktop, evidence |
...
| Verification profiles | server, desktop, graph |
...
|---|---|---|
| Concurrency and conflict-resolution tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Role-scoped review/adjudication matrix | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Unresolved dissent preserved in exports and synthesis | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Activity timeline and provenance closure | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Collaborative save conflict returned quickly with actionable diff | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Event fanout bounded and recoverable | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No last-writer-wins loss of scholarly decisions | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
institutional identity/isolation. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

institutional pilot and future cloud collaboration. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-12/index.html`; this slice page is `planning/review-site/CAP-12/CAP-12.S04.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

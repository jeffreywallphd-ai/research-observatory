...
slice_id: CAP-12.S05
title: Licensed sources, institutional rights, retention, and compute policy
status: proposed
wave: W10
priority: P2
deployment_profiles:
...
| Slice | `CAP-12.S05` — Licensed sources, institutional rights, retention, and compute policy |
| Slice outcome | Institutional entitlements and unpublished materials are governed consistently across search, storage, model egress, collaboration, and export. |
| Wave / priority | `W10` / `P2` |
| Deployment profiles | `UNI` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-12.S05.T01`, `CAP-12.S05.T02`, `CAP-12.S05.T03` |
| Slice dependencies | `CAP-12.S03.T02`, `CAP-04.S05.T03`, `CAP-07.S01.T03` |
...
Institutional entitlements and unpublished materials are governed consistently across search, storage, model egress, collaboration, and export.
...
**Implementation thesis.** Enforce institutional source, retention and compute policy at every use boundary while keeping policy decisions transparent to researchers and librarians.
...
- Licensed-source entitlement and downstream rights.
- Retention, legal hold and deletion.
- Institutional model/compute and egress policy.
...
- **Consumes:** `institutional identity and hosted services` and formal dependencies `CAP-12.S03.T02`, `CAP-04.S05.T03`, `CAP-07.S01.T03`.
- **Produces:** Institutional entitlements and unpublished materials are governed consistently across search, storage, model egress, collaboration, and export.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-12.S03.T02`, `CAP-04.S05.T03`, `CAP-07.S01.T03`.
...
|---|---|---|---|---|
| `CAP-12-D10` | Rights enforcement | **Evaluate entitlements at acquisition, view, derivation, export and cache boundaries** | Check license only when a document is downloaded | Institutional source rights continue to govern downstream use and redistribution. | None |
| `CAP-12-D11` | Model egress | **Use institution policy to route local, private hosted or approved external models with fail-closed payload checks** | Let each user send any project content to any provider | Private manuscripts and licensed text require enforceable egress governance. | None |
| `CAP-12-D12` | Retention and legal hold | **Use versioned policy, deletion workflow, legal-hold override and auditable exceptions** | Use one hard-coded deletion interval | Institutional records require explainable retention and exception handling. | None |
| `CAP-12-D13` | Observability | **Use OpenTelemetry with project/tenant context and strict content redaction** | Centralize raw prompts, source passages and reports in logs | Operations require traceability without duplicating sensitive research content. | None |
...
|---|---|
| `modules/rights/institutional` | entitlement and action policy |
| `modules/retention` | lifecycle, hold and deletion jobs |
| `modules/model_policy` | provider, data-class and residency routing |
...
|---|---|
| `Entitlement` | Versioned Entitlement state with stable identity, lifecycle/status, provenance and policy metadata appropriate to licensed sources, institutional rights, retention, and compute policy. |
| `RightsDecision` | Versioned RightsDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to licensed sources, institutional rights, retention, and compute policy. |
| `RetentionPolicy` | Versioned RetentionPolicy state with stable identity, lifecycle/status, provenance and policy metadata appropriate to licensed sources, institutional rights, retention, and compute policy. |
| `LegalHold` | Versioned LegalHold state with stable identity, lifecycle/status, provenance and policy metadata appropriate to licensed sources, institutional rights, retention, and compute policy. |
| `ComputePolicy` | Versioned ComputePolicy state with stable identity, lifecycle/status, provenance and policy metadata appropriate to licensed sources, institutional rights, retention, and compute policy. |
| `EgressApproval` | Versioned EgressApproval state with stable identity, lifecycle/status, provenance and policy metadata appropriate to licensed sources, institutional rights, retention, and compute policy. |
...
- `RightsService.decide(action,source,user,project)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `RetentionService.schedule/hold/delete/verify` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ModelPolicyRouter.authorize(request)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** institutional identity and hosted services.
- **Downstream:** operations, DR and pilot acceptance.
...
- Rights denials explain the governing source/policy and available alternatives.
- Users can distinguish institution-enforced retention/compute rules from personal preferences.
- Egress preview identifies provider, region and exact data classes, not just a generic consent checkbox.
...
- Fail closed when entitlement or policy service is unavailable.
- Deletion cannot override legal hold; hold access is narrowly authorized.
- External providers receive only approved payloads under institutional routing.
...
|---|---|---|
| Entitlement revoked after caching | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Conflicting retention and project deletion request | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| External model policy changes mid-workflow | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Licensed document included in redistributable export | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Estimate / risk | `L` / `high` |
| Review gate | `privacy-rights-review` |
| Verification profiles | server |
...
| Verification profiles | server, ai |
...
|---|---|---|
| Rights checks across acquire/view/derive/search/export/cache | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Legal hold and verified deletion tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Model route/deny/fallback matrix | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Redistribution and disclosure manifest audit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Policy decision and audit overhead bounded | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Deletion jobs resumable across all stores | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No unauthorized content appears in logs or model payloads | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
institutional identity and hosted services. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

operations, DR and pilot acceptance. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-12/index.html`; this slice page is `planning/review-site/CAP-12/CAP-12.S05.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

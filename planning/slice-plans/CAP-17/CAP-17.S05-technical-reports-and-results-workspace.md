...
slice_id: CAP-17.S05
title: Technical Reports & Results workspace
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-17.S05` — Technical Reports & Results workspace |
| Slice outcome | Researchers can inspect, verify, and approve result evidence for downstream manuscripts. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-17.S05.T01`, `CAP-17.S05.T02`, `CAP-17.S05.T03` |
| Slice dependencies | `CAP-17.S04.T03` |
...
Researchers can inspect, verify, and approve result evidence for downstream manuscripts.
...
**Implementation thesis.** Give researchers a source-first results workspace and export a complete verified evidence package rather than a prose summary.
...
- Technical reports & results workspace.
- Result evidence matrix and manuscript claim mapping.
- Verification/deviation queues and source inspection.
- Ro-crate, structured result and table exports.
...
- **Consumes:** `CAP-17.S01-S04 artifact, result, verification and graph state` and formal dependencies `CAP-17.S04.T03`.
- **Produces:** Researchers can inspect, verify, and approve result evidence for downstream manuscripts.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-17.S04.T03`.
...
| `CAP-17-D17` | Evidence-package export | **Export RO-Crate metadata, JSON-LD/JSON records, Frictionless tables, source manifests and checksums** | Export only a PDF summary | A machine- and human-readable package supports audit, reuse and manuscript transfer. | ADR-RESULT-EVIDENCE-PACKAGE |
...
|---|---|
| `apps/desktop/technical-reports` | approved workspace |
| `packages/ui/results` | result matrix, source viewer and mapping |
| `workers/result-export` | RO-Crate/Frictionless package builder |
...
|---|---|
| `ResultWorkspaceView` | Versioned ResultWorkspaceView state with stable identity, lifecycle/status, provenance and policy metadata appropriate to technical reports & results workspace. |
| `ManuscriptResultMapping` | Versioned ManuscriptResultMapping state with stable identity, lifecycle/status, provenance and policy metadata appropriate to technical reports & results workspace. |
| `VerifiedResultPackage` | Versioned VerifiedResultPackage state with stable identity, lifecycle/status, provenance and policy metadata appropriate to technical reports & results workspace. |
| `ResultExportManifest` | Versioned ResultExportManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to technical reports & results workspace. |
...
- `Workspace status derives from canonical result/verification state.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Export contains only authorized evidence and explicit omissions.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Manuscript mappings reference stable result IDs, never copied numbers alone.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-17.S01-S04 artifact, result, verification and graph state.
- **Downstream:** CAP-18 empirical drafting and external research archive/review.
...
- Conform to `technical-reports.html` in light/dark mode.
- Make unverified/missing/conflicting states visually unambiguous.
- Provide direct handoff to Manuscript Studio when section readiness is satisfied.
...
- Preview egress and restrict confidential artifacts from export.
- Redact support logs and temporary files.
...
|---|---|---|
| package build interrupted | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| restricted source included by dependency | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| result mapping references superseded record | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| export schema validation fails | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | results, desktop |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | results, evidence, manuscript |
...
| RO-Crate and Frictionless validation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| authorized-omission and denial tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| manuscript mapping round-trip | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| result matrix virtualizes large tables | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| package build is streaming/resumable | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| export validation completes within approved release budget | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-17.S01-S04 artifact, result, verification and graph state. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-18 empirical drafting and external research archive/review. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-17/index.html`; this slice page is `planning/review-site/CAP-17/CAP-17.S05.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

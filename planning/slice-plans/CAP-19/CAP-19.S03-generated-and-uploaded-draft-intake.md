...
slice_id: CAP-19.S03
title: Generated and uploaded draft intake
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-19.S03` — Generated and uploaded draft intake |
| Slice outcome | Any draft can enter a reproducible, evidence-aware reviewer simulation without losing its original state. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-19.S03.T01`, `CAP-19.S03.T02`, `CAP-19.S03.T03` |
...
Any draft can enter a reproducible, evidence-aware reviewer simulation without losing its original state.
...
**Implementation thesis.** Review generated and externally uploaded manuscripts against the same frozen blueprint, venue, evidence and result contracts.
...
- Generated draft selection and external manuscript intake.
- Blueprint/venue/evidence/report audit.
- Format parsing and stable review-location mapping.
- Immutable review snapshot and allowed context.
...
- **Consumes:** `CAP-18 generated manuscript or user-uploaded article plus CAP-16/17 evidence contracts` and formal dependencies `CAP-19.S01.T03`.
- **Produces:** Any draft can enter a reproducible, evidence-aware reviewer simulation without losing its original state.
...
|---|---|
| `services/core-api/reviews/intake` | manuscript selection/upload |
| `workers/review-manuscript-parser` | DOCX/PDF/LaTeX/Markdown parsing |
| `packages/core/review/audit` | blueprint/evidence compliance |
| `packages/core/review/locations` | block/annotation mapping |
...
|---|---|
| `ReviewSubmission` | Versioned ReviewSubmission state with stable identity, lifecycle/status, provenance and policy metadata appropriate to generated and uploaded draft intake. |
| `ExternalDraft` | Versioned ExternalDraft state with stable identity, lifecycle/status, provenance and policy metadata appropriate to generated and uploaded draft intake. |
| `ReviewLocation` | Versioned ReviewLocation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to generated and uploaded draft intake. |
| `PreReviewAudit` | Versioned PreReviewAudit state with stable identity, lifecycle/status, provenance and policy metadata appropriate to generated and uploaded draft intake. |
| `ReviewSnapshotBundle` | Versioned ReviewSnapshotBundle state with stable identity, lifecycle/status, provenance and policy metadata appropriate to generated and uploaded draft intake. |
...
- `External drafts are candidate project artifacts and never overwrite canonical manuscripts.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Review locations use stable block IDs or revision-aware selectors.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Audit missingness is reported before reviewer run.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-18 generated manuscript or user-uploaded article plus CAP-16/17 evidence contracts.
- **Downstream:** CAP-19.S02 reviewer runs and S04 reports.
...
- Show pre-review readiness, unsupported claims and missing evidence before launch.
- Permit review with warnings only through explicit human decision.
...
- Uploaded drafts are untrusted and confidential.
- External model egress requires explicit permission and disclosure.
...
|---|---|---|
| DOCX/PDF structure cannot map reliably | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| uploaded draft has no blueprint | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| source links unavailable | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| snapshot build interrupted | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | reviewer, documents, security-local |
...
| Review gate | `agent-review` |
| Verification profiles | reviewer, manuscript, evidence |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | reviewer, data, security-local |
...
|---|---|---|
| multi-format parsing/location fixtures | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| pre-review audit precision | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| snapshot hash/reproducibility | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| untrusted content and denial tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| large draft intake streams | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| snapshot package bounded/deduplicated | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| location mapping uncertainty reported rather than hidden | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-18 generated manuscript or user-uploaded article plus CAP-16/17 evidence contracts. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-19.S02 reviewer runs and S04 reports. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-19/index.html`; this slice page is `planning/review-site/CAP-19/CAP-19.S03.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

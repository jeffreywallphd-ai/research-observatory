...
slice_id: CAP-11.S06
title: Local release candidate and acceptance gate
status: proposed
wave: W5
priority: P0
deployment_profiles:
...
| Slice | `CAP-11.S06` — Local release candidate and acceptance gate |
| Slice outcome | The PC/lab edition is released only after technical, scholarly, and operational acceptance on representative workflows. |
| Wave / priority | `W5` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-11.S06.T01`, `CAP-11.S06.T02`, `CAP-11.S06.T03` |
| Slice dependencies | `CAP-11.S03.T03`, `CAP-11.S04.T03`, `CAP-11.S05.T03`, `CAP-10.S03.T03` |
...
The PC/lab edition is released only after technical, scholarly, and operational acceptance on representative workflows.
...
**Implementation thesis.** Release the Windows PC/lab edition only from immutable, independently reviewed evidence that demonstrates complete scholarly workflows and operational recovery on declared configurations.
...
- Signed release candidate and manifests.
- Complete end-to-end regression/acceptance.
- G5 approval, manuals and support disclosure.
...
- **Consumes:** `all CAP-11 slices and CAP-10 end-to-end opportunity dossier` and formal dependencies `CAP-11.S03.T03`, `CAP-11.S04.T03`, `CAP-11.S05.T03`, `CAP-10.S03.T03`.
- **Produces:** The PC/lab edition is released only after technical, scholarly, and operational acceptance on representative workflows.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-11.S03.T03`, `CAP-11.S04.T03`, `CAP-11.S05.T03`, `CAP-10.S03.T03`.
...
|---|---|---|---|---|
| `CAP-11-D13` | Release evidence | **Require signed artifacts, SBOM, SLSA-style provenance, checksums and criterion-linked gate evidence** | Publish an installer after tests pass on the developer machine | A production release must be independently verifiable and reproducible. | None |
| `CAP-11-D14` | Support lifecycle | **Publish supported OS/runtime/model matrix, update rings, known limitations and deprecation policy** | Support all observed combinations indefinitely | Explicit support boundaries enable reliable maintenance and honest user expectations. | None |
| `CAP-11-D15` | Release gate authority | **Human approves G5 from immutable evidence; automation assembles but cannot self-certify** | Allow the release automation to approve itself when checks are green | Technical evidence does not replace product, scholarly and operational accountability. | None |
| `CAP-11-D17` | Benchmark disclosure | **Publish limits and methodology with versioned hardware/software manifests** | Publish only headline throughput numbers | Users and reviewers must understand the scope of performance claims. | None |
...
|---|---|
| `release/g5` | artifact assembly and evidence map |
| `tests/release/windows` | automated/manual acceptance |
| `docs/release` | user, admin, limitations and AI disclosure |
...
|---|---|
| `ReleaseCandidate` | Versioned ReleaseCandidate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to local release candidate and acceptance gate. |
| `ArtifactManifest` | Versioned ArtifactManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to local release candidate and acceptance gate. |
| `GateEvidence` | Versioned GateEvidence state with stable identity, lifecycle/status, provenance and policy metadata appropriate to local release candidate and acceptance gate. |
| `KnownLimitation` | Versioned KnownLimitation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to local release candidate and acceptance gate. |
| `SupportPolicy` | Versioned SupportPolicy state with stable identity, lifecycle/status, provenance and policy metadata appropriate to local release candidate and acceptance gate. |
...
- `ReleaseAssembler.build(revision, profile)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `GateEvaluator.map(criteria,evidence)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ReleasePublisher.publish(after_human_approval)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** all CAP-11 slices and CAP-10 end-to-end opportunity dossier.
- **Downstream:** CAP-12 university unlock and CAP-14 cross-platform qualification.
...
- Release notes describe user-visible changes, migrations and limitations in plain language.
- Installation and first-run paths match the approved Academic Minimal references.
- Known unsupported workloads are surfaced before users invest time.
...
- All binaries/installers/manifests are signed and checksum verified.
- SBOM and provenance identify source, builder and dependencies.
- Publishing credentials are unavailable to implementation agents without explicit authorization.
...
|---|---|---|
| Artifact hash mismatch | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Acceptance test uses wrong build or fixture version | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Manual evidence not linked to immutable revision | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Gate approved with unresolved high-severity finding | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | foundation, desktop, security-local |
...
| Verification profiles | e2e-local, security-local |
...
| Verification profiles | foundation |
...
|---|---|---|
| Fresh install, upgrade, rollback, backup/restore and offline workflow | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Criterion-to-evidence closure across technical, scholarly and operational exits | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Independent review of release artifacts and support claims | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Reproducible clean-build comparison | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Release artifact sizes and install time remain within published limits | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No critical/high unresolved defects; lower-severity exceptions are explicit | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Evidence bundle can be independently revalidated | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
all CAP-11 slices and CAP-10 end-to-end opportunity dossier. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-12 university unlock and CAP-14 cross-platform qualification. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-11/index.html`; this slice page is `planning/review-site/CAP-11/CAP-11.S06.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

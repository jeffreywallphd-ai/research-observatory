...
REQUIRED_HEADINGS = [
    '## 0. Plan control', '## 1. Purpose and contribution to the larger vision',
    '## 2. Scope', '## 3. Authority, dependencies, and campaign stop conditions',
    '## 4. Selected implementation decisions', '## 5. Architecture and implementation design',
    ...
    '## 9. Task-by-task implementation plan', '## 10. Slice-wide verification matrix',
    '## 11. Performance and resource budgets', '## 12. Observability and provenance',
    '## 13. Adjacent-slice handoffs', '## 14. Migration and backward compatibility',
    '## 15. Required slice evidence bundle', '## 16. Definition of Ready',
    '## 17. Definition of Done', '## 18. Risks and mitigations',
    '## 19. Required ADRs and human decisions', '## 20. Research and standards basis',
...
def main() -> int:
    ...
    for path in paths:
        ...
        sid, cid = meta.get('slice_id'), meta.get('capability_id')
        if sid in seen:
        ...
        if meta.get('capability_plan') != f'planning/capability-plans/{cid}.md':
            errors.append(f'{sid}: capability_plan link is incorrect')
        if not (root/meta.get('capability_plan','')).exists():

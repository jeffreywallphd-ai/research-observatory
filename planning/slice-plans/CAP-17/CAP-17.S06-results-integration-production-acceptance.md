...
slice_id: CAP-17.S06
title: Results integration production acceptance
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-17.S06` — Results integration production acceptance |
| Slice outcome | Technical-report and result evidence is trustworthy enough for controlled manuscript use. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-17.S06.T01`, `CAP-17.S06.T02`, `CAP-17.S06.T03` |
| Slice dependencies | `CAP-17.S05.T03` |
...
Technical-report and result evidence is trustworthy enough for controlled manuscript use.
...
**Implementation thesis.** Accept result integration only when multiple method families, correction propagation and no-invention/privacy controls hold end to end.
...
- Quantitative, qualitative, mixed and technical fixture suite.
- No-result-invention and private-egress acceptance.
- Correction/staleness/manuscript handoff tests.
- G8 result-integration gate evidence.
...
- **Consumes:** `all CAP-17 slices and CAP-15/CAP-16 handoffs` and formal dependencies `CAP-17.S05.T03`.
- **Produces:** Technical-report and result evidence is trustworthy enough for controlled manuscript use.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-17.S05.T03`.
...
|---|---|
| `tests/acceptance/results` | multi-method end-to-end suite |
| `benchmarks/results` | extraction/verification corpus |
| `artifacts/gates/G8-results` | acceptance bundle |
...
|---|---|
| `ResultAcceptanceRun` | Versioned ResultAcceptanceRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to results integration production acceptance. |
| `NoInventionAudit` | Versioned NoInventionAudit state with stable identity, lifecycle/status, provenance and policy metadata appropriate to results integration production acceptance. |
| `ResultGateDecision` | Versioned ResultGateDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to results integration production acceptance. |
...
- `Every generated empirical result sentence in acceptance traces to a verified result.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Corrections make dependent drafts/reviews stale before reuse.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** all CAP-17 slices and CAP-15/CAP-16 handoffs.
- **Downstream:** G8 result readiness and CAP-18/19 production workflows.
...
- Test report intake through verified package and manuscript readiness.
- Include denial, ambiguity and correction recovery.
...
- Use synthetic/permissive data for automated fixtures.
- Pen-test egress controls for unpublished artifacts.
...
|---|---|---|
| model invents absent result | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| private report leaks to remote provider | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| correction fails to stale downstream claim | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| one method family lacks verification | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | results, e2e-local |
...
| Verification profiles | results, security-local, ai |
...
|---|---|---|
| field-level extraction/verification metrics | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| no-invention red-team suite | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| rights/egress audit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| independent methods and security review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| acceptance corpus runs within release window | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| no critical/high result-integrity issue | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| resource reports for minimum/recommended desktop profiles | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
all CAP-17 slices and CAP-15/CAP-16 handoffs. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

G8 result readiness and CAP-18/19 production workflows. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-17/index.html`; this slice page is `planning/review-site/CAP-17/CAP-17.S06.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

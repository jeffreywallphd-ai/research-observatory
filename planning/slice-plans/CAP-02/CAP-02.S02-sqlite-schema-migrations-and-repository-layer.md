...
| Slice | `CAP-02.S02` - SQLite schema, migrations, and repository layer |
| Slice outcome | Canonical local state is transactional, versioned, testable, and insulated from UI or model code. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-02.S02.T01`, `CAP-02.S02.T02`, `CAP-02.S02.T03` |
| Slice dependencies | `CAP-02.S01.T01`, `CAP-03.S01.T01` |
...
Provide a strongly governed local canonical store that supports durable scholarly state, deterministic migrations, and transaction boundaries shared by later desktop and hosted implementations.
...
**Implementation thesis.** Use SQLAlchemy 2 repositories over SQLite WAL with STRICT tables where feasible, foreign keys enabled, explicit units of work, and forward-only Alembic migrations protected by verified backup and compatibility checks.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-02.S01.T01`, `CAP-03.S01.T01`.
- **Produces:** Canonical local state is transactional, versioned, testable, and insulated from UI or model code.
...
- `planning/backlog.yaml` for `CAP-02.S02` and its task state/dependencies.
...
- `CAP-02.S01.T01` is complete or explicitly gated.
- `CAP-03.S01.T01` is complete or explicitly gated.
...
1. **Enable foreign keys, WAL, busy timeout, defensive pragmas, and explicit checkpoint policy on every canonical connection.**
2. **Use STRICT tables for canonical scalar records where supported; isolate FTS5 and virtual/index tables behind repository ports.**
3. **Use UUIDv7 stored in a single canonical representation and UTC timestamps with explicit precision.**
4. **Use SQLAlchemy 2 typed models/repositories and explicit unit-of-work boundaries; prohibit desktop renderer and feature code from issuing ad hoc SQL.**
5. **Use Alembic forward migrations, backup-before-migrate, migration journal, post-migration validation, and fail-closed rollback to the untouched backup.**
6. **Do not use ORM cascades as the sole expression of domain deletion or retention policy.**
...
- Database connection/profile factory and pragma verification.
- Initial normalized schema spanning project, domain identities, workflow/provenance skeleton, object metadata, and outbox.
- Alembic migration environment plus migration/recovery runner.
- Repository interfaces, SQLAlchemy adapters, unit-of-work, and transaction-outbox helper.
...
- A transaction commits canonical state, provenance/outbox facts, and dependency mutations atomically.
- Repositories return domain types, not ORM entities, across module boundaries.
- Schema compatibility is checked before write access and exposed in the Core handshake.
...
- Migration progress and failures are presented through project-open recovery, not raw SQL errors.
- Users can inspect backup location, migration version, and safe recovery actions.
...
- Database connections are created only after protection profile/key acquisition.
- Use parameterized queries, least-privilege file permissions, and no SQL debug logging containing values in production.
- Run `quick_check` routinely and SQLCipher integrity checks when the protected profile is active.
...
- Test power loss at migration checkpoints, insufficient disk, corrupted backup, wrong key, lock contention, and application downgrade.
- A failed migration preserves the original database and records a recovery manifest outside the database being migrated.
- Checkpoint WAL before relocatable snapshots; never copy only the main database file while WAL state is active.
...
### 9.1 `CAP-02.S02.T01` - Create the initial normalized SQLite schema in WAL mode
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S02.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-02.S02.T02` - Implement forward migrations and backup-before-migrate policy
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S02.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-02.S02.T03` - Build typed repositories and transaction boundaries
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S02.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
|---|---|
| Backlog profiles | `data`, `e2e-local` |
...
- Establish query budgets and indexes from representative fixtures; require `EXPLAIN QUERY PLAN` evidence for critical list/search paths.
- Bound write transactions and avoid long-running parser/model work inside database transactions.
- Monitor WAL size and checkpoint latency.
...
- Record connection profile, schema version, migration timings, checkpoint results, lock wait, slow query fingerprints, and integrity outcomes.
- Never log SQL parameter values by default.
...
- CAP-03 aggregates and provenance use repository/unit-of-work contracts.
- CAP-04/05 add domain migrations through module-owned migration fragments reviewed against the canonical schema.
- Hosted PostgreSQL adapters later implement the same repository ports, not SQLite-specific behavior in domain services.
...
- Each migration is forward-only, idempotence-tested where applicable, and accompanied by old/new fixture tests.
- Breaking data-model changes require export compatibility and documented read support for the prior production format.
...
- The promised outcome is demonstrable end to end: Canonical local state is transactional, versioned, testable, and insulated from UI or model code.
...
- ADR: UUID storage representation and timestamp precision.
- ADR: SQLAlchemy repository/unit-of-work conventions.
- ADR: WAL checkpoint and database integrity policy.
...
|---|---|---|
| `SQLITE_WAL` | [SQLite Write-Ahead Logging](https://www.sqlite.org/wal.html) - SQLite | Local concurrency and checkpoint/recovery behavior. |
| `SQLITE_STRICT` | [SQLite STRICT Tables](https://www.sqlite.org/stricttables.html) - SQLite | Type discipline in the local store. |
| `SQLITE_BACKUP` | [SQLite Online Backup API](https://www.sqlite.org/backup.html) - SQLite | Consistent online database snapshots. |
| `SQLALCHEMY` | [SQLAlchemy 2.0 Documentation](https://docs.sqlalchemy.org/en/20/) - SQLAlchemy | Typed repositories and transaction units. |
| `ALEMBIC` | [Alembic Documentation](https://alembic.sqlalchemy.org/en/latest/) - SQLAlchemy | Versioned forward migrations. |
| `UUIDV7` | [RFC 9562 - UUIDs](https://www.rfc-editor.org/rfc/rfc9562.html) - IETF | Sortable globally unique UUIDv7 identifiers. |
...
2. Confirm `CAP-02` is the active capability campaign and `CAP-02.S02` is the next eligible slice.
3. Claim the first READY task in order: `CAP-02.S02.T01`. Do not globally select work outside the capability.

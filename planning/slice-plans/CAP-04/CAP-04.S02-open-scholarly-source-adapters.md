...
2. **Normalize query intent into connector-specific requests while retaining the exact request URL/parameters, source version, retrieval time, and cursor.**
3. **Use source-specific rate-limit controllers, exponential backoff with jitter, Retry-After handling, and circuit breakers.**
4. **Identify the application/research contact where provider guidance recommends it; credentials remain optional/scoped.**
...
- ADR: Shared connector HTTP/cache/rate-control implementation.
- ADR: Raw response retention and provider-terms policy.
- ADR: Live qualification cadence and fixture recording.
...
2. Confirm `CAP-04` is the active capability campaign and `CAP-04.S02` is the next eligible slice.
3. Claim the first READY task in order: `CAP-04.S02.T01`. Do not globally select work outside the capability.

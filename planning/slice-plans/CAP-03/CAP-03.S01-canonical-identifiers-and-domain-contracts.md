...
slice_id: CAP-03.S01
title: Canonical identifiers and domain contracts
status: approved
wave: W1
priority: P0
deployment_profiles:
...
| Slice | `CAP-03.S01` - Canonical identifiers and domain contracts |
| Slice outcome | A small stable core model defines records, documents, evidence, decisions, workflows, ontologies, graphs, opportunities, and monitoring events. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-03.S01.T01`, `CAP-03.S01.T02`, `CAP-03.S01.T03` |
| Slice dependencies | `CAP-00.S02.T01`, `CAP-00.S05.T03` |
...
Create durable scholarly identities and domain boundaries that remain stable across local, institutional, and cloud deployments and across model, parser, and storage changes.
...
**Implementation thesis.** Define framework-independent aggregates and value objects with UUIDv7 identities, explicit revisions and states, JSON Schema/OpenAPI interchange contracts, and invariants enforced in domain services rather than UI or persistence.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-00.S02.T01`, `CAP-00.S05.T03`.
- **Produces:** A small stable core model defines records, documents, evidence, decisions, workflows, ontologies, graphs, opportunities, and monitoring events.
...
- `planning/backlog.yaml` for `CAP-03.S01` and its task state/dependencies.
...
- `CAP-00.S02.T01` is complete or explicitly gated.
...
1. **Use UUIDv7 for internal aggregate/entity identifiers and preserve external identifiers as typed normalized value objects.**
2. **Separate stable aggregate identity from immutable revision identity; mutable scholarly meaning advances by explicit revision.**
3. **Define aggregates by transactional invariant, not by screen or database table.**
4. **Use UTC instants for system time and separate scholarly dates/partial dates as domain values.**
5. **Publish JSON Schema 2020-12 and generated language types for portable contracts; persistence models remain adapters.**
6. **Adopt additive compatibility by default, explicit deprecation, and no semantic repurposing of existing fields/enums.**
...
- `packages/contracts` schemas for project, work/version, document/revision, intent, provenance, workflow, corpus, evidence, and policy skeletons.
- `services/core-api/domain` aggregates, value objects, state machines, and invariant tests.
- Contract generation and compatibility-diff tooling.
- Fixture library with valid, invalid, prior-version, and unknown-field cases.
...
- Each aggregate declares commands, state transitions, emitted facts, invariants, revision/optimistic concurrency behavior, and retention implications.
- External identifier values preserve source, normalization, verification, and ambiguity state.
- Serialization is deterministic where hashing/signing is required; canonicalization version is explicit.
...
- Human-readable identifiers and source labels are shown alongside opaque internal IDs.
- Disputed or unresolved identity is represented explicitly; the UI never forces false canonical certainty.
...
- Identifiers must not encode user identity, machine paths, or sensitive project metadata.
- Validation is repeated at trusted service boundaries even when the generated client validated input.
- Unknown fields are preserved or rejected according to contract policy, never silently interpreted.
...
- Invalid transitions fail without partial mutation and return stable problem codes.
- Concurrent revision conflicts return current state and reconciliation options.
- Prior-version fixture replay demonstrates compatibility or explicit migration refusal.
...
### 9.1 `CAP-03.S01.T01` - Define core aggregate and value-object contracts
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S01.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-03.S01.T02` - Define aggregate state machines and invariants
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S01.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Author the versioned schema/interface/state-machine definitions first, including unknown/not-applicable states, compatibility metadata, validation rules, and negative fixtures. Keep framework and persistence types outside the portable contract.
3. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
4. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
### 9.3 `CAP-03.S01.T03` - Publish versioning and compatibility policy for domain APIs
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S01.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Author the versioned schema/interface/state-machine definitions first, including unknown/not-applicable states, compatibility metadata, validation rules, and negative fixtures. Keep framework and persistence types outside the portable contract.
3. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
4. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
|---|---|
| Backlog profiles | `foundation`, `service` |
...
- UUID and value-object operations must be negligible relative to persistence; avoid reflection-heavy validation in hot query paths.
- Schema validation is required at boundaries, not repeated indiscriminately in trusted internal loops.
...
- Every command carries actor, project, trace, causation, and correlation identifiers.
- Contract/version mismatch metrics identify producer and consumer versions.
...
- CAP-02 repositories persist these contracts.
- CAP-03.S02-S06 specialize intent, provenance, workflow, dependency, and profile aggregates.
- CAP-04/05 extend scholarly entities without redefining identity/version semantics.
- Hosted adapters reuse the same contracts.
...
- Contract evolution tests include prior serialized fixtures and generated client/server compatibility.
- Enum additions are tolerated only where consumers implement unknown-state behavior.
...
- The promised outcome is demonstrable end to end: A small stable core model defines records, documents, evidence, decisions, workflows, ontologies, graphs, opportunities, and monitoring events.
...
- ADR: UUIDv7 representation and generator.
- ADR: Aggregate/revision identity conventions.
- ADR: Contract code-generation and compatibility tooling.
...
2. Confirm `CAP-03` is the active capability campaign and `CAP-03.S01` is the next eligible slice.
3. Claim the first READY task in order: `CAP-03.S01.T01`. Do not globally select work outside the capability.

...
slice_id: CAP-14.S04
title: Cross-platform scientific and AI runtime
status: proposed
wave: W6
priority: P0
deployment_profiles:
...
| Slice | `CAP-14.S04` — Cross-platform scientific and AI runtime |
| Slice outcome | Hardware acceleration is optional, governed, observable, and portable across supported desktop platforms. |
| Wave / priority | `W6` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64`, `macos-arm64`, `linux-x64`, `linux-arm64` |
| Backlog tasks | `CAP-14.S04.T01`, `CAP-14.S04.T02`, `CAP-14.S04.T03` |
| Slice dependencies | `CAP-14.S01.T03`, `CAP-07.S02.T03` |
...
Hardware acceleration is optional, governed, observable, and portable across supported desktop platforms.
...
**Implementation thesis.** Keep scientific and AI results semantically portable by defining CPU behavior as the baseline and treating CUDA, Metal and other accelerators as policy-governed, observable optimizations.
...
- Hardware-aware acceleration ports.
- Linux cuda/dgx spark arm64 qualification.
- Parser, vector, embedding, reranking and model fallback parity.
...
- **Consumes:** `CAP-07 model gateway and platform target builds` and formal dependencies `CAP-14.S01.T03`, `CAP-07.S02.T03`.
- **Produces:** Hardware acceleration is optional, governed, observable, and portable across supported desktop platforms.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-14.S01.T03`, `CAP-07.S02.T03`.
...
|---|---|---|---|---|
| `CAP-14-D09` | AI portability | **Use CPU as the semantic baseline; expose ONNX Runtime/llama.cpp backends and optional acceleration ports** | Implement platform-specific model behavior with no common fallback | Acceleration must improve performance without changing accepted outputs or project semantics. | None |
| `CAP-14-D10` | DGX Spark support | **Treat DGX Spark as Linux ARM64 with native builds, CUDA qualification, unified-memory limits and CPU fallback** | Create a separate DGX Spark product fork or use x86 emulation | NVIDIA documents the device as an ARM64 Ubuntu-based Grace Blackwell system. | ADR-DGX-SPARK-RUNTIME |
| `CAP-14-D11` | Parser/vector portability | **Pin target-specific artifacts, qualify equivalent behavior and declare a supported fallback** | Silently omit features when a native dependency is unavailable | Users must know when performance changes without losing functional semantics. | None |
...
|---|---|
| `services/accelerators` | discovery and policy |
| `services/inference/backends` | CPU/CUDA/Metal execution |
| `tests/runtime/cross-platform` | semantic and recovery benchmarks |
...
|---|---|
| `AcceleratorCapability` | Versioned AcceleratorCapability state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform scientific and ai runtime. |
| `BackendSelection` | Versioned BackendSelection state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform scientific and ai runtime. |
| `RuntimeManifest` | Versioned RuntimeManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform scientific and ai runtime. |
| `FallbackEvent` | Versioned FallbackEvent state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform scientific and ai runtime. |
| `SemanticComparison` | Versioned SemanticComparison state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cross-platform scientific and ai runtime. |
...
- `AcceleratorRouter.select(task,policy,resources)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `RuntimeBackend.execute/cancel` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `SemanticParityEvaluator.compare` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-07 model gateway and platform target builds.
- **Downstream:** project portability and G6 release.
...
- Show selected backend and fallback only where it affects time, privacy, cost or known output variance.
- Allow users/admins to cap GPU memory and prefer CPU.
- DGX Spark appears as Linux ARM64 with NVIDIA acceleration, not a separate project format.
...
- Model/native artifacts are pinned and verified per architecture.
- GPU worker access follows the same project/rights/model policy.
- Fallback never sends local data to a remote provider automatically.
...
|---|---|---|
| CUDA/Metal initialization fails | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Unified memory pressure on DGX Spark | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Backend produces incompatible dimension/schema | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Cancellation leaves accelerator memory allocated | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | desktop-cross-platform, ai |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | desktop-cross-platform, ai, e2e-local |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | desktop-cross-platform, documents, search, ai |
...
|---|---|---|
| CPU baseline on every platform | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Backend functional and semantic-tolerance benchmarks | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| DGX Spark native ARM64/CUDA tests where hardware available | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Resource limits, cancellation and deterministic fallback | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Per-backend memory/latency/throughput profiles published | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Fallback completes within supported workload envelope | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Semantic differences above threshold block qualification | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-07 model gateway and platform target builds. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

project portability and G6 release. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-14/index.html`; this slice page is `planning/review-site/CAP-14/CAP-14.S04.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

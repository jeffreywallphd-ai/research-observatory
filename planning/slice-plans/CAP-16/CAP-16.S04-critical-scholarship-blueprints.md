...
slice_id: CAP-16.S04
title: Critical scholarship blueprints
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-16.S04` — Critical scholarship blueprints |
| Slice outcome | Critical manuscripts receive rigorous evidence and argument scaffolding without epistemic flattening. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-16.S04.T01`, `CAP-16.S04.T02`, `CAP-16.S04.T03` |
| Slice dependencies | `CAP-16.S01.T03`, `CAP-10.S05.T03` |
...
Critical manuscripts receive rigorous evidence and argument scaffolding without epistemic flattening.
...
**Implementation thesis.** Give critical scholarship a first-class, contestable architecture for standpoint, problematization, evidence, counter-reading and reflexivity.
...
- Critical scholarship pattern library.
- Standpoint, assumptions, stakeholders, authority/dependency and normative claim plans.
- Counter-reading, reflexivity and alternative-framing requirements.
- Conference and journal variants.
...
- **Consumes:** `CAP-10 critical/hermeneutic dossier and CAP-09 claim/evidence graph` and formal dependencies `CAP-16.S01.T03`, `CAP-10.S05.T03`.
- **Produces:** Critical manuscripts receive rigorous evidence and argument scaffolding without epistemic flattening.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-16.S01.T03`, `CAP-10.S05.T03`.
...
|---|---|
| `packages/core/manuscript/critical` | critical argument/problematization model |
| `packages/method-packs/manuscript-critical` | critical architecture packs |
| `services/core-api/manuscripts/critical` | alternative pattern and completeness analysis |
...
|---|---|
| `CriticalBlueprintPattern` | Versioned CriticalBlueprintPattern state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
| `StandpointPlan` | Versioned StandpointPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
| `ProblematizationPlan` | Versioned ProblematizationPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
| `StakeholderPlan` | Versioned StakeholderPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
| `NormativeClaimPlan` | Versioned NormativeClaimPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
| `ReflexivityPlan` | Versioned ReflexivityPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
| `CounterReadingPlan` | Versioned CounterReadingPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to critical scholarship blueprints. |
...
- `AI-proposed critical readings remain candidate interpretations until accepted by the researcher.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `The blueprint preserves competing readings rather than collapsing to one diagnosis.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Normative and empirical claims retain distinct evidence statuses.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-10 critical/hermeneutic dossier and CAP-09 claim/evidence graph.
- **Downstream:** CAP-16.S05 blueprint approval and CAP-18.S04 critical drafting.
...
- Show standpoint, excluded alternatives, stakeholders, burdens/benefits and counter-readings near the top.
- Require explicit researcher adjudication of the central critical architecture.
...
- Protect sensitive community/stakeholder annotations and unpublished critiques.
- Avoid automated labeling of people or organizations as harmful/misconducting.
...
|---|---|---|
| critical concept checklist replaces contextual interpretation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| counter-reading omitted | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| normative claim presented as empirical finding | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| ontology pack constrains an emergent interpretation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| critical scholar walkthroughs | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| candidate-versus-accepted interpretation state tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| counter-reading and reflexivity completeness | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| plurality-preservation regression fixtures | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| workspace supports large memo/evidence sets without flattening | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| critical prompts use bounded evidence packets | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| all model-produced interpretations remain traceable | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-10 critical/hermeneutic dossier and CAP-09 claim/evidence graph. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-16.S05 blueprint approval and CAP-18.S04 critical drafting. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-16/index.html`; this slice page is `planning/review-site/CAP-16/CAP-16.S04.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

...
slice_id: CAP-16.S06
title: Manuscript blueprint production acceptance
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-16.S06` — Manuscript blueprint production acceptance |
| Slice outcome | The article-architecture capability is production-ready across research and output types. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-16.S06.T01`, `CAP-16.S06.T02`, `CAP-16.S06.T03` |
| Slice dependencies | `CAP-16.S05.T03` |
...
The article-architecture capability is production-ready across research and output types.
...
**Implementation thesis.** Accept manuscript blueprints only when method/argument completeness, usability, export fidelity and downstream drafting contracts are proven.
...
- Cross-type golden acceptance suite.
- Author and methodologist usability evaluation.
- Export and venue-profile validation.
- G7 manuscript-blueprint readiness evidence.
...
- **Consumes:** `all CAP-16 slices and cross-platform desktop foundation` and formal dependencies `CAP-16.S05.T03`.
- **Produces:** The article-architecture capability is production-ready across research and output types.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-16.S05.T03`.
...
| `CAP-16-D08` | Theory blueprint model | **Provide multiple argument/contribution architectures with explicit warrants, mechanisms, boundaries and counterarguments** | Force all theory papers into an empirical-section template | Theory development requires flexible but inspectable argument architecture. | None |
| `CAP-16-D09` | Critical blueprint model | **Model standpoint, problematization, evidence, reflexivity, counter-readings and normative implications** | Treat critical work as a conventional literature review with a discussion section | Critical scholarship has distinct evidentiary and reflexive obligations. | None |
...
|---|---|
| `tests/acceptance/manuscript-blueprint` | end-to-end fixtures |
| `benchmarks/manuscript` | scale and export corpus |
| `artifacts/gates/G7-blueprint` | criterion-linked acceptance bundle |
...
|---|---|
| `BlueprintAcceptanceRun` | Versioned BlueprintAcceptanceRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint production acceptance. |
| `BlueprintUsabilityFinding` | Versioned BlueprintUsabilityFinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint production acceptance. |
| `BlueprintGateDecision` | Versioned BlueprintGateDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint production acceptance. |
...
- `Acceptance covers empirical, theory and critical work, not only one template.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Known limitations and unsupported venue rules are explicit in the gate bundle.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** all CAP-16 slices and cross-platform desktop foundation.
- **Downstream:** G7 blueprint approval and CAP-18 production drafting.
...
- Validate complete workflows from objective selection to approved skeleton/export.
- Include keyboard, screen-reader, light/dark and error recovery studies.
...
- Use synthetic/permissive fixtures for automated acceptance.
- Confirm private venue/manuscript data never leaves approved boundaries.
...
|---|---|---|
| one research type passes while another is incomplete | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| export appears successful but semantic sidecar diverges | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| usability participants cannot understand readiness or venue provenance | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| gate evidence references a different build | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| all CAP-16 exit criteria mapped to immutable evidence | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| expert review across empirical, theory and critical types | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| cross-format semantic diff | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| independent capability review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| acceptance corpus runs within release qualification window | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| no unresolved critical/high usability or integrity issue | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| resource results reported for Windows plus qualified macOS/Linux targets | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
all CAP-16 slices and cross-platform desktop foundation. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

G7 blueprint approval and CAP-18 production drafting. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-16/index.html`; this slice page is `planning/review-site/CAP-16/CAP-16.S06.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

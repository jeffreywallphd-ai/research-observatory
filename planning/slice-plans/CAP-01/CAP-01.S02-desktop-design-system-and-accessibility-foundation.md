...
slice_id: CAP-01.S02
title: Desktop design system and accessibility foundation
status: proposed
wave: W1
priority: P0
deployment_profiles:
...
| Slice | `CAP-01.S02` - Desktop design system and accessibility foundation |
| Slice outcome | Reusable components express status, provenance, evidence, uncertainty, and human decision states consistently. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-01.S02.T01`, `CAP-01.S02.T02`, `CAP-01.S02.T03` |
| Slice dependencies | `CAP-01.S01.T01` |
...
Make the research workbench consistently understandable, accessible, and trustworthy across dense evidence, graph, authoring, and workflow surfaces.
...
**Implementation thesis.** Implement the approved Academic Minimal design as semantic tokens plus accessible primitives, not as copied page CSS. Validate components in isolation and validate complete workflow pages in both themes.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-01.S01.T01`.
- **Produces:** Reusable components express status, provenance, evidence, uncertainty, and human decision states consistently.
...
- `planning/backlog.yaml` for `CAP-01.S02` and its task state/dependencies.
...
1. **CSS custom properties are the canonical token transport; component code consumes semantic tokens rather than literal colors or measurements.**
2. **Target WCAG 2.2 AA for all researcher-facing workflows, including focus visibility, target size, non-drag alternatives, and accessible authentication behavior where applicable.**
3. **Use native HTML elements first; use WAI-ARIA patterns only where native semantics are insufficient.**
4. **Provide a reusable state model for loading, empty, offline, denied, stale, partial, failed, and recovery-required conditions.**
5. **Keep light and dark themes semantically equivalent; status may never be communicated by color alone.**
6. **Create a component-story and accessibility test surface for every shared primitive and complex pattern.**
...
- `packages/ui-tokens` generated semantic tokens and contrast tests.
- `packages/ui-components` buttons, fields, dialogs, navigation, tables, grids, badges, callouts, drawers, stepper/workflow, source anchors, and evidence states.
- Storybook or equivalent local component catalog with automated a11y checks.
- Visual regression fixtures for approved representative states and theme parity.
...
- Token names, component APIs, focus behavior, keyboard maps, and semantic state names are versioned contracts.
- Every complex component documents accessible name, role, state, keyboard behavior, and empty/error semantics.
- UI implementation manifests identify approved reference ID and component/token versions.
...
- Use 4 px spacing increments, restrained 10 px card radii, 44 px primary control height, and the approved desktop shell measurements unless the reference is revised first.
- Dense tables use optional compact mode without reducing interactive target accessibility.
- Workflows expose current stage, completed stages, next action, checkpoints, and supporting tools consistently.
- Long-running operations remain inspectable and cancellable; no indefinite spinner without status.
...
- Sanitize any rich text rendered in reusable components.
- Never use disabled-looking styling for active controls or rely on hidden hover-only actions.
- Prevent focus from entering inert or concealed panes and restore focus after dialogs/drawers close.
...
- Each async component supports retry and inspect-details actions where recovery is possible.
- Error boundaries preserve researcher-entered form data when technically safe.
- Offline and degraded states explicitly distinguish unavailable remote functions from fully local functions.
...
### 9.1 `CAP-01.S02.T01` - Implement design tokens and core components
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S02.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-01.S02.T02` - Establish keyboard, focus, and screen-reader behavior
...
**Risk / review gate:** `high` / `accessibility-review`  
**Verification profiles:** `desktop`
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S02.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-01.S02.T03` - Create consistent loading, error, offline, and recovery states
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-01.S02.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
- Shared component bundle remains tree-shakeable and avoids importing graph, editor, or PDF engines.
- Theme change should complete without page reload or visible unstyled content.
- Large tables and navigation should remain responsive at 10,000-row mock datasets through virtualization/pagination.
...
- Automated accessibility results, visual diffs, and token-conformance reports are retained as build evidence.
- Runtime logging records component exceptions and failed focus restoration without collecting content.
...
- CAP-01.S01 consumes the shell components.
- All later desktop slices consume the same primitives and may not create parallel page-level design systems.
- CAP-14 requalifies accessibility and visual parity on macOS/Linux.
...
- Token changes are non-breaking only when semantic meaning and contrast remain stable; breaking visual/interaction changes require a new approved UI reference.
- Component deprecations require codemods or a documented migration path.
...
- The promised outcome is demonstrable end to end: Reusable components express status, provenance, evidence, uncertainty, and human decision states consistently.
...
- ADR: Design-token build and distribution strategy.
- ADR: Component catalog and visual-regression infrastructure.
...
| `STORYBOOK_A11Y` | [Storybook Accessibility Testing](https://storybook.js.org/docs/writing-tests/accessibility-testing) - Storybook | Component-level accessibility checks. |
...
2. Confirm `CAP-01` is the active capability campaign and `CAP-01.S02` is the next eligible slice.
3. Claim the first READY task in order: `CAP-01.S02.T01`. Do not globally select work outside the capability.

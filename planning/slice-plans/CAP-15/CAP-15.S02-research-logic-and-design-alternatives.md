...
slice_id: CAP-15.S02
title: Research logic and design alternatives
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-15.S02` — Research logic and design alternatives |
| Slice outcome | Researchers receive plural, evidence-backed design options and retain final authority. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-15.S02.T01`, `CAP-15.S02.T02`, `CAP-15.S02.T03` |
| Slice dependencies | `CAP-15.S01.T03` |
...
Researchers receive plural, evidence-backed design options and retain final authority.
...
**Implementation thesis.** Generate and compare method-plural research logics as transparent alternatives, then let the researcher select or combine them with an explicit scholarly rationale.
...
- Research question/mechanism/estimand or interpretive aim alternatives.
- Design-family comparison across epistemologies.
- Human selection and rationale adjudication.
...
- **Consumes:** `study-design domain/evidence foundation` and formal dependencies `CAP-15.S01.T03`.
- **Produces:** Researchers receive plural, evidence-backed design options and retain final authority.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-15.S01.T03`.
...
|---|---|---|---|---|
| `CAP-15-D04` | Alternative generation | **Generate multiple source-grounded design alternatives with tradeoffs and a recommendation** | Generate one “best” design with no comparison | Plural alternatives reveal assumptions and preserve researcher authority. | None |
...
| `CAP-15-D11` | Qualitative/mixed support | **Provide coding/interpretive and integration logic without coercing it into quantitative fields** | Translate all qualitative work into variables and hypotheses | APA JARS and method practice require distinct qualitative and mixed-method reporting needs. | None |
...
|---|---|
| `modules/study_design/logic` | questions, mechanisms and inference aims |
| `modules/study_design/alternatives` | family packs and tradeoff matrix |
| `packages/ui/study-design/alternatives` | inspect, contest and adjudicate |
...
|---|---|
| `ResearchPurpose` | Versioned ResearchPurpose state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
| `ResearchQuestion` | Versioned ResearchQuestion state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
| `HypothesisOrProposition` | Versioned HypothesisOrProposition state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
| `Estimand` | Versioned Estimand state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
| `InterpretiveAim` | Versioned InterpretiveAim state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
| `DesignLogicAlternative` | Versioned DesignLogicAlternative state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
| `SelectionDecision` | Versioned SelectionDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to research logic and design alternatives. |
...
- `DesignAlternativeGenerator.generate(context,mode)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `DesignComparator.compare(alternatives,criteria)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `DesignSelection.adjudicate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** study-design domain/evidence foundation.
- **Downstream:** sampling, measurement and procedures.
...
- Present alternatives in parallel with assumptions, evidence, inference limits, feasibility and ethics.
- Do not label one design universally best; explain why the recommendation fits the project intent.
- Allow researchers to combine elements and record consultations/open tradeoffs.
...
- Model prompts include only authorized project evidence.
- Generated alternatives are labeled candidate/inferred and not methods approval.
- Sensitive opportunity ideas stay local unless egress is approved.
...
|---|---|---|
| AI forces hypotheses on interpretive work | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Alternatives differ only cosmetically | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Recommendation cites irrelevant method literature | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Selected hybrid design contains incompatible inference claims | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | study-design, ai |
...
|---|---|---|
| Gold cases for quantitative, qualitative, mixed, computational and field designs | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Diversity/nonredundancy and evidence-grounding evaluation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Human methods review and rationale persistence | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Contradiction audit after combining alternatives | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Alternative generation bounded by approved model/evidence budget | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| At least three substantively distinct options where feasible | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Recommendation latency supports interactive review with cancellable background work | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
study-design domain/evidence foundation. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

sampling, measurement and procedures. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-15/index.html`; this slice page is `planning/review-site/CAP-15/CAP-15.S02.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

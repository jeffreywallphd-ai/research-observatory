...
slice_id: CAP-02.S05
title: Backup, restore, relocation, and lab portability
status: proposed
wave: W5
priority: P0
deployment_profiles:
...
| Slice | `CAP-02.S05` - Backup, restore, relocation, and lab portability |
| Slice outcome | Researchers can protect and move projects without breaking identities, provenance, or evidence links. |
| Wave / priority | `W5` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-02.S05.T01`, `CAP-02.S05.T02`, `CAP-02.S05.T03` |
| Slice dependencies | `CAP-02.S04.T03`, `CAP-03.S05.T03` |
...
Allow researchers and labs to recover, move, clone, and share authorized projects without breaking provenance, source anchors, encryption, or version compatibility.
...
**Implementation thesis.** Create a manifest-driven, streaming encrypted backup bundle from a consistent SQLite snapshot and referenced encrypted objects. Verify before declaration of success, restore into staging, and publish atomically with new local identity when cloning.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-02.S04.T03`, `CAP-03.S05.T03`.
- **Produces:** Researchers can protect and move projects without breaking identities, provenance, or evidence links.
...
- `planning/backlog.yaml` for `CAP-02.S05` and its task state/dependencies.
...
- `CAP-02.S04.T03` is complete or explicitly gated.
- `CAP-03.S05.T03` is complete or explicitly gated.
...
1. **Use SQLite online backup or an equivalent protected snapshot under a project write barrier; checkpoint WAL and verify the snapshot.**
2. **Represent backups as versioned manifests plus database snapshot, referenced object envelopes, workflow checkpoints, and optional rebuildable caches excluded by default.**
3. **Encrypt portable bundles with a distinct export key protected by recipient/passphrase or lab policy; never copy the profile vault root.**
4. **Restore into staging, verify hashes/signatures/schema/keys, run migrations if allowed, then atomically publish.**
5. **Differentiate restore (same project identity), clone (new project instance identity with lineage), and relocate (same identity/new path).**
6. **Shared caches are read-through and non-authoritative; project correctness never depends on cache availability.**
...
- Backup planner and consistency barrier.
- Streaming bundle writer/reader and manifest schema.
- Restore/clone/relocate service and conflict resolver.
- Scheduled backup policy, retention, verification drills, and lab transfer UI.
...
- Backup manifest includes project/package/schema versions, source project ID, snapshot time, object inventory, cryptographic parameters, excluded classes, application compatibility, and verification result.
- A backup is not `complete` until database integrity, object inventory, and bundle authentication pass.
- Restore returns a detailed compatibility/repair plan before mutation.
...
- Backup UI distinguishes local recovery copy, portable encrypted transfer, and managed lab backup.
- Users choose whether optional caches/exports are included and see size/time estimates.
- Restore preview lists conflicts, missing capabilities, required migrations, and identity disposition.
...
- Portable bundle names and manifest exposure are minimized; sensitive metadata remains inside the encrypted envelope where feasible.
- Use authenticated streaming encryption and verify before any decrypted material is committed.
- Never include unrelated OS-stored secrets or provider credentials by default.
- Rights policies can exclude non-portable licensed content and retain reacquisition instructions instead.
...
- Test interruption at every bundle/restore stage, insufficient disk, missing object, wrong key, corrupted segment, incompatible version, and existing project collision.
- A failed restore removes or quarantines staging output and never overwrites the current project.
- Scheduled backups include periodic restore drills, not only archive creation.
...
### 9.1 `CAP-02.S05.T01` - Implement consistent project snapshot and backup creation
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S05.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-02.S05.T02` - Implement verified restore, clone, and relocation
...
**Risk / review gate:** `high` / `agent-review`  
**Verification profiles:** `data`, `desktop`, `e2e-local`

**Expected deliverables**
- Restore wizard and service operations for original restore, clone-as-new, location change, and conflict handling.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S05.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
...
### 9.3 `CAP-02.S05.T03` - Create portable lab project and shared-cache conventions
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-02.S05.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
5. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
...
- Stream large projects with bounded memory and incremental progress.
- Support incremental backup later only if manifest-chain recovery remains independently verifiable; full baseline backup is primary.
- Record throughput and size ratios but do not compress already compressed/encrypted content wastefully.
...
- Record backup/restore stages, included/excluded classes, bytes, duration, verification, retention decisions, and trace IDs.
- Support bundles reference backup attempt IDs without exposing passphrases or content.
...
- CAP-01.S05 invokes pre-update backup policy where schema risk requires it.
- CAP-14 uses these bundles for cross-platform transfer qualification.
- University promotion later consumes the same portable project/export contracts.
...
- Bundle readers support a declared range and may invoke tested project migrations only after restoring an untouched source snapshot.
- New bundle formats require backward-reader tests or a documented bridge tool.
...
- The promised outcome is demonstrable end to end: Researchers can protect and move projects without breaking identities, provenance, or evidence links.
...
- ADR: Portable bundle format and encryption recipient model.
- ADR: Lab shared-cache layout and authority rules.
- ADR: Backup retention and verified-restore schedule.
...
| `SQLITE_VACUUM` | [SQLite VACUUM INTO](https://www.sqlite.org/lang_vacuum.html#vacuuminto) - SQLite | Compact snapshot/export tooling. |
...
2. Confirm `CAP-02` is the active capability campaign and `CAP-02.S05` is the next eligible slice.
3. Claim the first READY task in order: `CAP-02.S05.T01`. Do not globally select work outside the capability.

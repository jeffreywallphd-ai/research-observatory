...
slice_id: CAP-16.S03
title: Theory article blueprints
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-16.S03` — Theory article blueprints |
| Slice outcome | Theory manuscripts receive coherent argument architecture while preserving epistemic plurality. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-16.S03.T01`, `CAP-16.S03.T02`, `CAP-16.S03.T03` |
| Slice dependencies | `CAP-16.S01.T03`, `CAP-09.S05.T03` |
...
Theory manuscripts receive coherent argument architecture while preserving epistemic plurality.
...
**Implementation thesis.** Represent theory manuscripts as explicit argument and contribution architectures while preserving multiple legitimate forms of theorizing.
...
- Theory article architecture pattern library.
- Construct/mechanism/proposition/argument/contribution plans.
- Evidence and counterargument requirements.
- Conference and journal variants with transparent compression.
...
- **Consumes:** `CAP-09 theory/construct/claim graph and CAP-10 accepted opportunity dossier` and formal dependencies `CAP-16.S01.T03`, `CAP-09.S05.T03`.
- **Produces:** Theory manuscripts receive coherent argument architecture while preserving epistemic plurality.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-16.S01.T03`, `CAP-09.S05.T03`.
...
|---|---|
| `packages/core/manuscript/theory` | argument-architecture engine |
| `packages/method-packs/manuscript-theory` | governed theory patterns |
| `services/core-api/manuscripts/theory` | pattern comparison and blueprint construction |
...
|---|---|
| `TheoryBlueprintPattern` | Versioned TheoryBlueprintPattern state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
| `ArgumentNode` | Versioned ArgumentNode state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
| `WarrantPlan` | Versioned WarrantPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
| `MechanismPlan` | Versioned MechanismPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
| `BoundaryPlan` | Versioned BoundaryPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
| `CounterargumentPlan` | Versioned CounterargumentPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
| `TheoryContributionPlan` | Versioned TheoryContributionPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to theory article blueprints. |
...
- `Patterns are alternatives, not a universal mandatory sequence.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Every planned theoretical claim links to literature evidence or accepted researcher reasoning status.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Competing interpretations and unresolved disputes remain representable.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-09 theory/construct/claim graph and CAP-10 accepted opportunity dossier.
- **Downstream:** CAP-16.S05 blueprint approval and CAP-18.S04 theory drafting.
...
- Present multiple theory architectures with assumptions, strengths and risks.
- Visualize argument dependencies and section placement before approval.
...
- Treat unpublished theoretical ideas and memos as private project content.
- Do not expose idea graphs to external providers without explicit egress approval.
...
|---|---|---|
| forced pattern fit loses argument content | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| circular argument dependency | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| unsupported theoretical assertion promoted as established evidence | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| conference compression removes contribution logic | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | manuscript, graph |
...
|---|---|---|
| expert-authored theory blueprint fixtures | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| argument acyclicity and dependency tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| evidence/interpretation status completeness | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| conference/journal variant comparison | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| argument graph interaction remains responsive for representative manuscript scale | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| pattern comparison returns bounded alternatives rather than unbounded generation | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| no silent loss during structure transformations | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-09 theory/construct/claim graph and CAP-10 accepted opportunity dossier. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-16.S05 blueprint approval and CAP-18.S04 theory drafting. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-16/index.html`; this slice page is `planning/review-site/CAP-16/CAP-16.S03.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

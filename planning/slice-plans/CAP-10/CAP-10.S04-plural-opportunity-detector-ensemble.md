...
slice_id: CAP-10.S04
title: Plural opportunity detector ensemble
status: proposed
wave: W9
priority: P1
deployment_profiles:
...
| Slice | `CAP-10.S04` — Plural opportunity detector ensemble |
| Slice outcome | Opportunity signals are separated by contribution logic, evidence requirements, and false-positive risks. |
| Wave / priority | `W9` / `P1` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-10.S04.T01`, `CAP-10.S04.T02`, `CAP-10.S04.T03` |
| Slice dependencies | `CAP-10.S03.T03`, `CAP-09.S03.T03` |
...
Opportunity signals are separated by contribution logic, evidence requirements, and false-positive risks.
...
**Implementation thesis.** Run distinct opportunity detector families with detector-specific evidence and false-positive explanations, then assemble candidates without implying that sparse, structurally unusual or model-inferred patterns are inherently important.
...
- Explicit-gap, coverage and future-work detectors.
- Contradiction, boundary, measurement and robustness detectors.
- Bridge, structural, temporal, method, data and theory-integration detectors.

### 2.2 Explicit non-goals

- A single universal “gap score.”
- Automatically accepting detector output as an opportunity.
- Letting density/topology substitute for theoretical significance.
...
- **Consumes:** `CAP-10.S03.T03`, `CAP-09.S03.T03` and the handoffs in Section 13.
- **Produces:** Opportunity signals are separated by contribution logic, evidence requirements, and false-positive risks.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-10.S03.T03`, `CAP-09.S03.T03`.
...
|---|---|---|---|---|
| **Detector contract** | Each detector declares opportunity type, required inputs, comparison universe, algorithm/model, output evidence, uncertainty, known false positives, benchmark and version. | One generic prompt asking an LLM to find gaps. | Different opportunity types have different epistemic and technical validity conditions. | [GAPMAP: Mapping Scientific Knowledge Gaps in Biomedical Literature Using Large Language Models](https://arxiv.org/abs/2510.25055) |
| **Structural signals** | Use density/bridge/topological methods only to propose regions/relationships; require semantic/evidence interpretation and challenger review. | Label empty/low-density graph regions as gaps. | Structural absence can be trivial, infeasible or caused by corpus bias. | [ROAD-tv: Research Opportunity Discovery via Topological Data Analysis and Adversarial Multi-LLM Validation](https://doi.org/10.1016/j.procs.2026.01.036) |
| **Promotion** | Candidates enter unverified state and require evidence packet, significance mechanism and S02 challenge before dossier decision. | Auto-add top-scoring candidates as validated gaps. | The platform’s differentiator is disciplined candidate generation plus disconfirmation. | [ROAD-tv: Research Opportunity Discovery via Topological Data Analysis and Adversarial Multi-LLM Validation](https://doi.org/10.1016/j.procs.2026.01.036) |
...
|---|---|
| `services/core/research_observatory/modules/opportunities/detectors/base.py` | Detector interface, manifests and candidate contract. |
| `services/core/research_observatory/modules/opportunities/detectors/explicit.py` | Future-work/limitation extraction and recency check. |
| `services/core/research_observatory/modules/opportunities/detectors/comparative.py` | Contradiction/boundary/measurement/robustness. |
| `services/core/research_observatory/modules/opportunities/detectors/structural.py` | Coverage/bridge/topological/temporal/method/data/theory. |
| `services/core/research_observatory/modules/opportunities/evidence_packet.py` | Supporting/disconfirming/coverage diagnostics. |
| `tests/benchmarks/opportunity-detectors` | Type-specific gold/negative/false-positive fixtures. |
...
|---|---|
| `DetectorDefinition` | type, requirements, algorithm/model, thresholds, benchmark and version |
| `DetectorRun` | snapshot, parameters, inputs, status, metrics and manifest |
| `OpportunitySignal` | detector-specific signal, location/entities, strength, uncertainty and raw evidence |
| `OpportunityCandidate` | type, research question draft, evidence packet, false-positive analysis and lifecycle |
| `DetectorEvaluation` | precision/recall/expert validity/diversity/calibration by type |
...
- `OpportunityDetector.validate_inputs/run/cancel/resume` implements a common durable contract.
- `CandidateAssembler.assemble(signals, snapshot)` preserves detector identities and disagreements.
- `SignificancePrompt.prepare` asks why resolving the candidate matters without treating the answer as evidence.
- `CandidatePromotion.submit_for_challenge` requires required evidence/coverage diagnostics.
...
- Opportunity Radar groups candidates by type and explains detector logic and common false positives.
- Users can inspect the underlying matrix, graph, claims, passages or temporal series that produced each signal.
- Detector thresholds/runs are versioned and previewable; changing them does not overwrite prior candidates.
- Low evidence or corpus-bias warnings are prominent.
...
- Detector workers have bounded project-local access and cannot independently contact unapproved sources/providers.
- Private corpus structure/ideas are excluded from telemetry and cross-project comparison by default.
- Untrusted source text cannot invoke tools or change detector configuration.
...
|---|---|
| Required normalized data missing | Detector reports not-runnable/partial coverage rather than zero gaps. |
| Run canceled/restarted | Checkpoint detector-specific partitions and deduplicate signals. |
| Detector versions disagree | Preserve runs/candidates separately and expose sensitivity. |
| High false-positive benchmark | Do not enable detector by default; retain research-preview status. |
...
| Verification profiles | `novelty`, `evidence`, `search` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S04.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `novelty`, `graph`, `evidence` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S04.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `research-preview-review` |
| Verification profiles | `novelty`, `graph`, `search` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-10.S04.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
3. Pin model, prompt/schema, runtime/provider and evaluation manifests. Enforce local schema validation, source/evidence closure, egress policy and deterministic fallback; treat model output as untrusted candidate state.
4. Preserve canonical evidence and decisions as authority; update only the versioned derivative projection/candidate relation. Test disputed, stale, cyclic, missing and rebuild cases.
5. Exercise security, privacy, rights and research-integrity boundaries with malformed/untrusted inputs, authorization denial, confidential-content redaction and unsupported-state fixtures. Failure must leave canonical state unchanged or exactly recoverable.
...
|---|---|
| Detector run | Each definition declares reference corpus/time/memory budget; no unbounded all-pairs operation. |
| Incremental mode | Recompute affected partitions when dependencies permit; otherwise full versioned run. |
| Radar query | p95 < 500 ms for 100,000 signals/candidates with filters. |
| Evaluation | Every production-enabled detector has type-specific gold/negative set and expert-review sample. |
...
- runs/status/cost
- signals/candidates by type
- expert validity/false-positive
- challenge defeat/narrow rate
- diversity/convergence
- coverage/bias warning frequency
...
- Consumes CAP-08 evidence, CAP-09 graph/comparison/synthesis and CAP-06 corpus diagnostics.
- Produces candidates to S06 radar and S03 dossier; S02 challenges promoted candidates.
- Living monitor can incrementally rerun affected detectors.
...
| Risk | Mitigation |
|---|---|
| Goodhart gap scoring | Separate raw dimensions and human significance review. |
| Detector monoculture | Independent families, ablations and diversity evaluation. |
| Corpus bias amplified | Coverage/reflexivity diagnostics in every candidate packet. |

## 19. Required ADRs and human decisions

- ADR-CAP10-DETECTORS: common detector/candidate contract and production-enablement criteria.
...
- **GAPMAP: Mapping Scientific Knowledge Gaps in Biomedical Literature Using Large Language Models** — Explicit and implicit gap inference with human validation.  

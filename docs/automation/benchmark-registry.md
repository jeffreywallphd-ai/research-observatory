...
Expected outputs are nonredirected files at the canonical `evaluation/baselines/<benchmark-id>.json` path, which is also recorded in current and historical lineage. A prompt path or hash cannot change while retaining the same prompt ID and version. Git-aware validation walks the complete unsimplified commit graph reachable from `HEAD`, validates each historical registry against its contemporaneous schema, verifies its assets against the hashes recorded at that commit, and replays every parent edge of a merge. Invalid historical shapes are reported and excluded from structural replay, so malformed snapshots fail actionably rather than crashing. Each approval path may have only one blob identity across the graph. The dirty worktree is checked against `HEAD`, Git inspection fails closed, and shallow checkouts are rejected because they cannot prove complete history. Source exports without Git metadata retain static validation.

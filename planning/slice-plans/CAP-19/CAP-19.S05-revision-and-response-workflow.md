...
slice_id: CAP-19.S05
title: Revision and response workflow
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-19.S05` — Revision and response workflow |
| Slice outcome | Reviewer feedback becomes a transparent, author-controlled revision plan and auditable response. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-19.S05.T01`, `CAP-19.S05.T02`, `CAP-19.S05.T03` |
| Slice dependencies | `CAP-19.S04.T03` |
...
Reviewer feedback becomes a transparent, author-controlled revision plan and auditable response.
...
**Implementation thesis.** Turn accepted reviewer comments into a governed, selective revision plan and traceable response rather than directly rewriting the paper.
...
- Comment triage and revision-plan mapping.
- Point-by-point response/rebuttal artifact.
- Selective manuscript change sets and diffs.
- Comment resolution and targeted follow-up review.
...
- **Consumes:** `CAP-19.S04 reports/synthesis and CAP-18 stable manuscript blocks/change sets` and formal dependencies `CAP-19.S04.T03`.
- **Produces:** Reviewer feedback becomes a transparent, author-controlled revision plan and auditable response.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-19.S04.T03`.
...
|---|---|
| `packages/core/revision` | comment-action-resolution domain |
| `apps/desktop/revision-response` | approved workspace |
| `services/core-api/revisions` | plan, change-set and response commands |
| `workers/reviewer/followup` | targeted re-review |
...
|---|---|
| `RevisionPlan` | Versioned RevisionPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
| `RevisionAction` | Versioned RevisionAction state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
| `CommentResolution` | Versioned CommentResolution state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
| `ResponseItem` | Versioned ResponseItem state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
| `ResponseLetter` | Versioned ResponseLetter state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
| `RevisionChangeSet` | Versioned RevisionChangeSet state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
| `FollowUpReview` | Versioned FollowUpReview state with stable identity, lifecycle/status, provenance and policy metadata appropriate to revision and response workflow. |
...
- `Every implemented revision links to reviewer comment or researcher-initiated rationale.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `The researcher approves change sets before canonical manuscript update.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Response text distinguishes accepted, partially accepted and declined recommendations with rationale.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-19.S04 reports/synthesis and CAP-18 stable manuscript blocks/change sets.
- **Downstream:** CAP-19.S06 acceptance and publication-ready package.
...
- Conform to `revision-response.html`.
- Place comment, proposed action, manuscript diff, evidence and response side by side.
- Provide explicit unresolved/high-severity queue and next action.
...
- Response exports obey manuscript confidentiality.
- Do not reveal hidden reviewer configuration or private provider metadata.
...
|---|---|---|
| comment points to deleted block | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| two comments demand conflicting revisions | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| change set fails to apply | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| follow-up reviewer uses wrong snapshot | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | reviewer, manuscript |
...
| Review gate | `agent-review` |
| Verification profiles | reviewer, manuscript, ai |
...
| Estimate / risk | `M` / `high` |
| Review gate | `agent-review` |
| Verification profiles | reviewer, manuscript, e2e-local |
...
|---|---|---|
| comment-to-change traceability | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| conflict and partial-acceptance workflow | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| response export round-trip | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| selective follow-up review exactness | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| large review sets filter and batch without losing identity | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| diff generation interactive | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| follow-up review avoids unnecessary full-panel cost | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-19.S04 reports/synthesis and CAP-18 stable manuscript blocks/change sets. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-19.S06 acceptance and publication-ready package. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-19/index.html`; this slice page is `planning/review-site/CAP-19/CAP-19.S05.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

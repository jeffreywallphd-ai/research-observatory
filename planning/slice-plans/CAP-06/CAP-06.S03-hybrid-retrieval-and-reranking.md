...
| Slice | `CAP-06.S03` — Hybrid retrieval and reranking |
| Slice outcome | Search ensembles combine exact, semantic, graph, and project-specific relevance while preserving component evidence. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-06.S03.T01`, `CAP-06.S03.T02`, `CAP-06.S03.T03` |
| Slice dependencies | `CAP-06.S01.T03`, `CAP-06.S02.T03` |
...
Search ensembles combine exact, semantic, graph, and project-specific relevance while preserving component evidence.
...
**Implementation thesis.** Fuse independently useful retrievers at the rank level, apply optional reranking only to a bounded candidate set, and persist a complete manifest so every result order can be inspected and replayed.
...
- Lexical/semantic fusion, deduplication and stable ties.
- Optional local or approved cross-encoder/LLM reranking with policy and fallback.
- SearchRun manifest, result snapshots, replay and drift report.

### 2.2 Explicit non-goals

- Training a universal ranker.
- Silently changing corpus membership based on ranking.
- Hiding component ranks behind one confidence score.
...
- **Consumes:** `CAP-06.S01.T03`, `CAP-06.S02.T03` and the handoffs in Section 13.
- **Produces:** Search ensembles combine exact, semantic, graph, and project-specific relevance while preserving component evidence.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-06.S01.T03`, `CAP-06.S02.T03`.
...
|---|---|
| `services/core/research_observatory/modules/corpus_search/fusion.py` | RRF, deduplication, tie handling and component explanation. |
| `services/core/research_observatory/ports/reranker.py` | Bounded reranking contract through CAP-07. |
| `services/core/research_observatory/modules/corpus_search/search_runs.py` | Manifest, result snapshot, replay and drift comparison. |
| `packages/contracts/search-run` | Versioned run/result/replay schemas. |
| `packages/ui/search` | Component ranking, rerank state and replay-difference views. |
| `tests/benchmarks/search` | Gold queries and ablation reports. |
...
|---|---|
| `SearchRun` | query, branch, snapshot, retriever manifests, fusion config, reranker, filters, errors, result artifact and trace |
| `RankComponent` | retriever name, rank, raw signal, weight and explanation |
| `ResultSetSnapshot` | ordered canonical IDs, component ranks, checksum and rights context |
| `ReplayReport` | exact/drifted status, changed inputs and result differences |
...
- `HybridSearch.execute(run_spec)` invokes retrievers concurrently under one cancellation token.
- `RankFusion.fuse(component_lists, policy)` is pure and deterministic.
- `Reranker.rerank(query, candidates, policy)` returns bounded scores and provenance.
- `SearchRun.replay(id, target_snapshot)` returns exact output or structured drift report.
...
- Users can inspect lexical rank, semantic rank, fusion contribution and reranker changes.
- Reranker disabled/timeout states are visible and do not change the saved query.
- Replay presents added/removed/reordered results and which corpus/index/model input changed.
...
- Rights and project filters are applied before result disclosure and before remote reranking egress.
- Remote reranking uses CAP-07 preview/consent; confidential text is local-only by policy.
- Run manifests redact secrets but retain provider/model identity and hashed input artifacts.
...
|---|---|
| One retriever fails | Return a partial result only if policy permits, label missing component and preserve error. |
| Reranker timeout | Cancel reranker and return fused order with explicit fallback status. |
| Duplicate versions | Canonical reconciliation policy selects/display versions while retaining component provenance. |
| Replay inputs unavailable | Return non-replayable report with exact missing snapshot/model details. |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-06.S03.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-06.S03.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
| Review gate | `agent-review` |
| Verification profiles | `search`, `service`, `data` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-06.S03.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
|---|---|
| Fusion overhead | p95 < 100 ms for 2,000 candidates excluding retrievers. |
| Hybrid first page | p95 < 1 s excluding optional reranking on reference 100,000-work corpus. |
| Reranking | Default top-50 p95 < 5 s CPU or configured remote timeout; cancellation acknowledged < 1 s. |
| Replay comparison | 100,000 result IDs compared < 5 s with bounded memory. |
...
- retriever latency/error/coverage
- fusion overlap and ablation
- reranker uplift and timeout
- result stability
- exact vs drifted replay
- known-item recall/nDCG
...
- Consumes S01 LexicalIndex, S02 VectorIndex and CAP-07 reranker/model gateway.
- Provides complete SearchRun manifests to Search Studio, screening, synthesis and novelty.
- Feeds search evaluation results to CAP-07.S05 evaluation operations.
...
| Risk | Mitigation |
|---|---|
| RRF parameter folklore | Benchmark k and optional weights; version every policy. |
| Reranker bias or homogenization | Ablate and show rank changes; retain disabled baseline. |
| Reproducibility overclaim | Require pinned input hashes for exact replay label. |

## 19. Required ADRs and human decisions

- ADR-CAP06-FUSION: default RRF policy and rerank boundary.
...
- **Reciprocal Rank Fusion Outperforms Condorcet and Individual Rank Learning Methods** — Deterministic rank-level lexical/semantic fusion without score calibration assumptions.  
  https://doi.org/10.1145/1571941.1572114
- **Retrieve & Re-Rank - Sentence Transformers** — Bi-encoder candidate retrieval followed by cross-encoder reranking.  

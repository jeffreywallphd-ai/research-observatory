...
slice_id: CAP-19.S01
title: Reviewer protocol, roles, and independence
status: proposed
wave: W8
priority: P0
deployment_profiles:
...
| Slice | `CAP-19.S01` — Reviewer protocol, roles, and independence |
| Slice outcome | Reviewer simulations are role-bounded, reproducible, and independent before editorial synthesis. |
| Wave / priority | `W8` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-19.S01.T01`, `CAP-19.S01.T02`, `CAP-19.S01.T03` |
| Slice dependencies | `CAP-18.S01.T03` |
...
Reviewer simulations are role-bounded, reproducible, and independent before editorial synthesis.
...
**Implementation thesis.** Define reviewer roles, criteria, transparency and independent execution before generating any critique.
...
- Review protocol, reviewer role, criterion, comment, round and decision contracts.
- Research-type and venue-aware reviewer-panel registry.
- Independent context and prompt/model/evidence manifests.
- Snapshot and allowed-context policy.
...
- **Consumes:** `CAP-16 blueprint/venue profile, CAP-18 draft and CAP-07 model gateway` and formal dependencies `CAP-18.S01.T03`.
- **Produces:** Reviewer simulations are role-bounded, reproducible, and independent before editorial synthesis.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-18.S01.T03`.
...
|---|---|---|---|---|
| `CAP-19-D01` | Reviewer profile registry | **Use governed versioned reviewer-panel profiles by research type, venue and review objective** | Use one generic reviewer prompt for every article | Method and contribution criteria differ across empirical, theory and critical work. | ADR-REVIEWER-PROTOCOL |
| `CAP-19-D02` | Peer-review terminology | **Map roles and transparency settings to ANSI/NISO peer-review terminology** | Invent project-specific names for review models | Standard terminology improves clarity and export/interoperability. | None |
| `CAP-19-D03` | Reviewer independence | **Run each reviewer in an isolated context with an explicit allowed-evidence manifest** | Let reviewer agents share intermediate opinions and converge | Independent first-pass reviews reduce correlated reasoning and premature consensus. | None |
| `CAP-19-D04` | Review snapshot | **Freeze an immutable manuscript, blueprint, evidence and policy snapshot for each review round** | Review the live manuscript while it changes | Reproducibility and response mapping require a stable reviewed object. | None |
| `CAP-19-D15` | Confidentiality and egress | **Keep drafts local by default and enforce explicit provider/permission policy with disclosure** | Upload drafts to any available model automatically | Submitted or unpublished manuscripts are confidential scholarly communications. | None |
...
|---|---|
| `packages/contracts/review` | portable review schemas |
| `packages/core/review` | panel, role, round and snapshot domain |
| `services/core-api/reviews` | panel creation and immutable snapshot commands |
| `packages/reviewer-profiles` | governed profile registry |
...
|---|---|
| `ReviewProtocol` | Versioned ReviewProtocol state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
| `ReviewerRole` | Versioned ReviewerRole state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
| `ReviewCriterion` | Versioned ReviewCriterion state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
| `ReviewerPanelProfile` | Versioned ReviewerPanelProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
| `ReviewRound` | Versioned ReviewRound state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
| `ReviewSnapshot` | Versioned ReviewSnapshot state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
| `AllowedContextManifest` | Versioned AllowedContextManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to reviewer protocol, roles, and independence. |
...
- `Reviewers cannot read one another before independent reports close.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Every review uses an immutable snapshot and explicit allowed sources.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Review profiles use standard peer-review terminology where applicable.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-16 blueprint/venue profile, CAP-18 draft and CAP-07 model gateway.
- **Downstream:** CAP-19.S02 independent panel and S03 draft intake.
...
- Use `reviewer-simulation.html` as the panel setup/reference.
- Show roles, criteria, context, privacy and model configuration before run.
...
- Drafts stay local by default; remote review requires explicit permission/policy.
- Prompt and output retention follow manuscript confidentiality.
...
|---|---|---|
| reviewer profile lacks applicable criteria | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| snapshot dependency is stale | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| provider policy denies confidential draft | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| two reviewers accidentally share context | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | reviewer, data |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | reviewer |
...
| Review gate | `agent-review` |
| Verification profiles | reviewer, ai, security-local |
...
|---|---|---|
| schema/profile validation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| context isolation test | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| snapshot immutability | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| privacy/egress denial | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| panel configuration interactive | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| snapshot creation content-addressed and deduplicated | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| review context budget declared per role | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-16 blueprint/venue profile, CAP-18 draft and CAP-07 model gateway. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-19.S02 independent panel and S03 draft intake. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-19/index.html`; this slice page is `planning/review-site/CAP-19/CAP-19.S01.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

...
slice_id: CAP-12.S03
title: Institutional identity, authorization, and project isolation
status: proposed
wave: W10
priority: P2
deployment_profiles:
...
| Slice | `CAP-12.S03` — Institutional identity, authorization, and project isolation |
| Slice outcome | University users authenticate through OIDC and access only permitted projects, sources, models, and administrative functions. |
| Wave / priority | `W10` / `P2` |
| Deployment profiles | `UNI` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-12.S03.T01`, `CAP-12.S03.T02`, `CAP-12.S03.T03` |
| Slice dependencies | `CAP-12.S02.T03` |
...
University users authenticate through OIDC and access only permitted projects, sources, models, and administrative functions.
...
**Implementation thesis.** Make institutional authorization a default-deny, cross-plane invariant using federated identity, explicit roles/attributes and data-layer isolation tests.
...
- Rbac/abac policy decision/enforcement.
- Project isolation across all stores and workflows.
...
- **Consumes:** `CAP-12 hosted foundation and connection profiles` and formal dependencies `CAP-12.S02.T03`.
- **Produces:** University users authenticate through OIDC and access only permitted projects, sources, models, and administrative functions.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-12.S02.T03`.
...
| `CAP-12-D05` | Authorization | **Combine role permissions, project/data attributes and default-deny PostgreSQL RLS** | Use UI-hidden buttons as the primary access control | Defense in depth must be enforced at API and data boundaries. | None |
| `CAP-12-D06` | Project isolation | **Carry project and organization scope through every repository, object, vector, workflow and audit operation** | Filter projects only in service-layer queries | Isolation tests must cover every storage and execution plane. | None |
| `CAP-12-D07` | Provisioning readiness | **Model stable subject IDs and groups now; add SCIM only when institutional lifecycle automation is required** | Build a proprietary user directory and manual account lifecycle | Federated identity should remain portable without prematurely mandating provisioning infrastructure. | None |
...
|---|---|
| `services/identity/oidc` | token/session validation |
| `modules/authz` | policy engine and enforcement points |
| `tests/isolation/institutional` | cross-project attack matrix |
...
|---|---|
| `InstitutionalIdentity` | Versioned InstitutionalIdentity state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional identity, authorization, and project isolation. |
| `Session` | Versioned Session state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional identity, authorization, and project isolation. |
| `RoleBinding` | Versioned RoleBinding state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional identity, authorization, and project isolation. |
| `PolicyDecision` | Versioned PolicyDecision state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional identity, authorization, and project isolation. |
| `IsolationEvidence` | Versioned IsolationEvidence state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional identity, authorization, and project isolation. |
...
- `IdentityService.authenticate/refresh/logout` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `AuthorizationService.decide(subject,action,resource,context)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `TenantScopedRepository` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-12 hosted foundation and connection profiles.
- **Downstream:** collaboration, licensed sources and pilot acceptance.
...
- Users can see their active institution, project role and why an action is denied.
- Role changes take effect predictably without stale privilege.
- Administrative impersonation is prohibited or separately privileged/audited.
...
- Validate issuer, audience, nonce, PKCE and token lifetime.
- Use stable subject IDs; do not authorize from mutable display names/email alone.
- Enable and FORCE RLS where applicable; test owner/bypass roles and backup behavior.
...
|---|---|---|
| Revoked/expired session during long workflow | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Missing/malformed group claims | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Cross-project object key or vector filter error | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Policy service unavailable | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| OIDC conformance-oriented token and redirect tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Permission matrix for researcher/reviewer/project admin/rights/model/platform roles | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Repository/object/vector/workflow cross-tenant negative tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Privilege change and session invalidation tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Authorization decision latency bounded and observable | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No security cache outlives source policy/session | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Isolation suite runs on every server release | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-12 hosted foundation and connection profiles. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

collaboration, licensed sources and pilot acceptance. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-12/index.html`; this slice page is `planning/review-site/CAP-12/CAP-12.S03.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

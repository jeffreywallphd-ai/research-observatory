...
slice_id: CAP-11.S01
title: Performance profiles, scale targets, and resource governance
status: proposed
wave: W5
priority: P0
deployment_profiles:
...
| Slice | `CAP-11.S01` — Performance profiles, scale targets, and resource governance |
| Slice outcome | The product has measured local limits and remains responsive under realistic corpus, document, model, and workflow loads. |
| Wave / priority | `W5` / `P0` |
| Deployment profiles | `LOC`, `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-11.S01.T01`, `CAP-11.S01.T02`, `CAP-11.S01.T03` |
| Slice dependencies | `CAP-10.S03.T02`, `CAP-07.S02.T03` |
...
The product has measured local limits and remains responsive under realistic corpus, document, model, and workflow loads.
...
**Implementation thesis.** Turn hardware variability into an explicit, benchmarked product contract and use one resource governor to keep interactive research work responsive under sustained local load.
...
- Repeatable performance/endurance corpus.
- Central cpu/ram/gpu/disk/concurrency governor.
...
- **Consumes:** `CAP-07 model/runtime manifests and CAP-10 representative opportunity workflows` and formal dependencies `CAP-10.S03.T02`, `CAP-07.S02.T03`.
- **Produces:** The product has measured local limits and remains responsive under realistic corpus, document, model, and workflow loads.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-10.S03.T02`, `CAP-07.S02.T03`.
...
|---|---|---|---|---|
| `CAP-11-D01` | Supported hardware profiles | **Publish benchmark-grounded minimum, recommended and high-capability profiles with explicit workload envelopes** | Advertise one generic minimum specification without workload limits | Measured envelopes make unsupported combinations visible and keep release claims evidence-based. | None |
| `CAP-11-D02` | Resource governance | **Use one central adaptive resource governor with interactive-job priority, reservations and pause/resume** | Let each worker independently consume available CPU, RAM and GPU | Central arbitration prevents parser/model jobs from starving the UI and makes low-resource behavior deterministic. | None |
| `CAP-11-D03` | Performance acceptance | **Use versioned representative projects, warm/cold runs, endurance tests and hardware-normalized thresholds** | Use developer anecdotes and one-time stopwatch measurements | Repeatable workloads are required for supportable scale claims and regression detection. | None |
...
|---|---|
| `modules/runtime_profiles` | profile contracts and support evaluation |
| `services/worker/resource_governor` | reservations, priorities and adaptive limits |
| `tests/benchmarks/local_release` | cold/warm/endurance scenarios |
...
|---|---|
| `HardwareProfile` | Versioned HardwareProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to performance profiles, scale targets, and resource governance. |
| `WorkloadEnvelope` | Versioned WorkloadEnvelope state with stable identity, lifecycle/status, provenance and policy metadata appropriate to performance profiles, scale targets, and resource governance. |
| `BenchmarkRun` | Versioned BenchmarkRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to performance profiles, scale targets, and resource governance. |
| `ResourceReservation` | Versioned ResourceReservation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to performance profiles, scale targets, and resource governance. |
| `PressureEvent` | Versioned PressureEvent state with stable identity, lifecycle/status, provenance and policy metadata appropriate to performance profiles, scale targets, and resource governance. |
...
- `ProfileService.evaluate(machine, workload)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ResourceGovernor.acquire/release/pause` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `BenchmarkRunner.execute(manifest)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-07 model/runtime manifests and CAP-10 representative opportunity workflows.
- **Downstream:** CAP-11 reliability and G5 release claims.
...
- Show the active profile, limiting resource and expected workload envelope without alarming ordinary users.
- Expose pause, resume and low-resource mode for long jobs while protecting foreground navigation and document reading.
- Warn before starting unsupported workloads and recommend smaller models, batches or deferred execution.
...
- Benchmark fixtures contain no private research content.
- Hardware telemetry is local and redacted unless a user explicitly exports it.
- Governor decisions cannot bypass project/model/rights policy.
...
|---|---|---|
| Out-of-memory or GPU allocation failure | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Disk pressure during indexing | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Thermal throttling or long-run degradation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Governor crash/restart with held leases | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Review gate | `agent-review` |
| Verification profiles | e2e-local, foundation |
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | e2e-local, service |
...
|---|---|---|
| Profile classification on representative minimum/recommended/high machines | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| P95 interactive latency while parser/model jobs run | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| 24-hour endurance with bounded resource growth | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Recovery of paused/reserved jobs after restart | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| UI action P95 under load ≤ 250 ms excluding deliberate long operations | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No unbounded worker or queue growth; memory high-water marks remain within profile | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Benchmark variance and thresholds recorded per hardware/software manifest | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-07 model/runtime manifests and CAP-10 representative opportunity workflows. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-11 reliability and G5 release claims. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-11/index.html`; this slice page is `planning/review-site/CAP-11/CAP-11.S01.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

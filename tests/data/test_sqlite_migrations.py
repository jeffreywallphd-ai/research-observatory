...
def create_version_8_fixture(database: Path) -> None:
    """Materialize the exact committed v8 profile for dependency migration."""

    create_version_7_fixture(database)
    connection = sqlite3.connect(database, autocommit=True)
    try:
        storage._configure_connection(connection, initialize=True)
        connection.execute("BEGIN IMMEDIATE")
        for statement in storage.WORKFLOW_EXECUTOR_DDL:
            connection.execute(statement)
        connection.execute("DROP TRIGGER schema_metadata_no_update")
        connection.execute("DROP TRIGGER schema_metadata_no_delete")
        connection.execute("ALTER TABLE schema_metadata RENAME TO schema_metadata_v7")
        connection.execute(storage.SCHEMA_METADATA_V8_DDL)
        connection.execute(
            """
            INSERT INTO schema_metadata (
                singleton, schema_version, database_profile, application_id,
            ...
            SELECT singleton, 8, database_profile, application_id, ?, ?, created_at
              FROM schema_metadata_v7
            """,
            (storage.WORKFLOW_EXECUTOR_PROFILE_SHA256, storage.WORKFLOW_EXECUTOR_SCHEMA_SHA256),
        )
        connection.execute("DROP TABLE schema_metadata_v7")
        for statement in v0002_schema_history.SCHEMA_METADATA_TRIGGERS:
            connection.execute(statement)
        connection.execute(
            """
            INSERT INTO schema_migrations (
                ...
                backup_manifest_sha256, source_schema_sha256, target_schema_sha256,
                migration_tool
            ) VALUES ('0008_workflow_executor', 7, 8, ?, ?, ?, ?, 'alembic-1.18.5')
            """,
            (
                CREATED_AT,
                "f" * 64,
        ...
        connection.execute(
            ...
            """,
            ("01890f6e-6a40-7cc5-98b7-123456789ad0", PROJECT_ID, CREATED_AT),
        )
        connection.execute(
            """
            ...
            ) VALUES (?, ?, 'evidence', ?, 0, '1.0.0', ?, ?, 'Legacy output', NULL,
                      'observed', 'unknown')
            """,
            (
                "01890f6e-6a40-7cc5-98b7-123456789ad1",
                "01890f6e-6a40-7cc5-98b7-123456789ad0",
                PROJECT_ID,
                CREATED_AT,
                CREATED_AT,
            ),
        ...
        connection.execute(
            "INSERT INTO evidence (revision_id) VALUES (?)",
            ("01890f6e-6a40-7cc5-98b7-123456789ad1",),
        )
        connection.execute("PRAGMA user_version=8")
        if storage._schema_fingerprint(connection) != storage.WORKFLOW_EXECUTOR_SCHEMA_SHA256:
            raise AssertionError(storage._schema_fingerprint(connection))
        connection.execute("COMMIT")
        checkpoint = tuple(connection.execute("PRAGMA wal_checkpoint(TRUNCATE)").fetchone())
        if checkpoint != (0, 0, 0):
...
def create_version_9_fixture(database: Path) -> None:
    """Materialize the exact committed v9 profile for impact migration."""

    create_version_8_fixture(database)
    connection = sqlite3.connect(database, autocommit=True)
    try:
        storage._configure_connection(connection, initialize=True)
        connection.execute("BEGIN IMMEDIATE")
        for statement in storage.MATERIAL_DEPENDENCY_DDL:
            connection.execute(statement)
        connection.execute(
            """
        ...
        connection.execute("DROP TRIGGER schema_metadata_no_update")
        connection.execute("DROP TRIGGER schema_metadata_no_delete")
        connection.execute("ALTER TABLE schema_metadata RENAME TO schema_metadata_v8")
        connection.execute(storage.SCHEMA_METADATA_V9_DDL)
        connection.execute(
            """
            INSERT INTO schema_metadata (
                singleton, schema_version, database_profile, application_id,
            ...
            SELECT singleton, 9, database_profile, application_id, ?, ?, created_at
              FROM schema_metadata_v8
            """,
            (storage.MATERIAL_DEPENDENCY_PROFILE_SHA256, storage.MATERIAL_DEPENDENCY_SCHEMA_SHA256),
        )
        connection.execute("DROP TABLE schema_metadata_v8")
        for statement in v0002_schema_history.SCHEMA_METADATA_TRIGGERS:
            connection.execute(statement)
        connection.execute(
            """
            INSERT INTO schema_migrations (
                ...
                backup_manifest_sha256, source_schema_sha256, target_schema_sha256,
                migration_tool
            ) VALUES ('0009_material_dependencies', 8, 9, ?, ?, ?, ?, 'alembic-1.18.5')
            """,
            (
                CREATED_AT,
                "f" * 64,
        ...
        if storage._schema_fingerprint(connection) != storage.MATERIAL_DEPENDENCY_SCHEMA_SHA256:
            raise AssertionError(storage._schema_fingerprint(connection))
        connection.execute("COMMIT")
        checkpoint = tuple(connection.execute("PRAGMA wal_checkpoint(TRUNCATE)").fetchone())
        if checkpoint != (0, 0, 0):
...
class SqliteMigrationTests(unittest.TestCase):
    ...
    def test_dry_run_is_exact_and_does_not_create_backup_or_change_source(self) -> None:
        ...
        self.assertEqual(11, plan.target_schema_version)
    ...
    def test_migrates_exact_v1_fixture_with_verified_restorable_backup_and_history(self) -> None:
        ...
        self.assertEqual(11, result.target_schema_version)
        self.assertEqual(
            (
                "0002_schema_history",
                "0003_object_envelopes",
    ...
    def test_migrates_exact_v8_fixture_without_inventing_legacy_dependency_edges(self) -> None:
        create_version_8_fixture(self.database)
        before = sqlite3.connect(self.database, autocommit=True)
        try:
            workflow_tables_before = {
                str(row[0]): int(row[1])
                for row in before.execute(
                    """
                    SELECT name, (
                        SELECT count(*) FROM sqlite_schema AS nested WHERE nested.name=outer.name
                    )
                      FROM sqlite_schema AS outer
        ...
        plan = plan_database_migration(self.database, expected_project_id=PROJECT_ID)
        self.assertEqual(storage.WORKFLOW_EXECUTOR_DATABASE_SCHEMA_VERSION, plan.source_schema_version)
        self.assertEqual(
            (v0009_material_dependencies.revision, v0010_dependency_impacts.revision, v0011_import_previews.revision),
            plan.migration_ids,
        )
        result = migrate_database(self.database, expected_project_id=PROJECT_ID)
        self.assertEqual(
        ...
        try:
            self.assertEqual(
                ("legacy-unreported", None, None),
                tuple(
                    current.execute(
                        """
                        SELECT coverage, registration_event_id, registered_at
    ...
    def test_migrates_exact_v9_fixture_without_inventing_impact_or_stale_state(self) -> None:
        create_version_9_fixture(self.database)

        plan = plan_database_migration(self.database, expected_project_id=PROJECT_ID)
        self.assertEqual(storage.MATERIAL_DEPENDENCY_DATABASE_SCHEMA_VERSION, plan.source_schema_version)
        self.assertEqual((v0010_dependency_impacts.revision, v0011_import_previews.revision), plan.migration_ids)
        self.assertEqual(storage.MATERIAL_DEPENDENCY_SCHEMA_SHA256, plan.source_schema_sha256)

        result = migrate_database(self.database, expected_project_id=PROJECT_ID)
        self.assertEqual((v0010_dependency_impacts.revision, v0011_import_previews.revision), result.migration_ids)
    ...
    def test_fresh_v6_history_remains_current_after_v11_upgrade_and_restart(self) -> None:
        create_fresh_version_6_fixture(self.database)
        projection = runner.migration_framework_projection()
        self.assertEqual(11, projection["targetSchemaVersion"])
        self.assertEqual(v0011_import_previews.revision, projection["revisions"][-1])
    ...
    def test_every_material_revision_step_rolls_back_to_exact_v1_and_retries(self) -> None:
        failpoints = (
            ...
            + tuple(
                (v0009_material_dependencies, step) for step in v0009_material_dependencies.MATERIAL_MIGRATION_STEPS
            )
            + tuple((v0010_dependency_impacts, step) for step in v0010_dependency_impacts.IMPACT_MIGRATION_STEPS)
        )

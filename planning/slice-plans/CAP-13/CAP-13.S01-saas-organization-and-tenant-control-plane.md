...
slice_id: CAP-13.S01
title: SaaS organization and tenant control plane
status: proposed
wave: W11
priority: P3
deployment_profiles:
...
| Slice | `CAP-13.S01` — SaaS organization and tenant control plane |
| Slice outcome | Organizations, regions, plans, policies, and tenant resources are provisioned through auditable lifecycle workflows. |
| Wave / priority | `W11` / `P3` |
| Deployment profiles | `CLD` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-13.S01.T01`, `CAP-13.S01.T02`, `CAP-13.S01.T03` |
| Slice dependencies | `CAP-12.S06.T03` |
...
Organizations, regions, plans, policies, and tenant resources are provisioned through auditable lifecycle workflows.
...
**Implementation thesis.** Create a content-minimizing global control plane that governs organizations, regions, entitlements and lifecycle while routing research content exclusively to selected regional data planes.
...
- Organization/tenant lifecycle and region assignment.
- Control-plane metadata and capability routing.
- Tenant context propagation and audit.
...
- **Consumes:** `CAP-12 remote client and hosted contracts` and formal dependencies `CAP-12.S06.T03`.
- **Produces:** Organizations, regions, plans, policies, and tenant resources are provisioned through auditable lifecycle workflows.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-12.S06.T03`.
...
|---|---|---|---|---|
| `CAP-13-D01` | Control/data plane split | **Keep global control plane free of routine research content; place projects in regional tenant data planes** | Store all tenant content in one global application database | The split supports residency, isolation and smaller blast radius. | ADR-CLOUD-TOPOLOGY |
| `CAP-13-D03` | Tenant context | **Require validated tenant/project identity on every request, event, object key, workflow and metric** | Infer tenancy from UI route or optional query parameters | Tenant context must be mandatory and testable across all planes. | None |
...
|---|---|
| `services/control-plane/organizations` | tenant lifecycle |
| `services/control-plane/routing` | region/data-plane registry |
| `packages/contracts/tenancy` | mandatory context and events |
...
|---|---|
| `Organization` | Versioned Organization state with stable identity, lifecycle/status, provenance and policy metadata appropriate to saas organization and tenant control plane. |
| `Tenant` | Versioned Tenant state with stable identity, lifecycle/status, provenance and policy metadata appropriate to saas organization and tenant control plane. |
| `RegionAssignment` | Versioned RegionAssignment state with stable identity, lifecycle/status, provenance and policy metadata appropriate to saas organization and tenant control plane. |
| `ServicePlan` | Versioned ServicePlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to saas organization and tenant control plane. |
| `TenantContext` | Versioned TenantContext state with stable identity, lifecycle/status, provenance and policy metadata appropriate to saas organization and tenant control plane. |
| `ControlPlaneEvent` | Versioned ControlPlaneEvent state with stable identity, lifecycle/status, provenance and policy metadata appropriate to saas organization and tenant control plane. |
...
- `TenantService.create/suspend/delete` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `RegionRouter.resolve(tenant,project)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `TenantContext.require/propagate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-12 remote client and hosted contracts.
- **Downstream:** regional data planes, identity and billing.
...
- Organization onboarding explains region, isolation tier, retention and provider implications before project creation.
- Global status pages never reveal routine project content.
- Administrative changes show scope, effect and rollback/confirmation.
...
- Control plane stores identifiers, policy and billing metadata—not source documents, evidence, reports or manuscripts.
- Tenant context is cryptographically/session bound and cannot be caller-supplied without validation.
- All lifecycle actions are privileged and audited.
...
|---|---|---|
| Regional plane unavailable during routing | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Tenant context missing or mismatched | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Organization suspension with running workflows | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Control-plane/data-plane version mismatch | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| Content-minimization inventory and tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Tenant context required through APIs/events/jobs | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Region routing and fail-closed behavior | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Lifecycle idempotency and audit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Control-plane availability independent of one data-plane region | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Routing cache bounded and promptly invalidated | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No project content replicated globally | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-12 remote client and hosted contracts. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

regional data planes, identity and billing. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-13/index.html`; this slice page is `planning/review-site/CAP-13/CAP-13.S01.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

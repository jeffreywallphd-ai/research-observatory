...
slice_id: CAP-08.S06
title: Coder comparison, adjudication, and evidence export
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Slice | `CAP-08.S06` — Coder comparison, adjudication, and evidence export |
| Slice outcome | Human plurality and review outcomes are measurable and preserved. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-08.S06.T01`, `CAP-08.S06.T02`, `CAP-08.S06.T03` |
| Slice dependencies | `CAP-08.S05.T03`, `CAP-04.S04.T02` |
...
Human plurality and review outcomes are measurable and preserved.
...
**Implementation thesis.** Support independent coding and transparent adjudication without forcing consensus, compute agreement with known denominators and assumptions, and export evidence tables with complete provenance and rights filtering.
...
- Coder assignments and agreement metrics.
- Adjudication workspace, competing rationale and resolution records.
- Evidence table, disagreement and audit exports.

### 2.2 Explicit non-goals

- Using agreement as a proxy for truth or construct validity.
- Erasing unresolved plurality.
- Exporting restricted source text without permission.
...
- **Consumes:** `CAP-08.S05.T03`, `CAP-04.S04.T02` and the handoffs in Section 13.
- **Produces:** Human plurality and review outcomes are measurable and preserved.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-08.S05.T03`, `CAP-04.S04.T02`.
...
|---|---|
| `services/core/research_observatory/modules/evidence_graph/coding.py` | Assignments, blinding, submissions and comparison. |
| `services/core/research_observatory/modules/evidence_graph/agreement.py` | Metric calculations and assumption checks. |
| `packages/ui/evidence/adjudication` | Side-by-side codings, source, discussion and resolution. |
| `services/core/research_observatory/modules/synthesis_exports/evidence_export.py` | Rights-aware CSV/JSON/appendix exports. |
| `packages/contracts/coding` | Assignment, coding, agreement and adjudication schemas. |
| `tests/benchmarks/agreement` | Known statistical fixtures and export snapshots. |
...
|---|---|
| `CoderAssignment` | record set, fields, protocol/schema, coder, blinding and due/status |
| `CoderRecord` | independent field/relation decisions, source anchors and rationale |
| `AgreementReport` | metric, assumptions, N, missingness, estimate/CI and per-field confusion |
| `AdjudicationRecord` | inputs, discussion/memos, resolution/unresolved state, adjudicator and rationale |
| `EvidenceExportManifest` | snapshot, schema, columns, rights filters, checksums and disclosures |
...
- `CodingService.assign/submit/reveal` enforces blinding state.
- `AgreementService.compute(spec, snapshot)` validates metric assumptions.
- `AdjudicationCommand.resolve/unresolve` preserves all inputs and emits evidence transition.
- `EvidenceExporter.build` operates from an immutable approved snapshot.
...
- Coder workspace shows only assigned schema/source and personal prior entries until reveal policy.
- Agreement report explains statistic and flags sparse/imbalanced categories.
- Adjudication presents source, each coding and alternatives; “remain unresolved” is valid.
- Export preview lists included fields, row counts, source-text restrictions and disclosures.
...
- Roles separate coder/adjudicator/export authorization.
- Blind assignments prevent API/UI leakage of peer labels before submission.
- Exports redact or omit restricted passages and confidential annotations according to policy.
...
|---|---|
| Incomplete assignment | Agreement report states coverage and excludes/handles missing only per selected metric. |
| Metric assumptions invalid | Refuse misleading statistic and offer raw matrix/alternative. |
| Adjudication conflict | Optimistic concurrency and revision; no last-write-wins. |
| Export interrupted | Write temp package, checksums and atomic finalize; no partial final artifact. |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S06.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S06.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
| Review gate | `agent-review` |
| Verification profiles | `evidence`, `security-local` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S06.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
|---|---|
| Agreement | 100,000 coding pairs/fields < 10 s for supported metrics. |
| Adjudication load | p95 < 500 ms with source/codings cached. |
| Export | 100,000 rows to CSV/JSON < 2 min, streaming memory bounded. |
| Integrity | 100% exported files recorded with SHA-256 and manifest. |
...
- agreement by field/metric
- disagreement/adjudication/unresolved
- time to resolution
- export volume/rights exclusions
...
- Consumes Evidence Matrix records and verifier/human decisions.
- Provides adjudicated evidence and agreement disclosure to CAP-09 synthesis/exports and manuscripts.
- Unresolved disagreements remain graph/synthesis inputs as plurality.
...
| Risk | Mitigation |
|---|---|
| Agreement fetishism | Contextual explanation, raw confusion and no quality claim from statistic alone. |
| Consensus erasure | Immutable coder records and unresolved option. |
| Rights breach | Manifested field-level export policy and preview. |

## 19. Required ADRs and human decisions

- ADR-CAP08-CODING: blinding, supported agreement metrics and adjudication policy.
...
- **RFC 8493 - The BagIt File Packaging Format** — Checksum-validated payload transfer and archival package layout.  

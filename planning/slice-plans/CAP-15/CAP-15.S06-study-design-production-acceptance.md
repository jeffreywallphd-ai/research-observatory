...
slice_id: CAP-15.S06
title: Study-design production acceptance
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-15.S06` — Study-design production acceptance |
| Slice outcome | The study-design capability is production-ready, source-grounded, and expert-reviewed. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-15.S06.T01`, `CAP-15.S06.T02`, `CAP-15.S06.T03` |
| Slice dependencies | `CAP-15.S05.T03` |
...
The study-design capability is production-ready, source-grounded, and expert-reviewed.
...
**Implementation thesis.** Approve the empirical study-design foundation only after representative method families move from opportunity evidence to expert-reviewed, nonfabricated protocols with complete provenance and governance.
...
- Opportunity-to-protocol end-to-end acceptance.
- Blinded methods/domain expert evaluation.
- G7 study-design gate evidence.
...
- **Consumes:** `all CAP-15 slices and CAP-10 opportunity dossiers` and formal dependencies `CAP-15.S05.T03`.
- **Produces:** The study-design capability is production-ready, source-grounded, and expert-reviewed.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-15.S05.T03`.
...
|---|---|---|---|---|
| `CAP-15-D02` | Method plurality | **Support quantitative, qualitative, mixed, computational, field and design-science families through extensible method packs** | Use one universal experimental-study schema | The platform serves multiple scholarly traditions and must not impose one epistemology. | None |
| `CAP-15-D05` | Design selection | **Researcher adjudicates, combines or rejects alternatives and records rationale** | Automatically select the highest-scoring design | Methods choice is consequential scholarly judgment. | None |
...
| `CAP-15-D18` | Integrity audit | **Detect missing components, unsupported choices, construct/measure inconsistency, circular logic and undisclosed deviations** | Assign one opaque design-quality score | Actionable findings and evidence are more useful than a scalar verdict. | None |
...
|---|---|
| `tests/e2e/study-design` | quantitative/qualitative/mixed/technical fixtures |
| `research/evaluation/study-design` | expert rubric and analysis |
| `release/g7/study-design` | criterion evidence and limitations |
...
|---|---|
| `StudyDesignAcceptanceRun` | Versioned StudyDesignAcceptanceRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design production acceptance. |
| `ExpertReview` | Versioned ExpertReview state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design production acceptance. |
| `QualityDimension` | Versioned QualityDimension state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design production acceptance. |
| `ResidualLimitation` | Versioned ResidualLimitation state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design production acceptance. |
| `StudyDesignGate` | Versioned StudyDesignGate state with stable identity, lifecycle/status, provenance and policy metadata appropriate to study-design production acceptance. |
...
- `StudyDesignAcceptance.execute` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ExpertEvaluation.record` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `G7StudyDesignGate.evaluate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** all CAP-15 slices and CAP-10 opportunity dossiers.
- **Downstream:** CAP-16 manuscript blueprints, CAP-17 result integration and CAP-18 drafting.
...
- Evaluation includes whether researchers retain authority and can understand/contest recommendations.
- Gate documentation identifies unsupported domains and method packs.
- No protocol is presented as ethically approved or guaranteed valid.
...
- Expert evaluation uses consented/synthetic or appropriately governed projects.
- Blinded artifacts remove unnecessary identifiers.
- Gate bundle excludes private content while retaining verifiable hashes/evidence references.
...
|---|---|---|
| System fabricates a requirement, measure or citation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Expert ratings reveal systematic method bias | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Null/mixed outcome value missing | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Protocol cannot hand off to manuscript/result capabilities | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | study-design, e2e-local |
...
|---|---|---|
| Four representative method-family workflows | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Citation/source and inference-label audit | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Blinded expert appropriateness/tractability/authority ratings | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Failure, cancellation, migration, cross-platform and export checks | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Predeclared expert-quality thresholds and disagreement analysis | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No critical integrity/ethics defect | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Evidence bundle tied to immutable revision and approved plans | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
all CAP-15 slices and CAP-10 opportunity dossiers. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-16 manuscript blueprints, CAP-17 result integration and CAP-18 drafting. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-15/index.html`; this slice page is `planning/review-site/CAP-15/CAP-15.S06.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

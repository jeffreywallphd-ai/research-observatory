...
slice_id: CAP-08.S02
title: Source-grounded extraction pipeline
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Slice | `CAP-08.S02` — Source-grounded extraction pipeline |
| Slice outcome | Schema-constrained extraction selects relevant source context and emits candidate evidence without fabricating absent fields. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-08.S02.T01`, `CAP-08.S02.T02`, `CAP-08.S02.T03` |
| Slice dependencies | `CAP-08.S01.T03`, `CAP-05.S03.T03`, `CAP-07.S04.T02` |
...
Schema-constrained extraction selects relevant source context and emits candidate evidence without fabricating absent fields.
...
**Implementation thesis.** Extract only from selected, immutable and anchored source passages, produce schema-constrained candidates with explicit absence states, and normalize terms as reviewable candidates rather than hidden substitutions.
...
- Section/passages selection and extraction packets.
- Field and relation extraction workflows with validated structured output.
- Normalization/entity-link candidate generation preserving original wording.

### 2.2 Explicit non-goals

- Whole-corpus unsourced summaries.
- Automatic acceptance of extracted records.
- Inventing missing fields from background knowledge.
...
- **Consumes:** `CAP-08.S01.T03`, `CAP-05.S03.T03`, `CAP-07.S04.T02` and the handoffs in Section 13.
- **Produces:** Schema-constrained extraction selects relevant source context and emits candidate evidence without fabricating absent fields.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-08.S01.T03`, `CAP-05.S03.T03`, `CAP-07.S04.T02`.
...
|---|---|---|---|---|
| **Selection strategy** | Deterministic structural/lexical retrieval plus optional semantic reranking selects bounded passages; packet records included and excluded rationale. | Send full PDFs by default. | Bounded context improves cost, privacy and source attribution while preserving reproducibility. | [Synthesizing Scientific Literature with Retrieval-Augmented Language Models](https://doi.org/10.1038/s41586-025-10072-4) |
| **Absence handling** | Every field supports observed value, not reported, unclear, inapplicable and inferred candidate according to schema. | Force a value or use null without meaning. | Missingness and inference are epistemically distinct and analytically important. | [JSON Schema Draft 2020-12](https://json-schema.org/draft/2020-12) |
| **Normalization** | Generate ranked entity-link candidates with lexical/semantic evidence; original source string remains canonical observation until human/rule acceptance. | Replace source language with closest ontology term automatically. | Construct variants and disciplinary terminology must not be collapsed silently. | [PROV-O: The PROV Ontology](https://www.w3.org/TR/prov-o/) |
...
|---|---|
| `services/core/research_observatory/modules/evidence_graph/extraction.py` | Extraction packet and workflow commands. |
| `services/workers/extraction` | Passage selection, gateway calls, validation and normalization candidates. |
| `services/core/research_observatory/modules/evidence_graph/entity_linking.py` | Candidate generation and accepted mappings. |
| `packages/contracts/extraction` | Packet, candidate, field-state and relation schemas. |
| `packages/ui/evidence-matrix` | Batch launch/progress and candidate preview. |
| `tests/benchmarks/extraction` | Gold passage/field/relation and missingness cases. |
...
|---|---|
| `ExtractionPacket` | schema/prompt/model, document revision, selected passages/selectors, selection scores and token budget |
| `ExtractionAttempt` | input hash, model result, validation, status, errors and trace |
| `FieldCandidate` | field, value/state, exact supporting spans, qualifiers and confidence components |
| `RelationCandidate` | typed endpoints, relation, passage evidence, direction and status |
| `EntityLinkCandidate` | source string, candidate term, match signals and resolution |
...
- `PassageSelector.select(document_revision, schema, policy)` returns immutable packet.
- `Extractor.execute(packet)` can only produce validated candidate records.
- `EntityLinker.suggest(candidate, ontology_binding)` never mutates source value.
- Workflow activities checkpoint per document/field and are idempotent by packet hash.
...
- Evidence Matrix launch dialog shows schema, corpus subset, passage strategy, model route, cost/time and privacy mode.
- Progress distinguishes selected, extracted, invalid, canceled and needs-review records.
- Candidate rows provide one-click source inspection and alternative link candidates.
- No field appears “complete” when it is inferred or not reported.
...
- Source text is untrusted data and is delimited from system/tool instructions.
- Rights/egress policy filters extraction packets before model route selection.
- Failed/raw outputs are quarantined and never written as canonical evidence.
...
|---|---|
| No adequate passage | Emit not-found/needs-source-review, not a guessed value. |
| Output validation failure | Bounded repair then failed attempt with diagnostics. |
| Anchor invalid | Reject candidate and route to parsing correction or source review. |
| Batch interruption | Resume uncompleted packet hashes; never duplicate accepted candidates. |
...
| Verification profiles | `evidence`, `search`, `documents` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S02.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `evidence`, `ai`, `service` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S02.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S02.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Define the versioned portable schema and invariants first. Validate examples and negative fixtures; keep provider, database, model, graph, and UI framework types behind adapters.
3. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
4. Pin model, prompt/schema, runtime/provider and evaluation manifests. Enforce local schema validation, source/evidence closure, egress policy and deterministic fallback; treat model output as untrusted candidate state.
...
|---|---|
| Passage selection | p95 < 2 s per ordinary document with local indexes. |
| Batch checkpoint | Persist after each document or <= 30 s of work. |
| Cancellation | Stop scheduling immediately and active model work within gateway limits. |
| Gold extraction | Field-level precision/recall and missingness targets declared per schema before production use. |
...
- selection recall and passage count
- field/relation precision/recall
- not-reported/unclear rates
- invalid output/repair
- entity-link acceptance
- cost/latency by schema/model
...
- Consumes CAP-05 document IR/anchors, CAP-06 retrieval, CAP-07 gateway and S01 schemas.
- Produces candidates for S03 evidence state/persistence and S04 verification.
- Accepted normalized entities feed CAP-09 graph; raw wording remains traceable.
...
| Risk | Mitigation |
|---|---|
| Selection misses evidence | Gold passage recall, multi-route retrieval and human source review. |
| Model fills gaps | Explicit absence states, evidence-span requirement and verifier. |
| Construct collapse | Original wording plus candidate mapping and human acceptance. |

## 19. Required ADRs and human decisions

- ADR-CAP08-EXTRACTION: passage-selection ensemble, default packet limits and repair policy.

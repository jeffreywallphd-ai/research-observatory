...
slice_id: CAP-16.S05
title: Manuscript Blueprint and venue adaptation
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-16.S05` — Manuscript Blueprint and venue adaptation |
| Slice outcome | Researchers can approve a publication-ready article architecture before prose generation. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-16.S05.T01`, `CAP-16.S05.T02`, `CAP-16.S05.T03` |
| Slice dependencies | `CAP-16.S02.T03`, `CAP-16.S03.T03`, `CAP-16.S04.T03` |
...
Researchers can approve a publication-ready article architecture before prose generation.
...
**Implementation thesis.** Provide one approved Blueprint workspace that turns governed manuscript architecture into stable editable skeletons and machine-readable manifests.
...
- Manuscript blueprint workspace.
- Verified venue adaptation and impact preview.
- Section readiness, claim/evidence requirements, budgets and publication artifacts.
- Docx, markdown/quarto, latex, jats and manifest exports.
...
- **Consumes:** `CAP-16.S01-S04 approved patterns and CAP-15 study-design outputs where applicable` and formal dependencies `CAP-16.S02.T03`, `CAP-16.S03.T03`, `CAP-16.S04.T03`.
- **Produces:** Researchers can approve a publication-ready article architecture before prose generation.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-16.S02.T03`, `CAP-16.S03.T03`, `CAP-16.S04.T03`.
...
| `CAP-16-D11` | Length governance | **Model venue/section word budgets as soft or hard constraints with transparent allocation** | Truncate generated prose after drafting | Planning length before prose is safer and more usable than post-hoc deletion. | None |
| `CAP-16-D13` | Figures, tables and disclosures | **Represent figures, tables, supplementary items, data/code statements, ethics and AI disclosures as first-class planned objects** | Add these manually after manuscript drafting | Publication artifacts and integrity statements affect section architecture and export readiness. | None |
| `CAP-16-D14` | Contributor roles | **Capture authorship plus CRediT roles with human confirmation and version history** | Infer final authorship and order from activity logs | Contribution transparency is useful, but authorship remains a human governance decision. | None |
| `CAP-16-D15` | Export toolchain | **Use a replaceable Quarto/Pandoc export service for DOCX, LaTeX, Markdown, JATS, HTML and PDF** | Write custom exporters independently for every format | A mature converter ecosystem reduces format divergence while stable manifests preserve traceability. | ADR-MANUSCRIPT-EXPORT |
| `CAP-16-D17` | Blueprint experience | **Use the approved Manuscript Blueprint workspace with outline, requirements, evidence readiness, budgets and impact preview** | Use a chat-only blueprint workflow | A persistent visual structure is inspectable and easier to adjudicate than conversational state. | None |
...
|---|---|
| `apps/desktop/manuscript-blueprint` | approved Academic Minimal workspace |
| `packages/ui/manuscript` | outline, budget, readiness and provenance components |
| `services/core-api/manuscripts/export` | asynchronous export orchestration |
| `workers/manuscript-export` | pinned Quarto/Pandoc/JATS export adapters |
...
|---|---|
| `BlueprintApproval` | Versioned BlueprintApproval state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint and venue adaptation. |
| `SectionReadiness` | Versioned SectionReadiness state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint and venue adaptation. |
| `VenueAdaptationPreview` | Versioned VenueAdaptationPreview state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint and venue adaptation. |
| `ManuscriptExport` | Versioned ManuscriptExport state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint and venue adaptation. |
| `ContributorRoleAssignment` | Versioned ContributorRoleAssignment state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript blueprint and venue adaptation. |
...
- `Blueprint approval freezes a revision used by CAP-18; later edits create a new revision and stale dependents.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Exported sections carry stable IDs in a structured sidecar even where target formats cannot retain them.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `External submission remains an explicit human action.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-16.S01-S04 approved patterns and CAP-15 study-design outputs where applicable.
- **Downstream:** CAP-18 manuscript project creation and CAP-19 review criteria.
...
- Conform to `manuscript-blueprint.html` in light/dark mode.
- Make next workflow step and blockers obvious for empirical, theory and critical objectives.
- Provide side-by-side generic/venue comparison and export validation results.
...
- Exports enforce rights/confidentiality and disclose omitted restricted content.
- Temporary export workspaces are isolated, cleaned and checksum-audited.
...
|---|---|---|
| Quarto/Pandoc conversion failure | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| venue overlay creates infeasible word budget | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| export loses stable identifier mapping | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| user cancels multi-format export | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Review gate | `agent-review` |
| Verification profiles | manuscript, documents, ai |
...
|---|---|---|
| UI reference and accessibility conformance | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| DOCX/Markdown/LaTeX/JATS/MECA structural golden tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| export manifest/checksum validation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| cancel/retry/restart export tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| interactive outline operations remain under approved latency | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| multi-format export completes within declared profile budget | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| export size and temporary disk use are bounded and cleaned | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-16.S01-S04 approved patterns and CAP-15 study-design outputs where applicable. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

CAP-18 manuscript project creation and CAP-19 review criteria. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-16/index.html`; this slice page is `planning/review-site/CAP-16/CAP-16.S05.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.

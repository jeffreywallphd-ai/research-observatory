...
capability_plan: planning/capability-plans/CAP-15.md
planning_gate: capability-decision-complete
slice_id: CAP-15.S04
title: Analysis, validity, ethics, and reproducibility
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Capability objective | Use literature evidence, opportunity dossiers, domain knowledge, and researcher constraints to propose, compare, and formalize rigorous empirical study designs without displacing scholarly or ethics authority. |
| Slice | `CAP-15.S04` — Analysis, validity, ethics, and reproducibility |
| Slice outcome | The protocol states how evidence will be produced, evaluated, governed, and interpreted under alternative outcomes. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-15.S04.T01`, `CAP-15.S04.T02`, `CAP-15.S04.T03` |
| Slice dependencies | `CAP-15.S03.T03` |
| Capability decision packet | `planning/capability-plans/CAP-15.md` — must be approved and decision-complete |
| Approved experience | `RO-UI-ACADEMIC-MINIMAL-1.3`; relevant references: `study-design.html`, `intent-contract.html`, `evidence-matrix.html`, `audit-lineage.html`, `new-project.html`, `index.html` |
...
The protocol states how evidence will be produced, evaluated, governed, and interpreted under alternative outcomes.

This slice advances the capability objective: **Use literature evidence, opportunity dossiers, domain knowledge, and researcher constraints to propose, compare, and formalize rigorous empirical study designs without displacing scholarly or ethics authority.** It is one production vertical inside a long-running capability campaign, not an isolated experiment. Its implementation must preserve evidence before prose, stable scholarly identity, source and decision provenance, bounded uncertainty, researcher authority, local-first privacy, rights-aware processing, deployment-neutral contracts, cross-platform portability and the distinction between canonical state and rebuildable analytical derivatives.

**Implementation thesis.** Define how evidence will be analyzed, challenged, ethically governed and interpreted under multiple outcomes before results exist.
...
- Quantitative/qualitative/mixed/computational analysis plan.
- Validity, rival explanation, sensitivity and outcome-contingent contribution.
- Ethics, data management, preregistration and reproducibility.
...
- **Consumes:** `sampling/measurement/procedure plan` and formal dependencies `CAP-15.S03.T03`.
- **Produces:** The protocol states how evidence will be produced, evaluated, governed, and interpreted under alternative outcomes.
...
4. Approved `planning/capability-plans/CAP-15.md` for all material capability decisions.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-15.S03.T03`.
- Every slice plan in `CAP-15` exists, matches the backlog and is approved at the same commit as the capability packet.
...
|---|---|---|---|---|
| `CAP-15-D10` | Analysis plan | **Separate primary, secondary and exploratory analyses; record preprocessing, assumptions, diagnostics, missingness and robustness** | Generate a generic list of statistical tests after data collection | A priori analysis structure improves transparency and guards against undisclosed flexibility. | None |
| `CAP-15-D12` | Validity analysis | **Model design-specific validity/quality threats, rival explanations, boundaries and sensitivity strategies** | Use one generic “limitations” text box | Threats must connect to design elements and mitigation evidence. | None |
| `CAP-15-D13` | Outcome-contingent value | **Specify contribution under supported, null, mixed and context-dependent findings** | Assume only a positive result is publishable | A rigorous design should create value without outcome fishing. | None |
| `CAP-15-D14` | Ethics authority | **Generate issues/checklists for human and institutional review; never claim IRB or ethics approval** | Have the AI classify projects as exempt or approved | Human-subject determinations remain with authorized people and institutions. | None |
| `CAP-15-D15` | Data management | **Create FAIR-aware data, metadata, access, retention, sharing and deletion plans with local policy overlays** | Use one generic open-data recommendation | Data governance must account for confidentiality, rights and field norms. | None |
| `CAP-15-D16` | Preregistration | **Export immutable, source-linked preregistration-ready packages; external submission remains an explicit human action** | Automatically submit or alter registrations | OSF registrations are frozen records and may use privacy/embargo controls. | None |
...
|---|---|
| `modules/study_design/analysis` | method-pack plans and diagnostics |
| `modules/study_design/validity` | threats, boundaries and sensitivity |
| `modules/study_design/governance` | ethics, DMP, registration and openness |
...
|---|---|
| `AnalysisPlan` | Versioned AnalysisPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `AnalysisItem` | Versioned AnalysisItem state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `ValidityThreat` | Versioned ValidityThreat state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `SensitivityPlan` | Versioned SensitivityPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `EthicsIssue` | Versioned EthicsIssue state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `DataManagementPlan` | Versioned DataManagementPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `RegistrationPackage` | Versioned RegistrationPackage state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
| `OutcomeScenario` | Versioned OutcomeScenario state with stable identity, lifecycle/status, provenance and policy metadata appropriate to analysis, validity, ethics, and reproducibility. |
...
- `AnalysisPlanner.compile/validate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ValidityService.assess/mitigate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ResearchGovernancePlanner.build` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** sampling/measurement/procedure plan.
- **Downstream:** Study Design Studio, exports and integrity audit.
...
- Separate primary, secondary and exploratory analyses visibly.
- Ethics panels say “requires human/institutional determination,” never “approved” or “exempt.”
- Show how supported, null, mixed and context-dependent outcomes could contribute.
...
|---|---|---|
| Analysis incompatible with design/measurement | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Missing-data or diagnostic plan absent | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Ethics determination inferred by model | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Open-data recommendation conflicts with confidentiality | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | study-design |
...
| Estimate / risk | `M` / `medium` |
| Review gate | `agent-review` |
| Verification profiles | study-design, evidence |
...
| Verification profiles | study-design, security-local |
...
|---|---|---|
| Method-specific plan validators and expert gold cases | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Validity/rival-explanation linkage | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Common Rule/IRB warning and authority tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| OSF-ready immutable export and FAIR metadata checks | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Calculators/simulations have deterministic seeds/manifests | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Completeness checks actionable, not one scalar score | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No external registration/network action during generation | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
sampling/measurement/procedure plan. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

Study Design Studio, exports and integrity audit. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-15/index.html`; this slice page is `planning/review-site/CAP-15/CAP-15.S04.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.
...
1. Run `python tools/planctl.py ready CAP-15 --require-approved`; stop and print the generated review URL if it fails.
...
9. Request independent slice review. If approved, allow `taskctl` to advance directly to the next ready slice in `CAP-15`.

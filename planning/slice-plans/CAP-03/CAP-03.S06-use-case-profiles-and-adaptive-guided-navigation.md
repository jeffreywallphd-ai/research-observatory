...
| Slice | `CAP-03.S06` - Use-case profiles and adaptive guided navigation |
| Slice outcome | A project begins from a scholarly objective and exposes a clear, versioned primary path through the workbench, with visible progress and access to supporting tools. |
| Wave / priority | `W1` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-03.S06.T01`, `CAP-03.S06.T02`, `CAP-03.S06.T03`, `CAP-03.S06.T04`, `CAP-03.S06.T05` |
| Slice dependencies | `CAP-03.S02.T01`, `CAP-00.S06.T04` |
...
Turn the platform from a collection of tools into clear objective-specific scholarly processes while preserving access to the complete research workbench.
...
**Implementation thesis.** Treat each approved use case as a versioned workflow profile that orders stages, routes, checkpoints, outputs, defaults, autonomy, and handoffs. Store the selected profile in the Intent Contract and render adaptive navigation/progress from the machine catalog.
...
### 2.3 Slice boundary
- **Consumes:** `CAP-03.S02.T01`, `CAP-00.S06.T04`.
- **Produces:** A project begins from a scholarly objective and exposes a clear, versioned primary path through the workbench, with visible progress and access to supporting tools.
...
- `planning/backlog.yaml` for `CAP-03.S06` and its task state/dependencies.
...
- `CAP-03.S02.T01` is complete or explicitly gated.
- `CAP-00.S06.T04` is complete or explicitly gated.
...
1. **Use the governed `WORKFLOW_CATALOG.json` as the seed contract and copy the selected profile/version into project authority through the accepted intent revision.**
2. **A profile contains ordered primary stages plus supporting tools; it does not hide or disable valid tools by default.**
3. **Navigation state derives from durable project/workflow state, not from the last page visited.**
4. **Each stage declares entry conditions, expected decisions/outputs, completion evidence, next/previous routes, supporting tools, and recalculation implications.**
5. **Changing profile is an intent revision with impact preview and explicit mapping of completed artifacts.**
6. **All approved workflows receive end-to-end usability and accessibility tests; profile-specific exceptions require governed reference updates.**
...
- WorkflowProfile and StageProfile schemas/registry/version compatibility.
- Project-creation use-case selector and workflow preview.
- Adaptive sidebar, stage stepper, workflow context header, next-action card, supporting-tool inventory, and return-to-workflow affordance.
- Checkpoint/completion projection and workflow handoff service.
- E2E fixture projects for every approved workflow.
...
- Profiles reference stable route/page-contract IDs and capability/output types.
- Stage status: not-started, available, current, in-progress, attention-required, blocked, completed, stale, skipped-with-rationale.
- Project stores selected profile ID/version and any explicit stage override/adjudication.
- Supporting-tool use records context and returns to the originating stage.
...
- Project creation asks the research objective in plain scholarly language and shows the resulting ordered process before creation.
- Primary stages are numbered and visually continuous in navigation and page headers.
- Every stage explains why it is next, what must be decided, and what output will be produced.
- All Tools remains available but visually secondary; opening a support tool retains workflow context.
- Project Home resumes the current stage rather than presenting disconnected cards.
...
- Profiles cannot grant permissions beyond project/intent policy.
- Machine catalog is signed/hashed under the approved UI reference; unapproved runtime profile injection is rejected.
- Profile labels and progress do not leak inaccessible project artifacts.
...
- Unknown/missing profile versions open with compatibility guidance and safe generic navigation.
- A stage projection failure does not alter durable task/artifact state; fallback shows raw workflow status.
- Profile change preserves prior selection and mapping plan until acceptance succeeds.
...
### 9.1 `CAP-03.S06.T01` - Define the versioned use-case and workflow-profile contracts
...
**Risk / review gate:** `high` / `human-and-agent-review`  
...
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S06.T01`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.2 `CAP-03.S06.T02` - Implement primary-use-case selection at project creation and intent revision
...
**Risk / review gate:** `high` / `human-and-agent-review`  
**Verification profiles:** `desktop`, `service`, `data`

**Expected deliverables**
- New-project and intent-revision application flows backed by the workflow-profile contracts.

**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S06.T02`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
### 9.3 `CAP-03.S06.T03` - Implement adaptive ordered navigation and workflow context
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S06.T03`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
5. Run the task verification commands plus targeted unit/contract/integration tests. Produce criterion-to-evidence records tied to the reviewed commit; update contracts, fixtures, documentation, ADRs, and the slice evidence index without adding unrelated work.
...
### 9.4 `CAP-03.S06.T04` - Implement workflow progress, checkpoints, handoffs, and recalculation impact
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S06.T04`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
2. Implement the domain/core path behind the approved port or aggregate boundary. Keep side effects behind adapters, use explicit transaction/idempotency boundaries, and emit provenance/dependency facts atomically where the governing architecture requires them.
3. Implement the desktop interaction using shared Academic Minimal tokens/components and the approved page/workflow contract. Cover keyboard, focus, screen reader, light/dark, loading, empty, offline, denied, error, and recovery states. If the required experience differs materially, stop and update/approve the governed reference before application code.
4. Implement and test the relevant trust boundary explicitly: validate untrusted input, constrain permissions/resources/destinations, redact diagnostics, deny unsupported access, and verify that failure leaves canonical state unchanged or recoverable.
5. Integrate persistence, events/provenance, migration/version metadata, and restart behavior. Exercise the path after process/application restart and against prior-compatible fixtures where applicable.
...
### 9.5 `CAP-03.S06.T05` - Verify all approved use-case workflows end to end
...
**Ordered implementation sequence**
1. Confirm the governing contracts, task dependencies, approved reference (when user-facing), and the specific fixture set for `CAP-03.S06.T05`. Add failing tests for the required success path and at least one material boundary/failure case before production code.
...
|---|---|
| Backlog profiles | `data`, `desktop`, `service` |
...
- Profile resolution and stage projection are cached per relevant project version and update incrementally.
- Navigation renders without waiting for expensive analytical queries.
...
- Record profile selection/change, stage transitions, support-tool detours, checkpoint completion, blocked causes, and workflow abandonment.
- Usability evaluations measure time-to-next-step, navigation errors, support-tool return success, and researcher comprehension.
...
- Consumes CAP-03.S02 intent and CAP-03.S05 impact.
- Every later page/capability declares workflow stage roles in approved page contracts.
- CAP-15-19 add study/manuscript/review workflows through the same governed catalog.
...
- Profile updates are immutable versions; projects remain on their accepted version until explicitly revised.
- Route/page changes require a profile migration map and new approved UI reference.
...
- The promised outcome is demonstrable end to end: A project begins from a scholarly objective and exposes a clear, versioned primary path through the workbench, with visible progress and access to supporting tools.
...
- ADR: Workflow-profile schema, stage completion projection, and version migration.
- ADR: Adaptive navigation state ownership and supporting-tool return context.
...
2. Confirm `CAP-03` is the active capability campaign and `CAP-03.S06` is the next eligible slice.
3. Claim the first READY task in order: `CAP-03.S06.T01`. Do not globally select work outside the capability.

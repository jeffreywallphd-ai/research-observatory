...
slice_id: CAP-12.S02
title: Institutional service and data-plane foundation
status: proposed
wave: W10
priority: P2
deployment_profiles:
...
| Slice | `CAP-12.S02` — Institutional service and data-plane foundation |
| Slice outcome | A deployable server stack implements the shared domain services with production-grade relational, object, vector, and workflow infrastructure. |
| Wave / priority | `W10` / `P2` |
| Deployment profiles | `UNI` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-12.S02.T01`, `CAP-12.S02.T02`, `CAP-12.S02.T03` |
| Slice dependencies | `CAP-12.S01.T01`, `CAP-03.S04.T03` |
...
A deployable server stack implements the shared domain services with production-grade relational, object, vector, and workflow infrastructure.
...
**Implementation thesis.** Provide production hosted adapters for the existing domain and workflow contracts while keeping the first university deployment operable on one institution-controlled server.
...
- Hardened container/service roles and reference deployment.
- Postgresql/s3/qdrant persistence adapters.
- Temporal durable workflow executor.
...
- **Consumes:** `CAP-12 client gateway and CAP-03 portable workflows` and formal dependencies `CAP-12.S01.T01`, `CAP-03.S04.T03`.
- **Produces:** A deployable server stack implements the shared domain services with production-grade relational, object, vector, and workflow infrastructure.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-12.S01.T01`, `CAP-03.S04.T03`.
...
|---|---|---|---|---|
| `CAP-12-D02` | Institutional pilot topology | **Use hardened OCI containers on one institution-controlled server with Compose or Podman; add Kubernetes only by ADR** | Require Kubernetes before the first university pilot | A single-server profile reduces operational burden while preserving container portability. | ADR-INSTITUTIONAL-TOPOLOGY |
| `CAP-12-D03` | Hosted persistence | **Use PostgreSQL, S3-compatible objects, Qdrant and Temporal behind existing ports** | Reuse shared filesystem and ad hoc background jobs | Production adapters preserve local semantics while adding concurrency, durability and operations. | None |
...
|---|---|
| `deploy/institutional/compose` | single-server reference stack |
| `services/adapters/server` | PostgreSQL, S3 and Qdrant |
| `services/workflows/temporal` | portable workflow mapping |
...
|---|---|
| `DeploymentProfile` | Versioned DeploymentProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional service and data-plane foundation. |
| `ServiceCapability` | Versioned ServiceCapability state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional service and data-plane foundation. |
| `HostedRepositoryState` | Versioned HostedRepositoryState state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional service and data-plane foundation. |
| `WorkflowExecution` | Versioned WorkflowExecution state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional service and data-plane foundation. |
| `MigrationRun` | Versioned MigrationRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to institutional service and data-plane foundation. |
...
- `RepositoryPort server implementations` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ObjectStorePort and VectorIndexPort` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `WorkflowExecutor start/signal/cancel/query` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-12 client gateway and CAP-03 portable workflows.
- **Downstream:** institutional identity, collaboration and operations.
...
- Infrastructure is mostly invisible to researchers; user-facing operational surfaces require approved reference work.
- Health and maintenance state is shown to authorized admins without exposing raw data.
- Long-running workflow progress retains the same stages and cancellation semantics as local mode.
...
- OCI images run nonroot with read-only filesystems where feasible and explicit resource limits.
- Secrets enter through platform secret mechanisms, never baked into images.
- Service-to-service and storage connections use authenticated encryption.
...
|---|---|---|
| PostgreSQL/S3/Qdrant unavailable independently | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Temporal activity retries across deployment | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Migration fails halfway | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Single-server restart with queued workflows | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | server, data |
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | server, service |
...
|---|---|---|
| Adapter contract tests reused from local implementations | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Compose/Podman clean install and upgrade | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Workflow replay/versioning/cancellation/human-task tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Consistency scan across relational, object, vector and workflow stores | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Reference pilot fits declared CPU/RAM/disk envelope | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Interactive APIs protected from batch-worker saturation | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Backpressure prevents unbounded workflow/activity queues | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-12 client gateway and CAP-03 portable workflows. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

institutional identity, collaboration and operations. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-12/index.html`; this slice page is `planning/review-site/CAP-12/CAP-12.S02.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

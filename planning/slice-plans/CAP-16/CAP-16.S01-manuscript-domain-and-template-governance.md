...
capability_plan: planning/capability-plans/CAP-16.md
planning_gate: capability-decision-complete
slice_id: CAP-16.S01
title: Manuscript domain and template governance
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Capability objective | Turn approved research intent, literature structures, study plans, and publication goals into governed conference/journal skeletons for empirical, theory, and critical work. |
| Slice | `CAP-16.S01` — Manuscript domain and template governance |
| Slice outcome | Article architecture is versioned, inspectable, and separated from generated prose. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-16.S01.T01`, `CAP-16.S01.T02`, `CAP-16.S01.T03` |
| Slice dependencies | `CAP-09.S06.T03` |
| Capability decision packet | `planning/capability-plans/CAP-16.md` — must be approved and decision-complete |
| Approved experience | `RO-UI-ACADEMIC-MINIMAL-1.3`; relevant references: `manuscript-blueprint.html`, `study-design.html`, `intent-contract.html`, `new-project.html` |
...
Article architecture is versioned, inspectable, and separated from generated prose.

This slice advances the capability objective: **Turn approved research intent, literature structures, study plans, and publication goals into governed conference/journal skeletons for empirical, theory, and critical work.** It is one production vertical inside a long-running capability campaign, not an isolated experiment. Its implementation must preserve evidence before prose, stable scholarly identity, source and decision provenance, bounded uncertainty, researcher authority, local-first privacy, rights-aware processing, deployment-neutral contracts, cross-platform portability and the distinction between canonical state and rebuildable analytical derivatives.

**Implementation thesis.** Create a portable semantic blueprint and governed profile registry before implementing any generated prose or venue-specific rendering.
...
- Manuscript, blueprint, profile, section, block, claim-plan and publication-artifact contracts.
- Generic template registry and verified venue overlays.
- Template/profile versioning, compatibility, impact preview and staleness.
- Jats and export mapping boundaries.
...
- **Consumes:** `CAP-03 durable identity/staleness, CAP-09 reproducibility exports and the approved Research Intent Contract` and formal dependencies `CAP-09.S06.T03`.
- **Produces:** Article architecture is versioned, inspectable, and separated from generated prose.
...
4. Approved `planning/capability-plans/CAP-16.md` for all material capability decisions.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-09.S06.T03`.
- Every slice plan in `CAP-16` exists, matches the backlog and is approved at the same commit as the capability packet.
...
|---|---|---|---|---|
| `CAP-16-D01` | Canonical manuscript model | **Use a versioned semantic ManuscriptBlueprint domain model with stable section/block identities** | Store a disposable outline as free-form Markdown | A canonical model supports staleness, venue overlays, traceable drafting, reviewer comments and multi-format export. | ADR-MANUSCRIPT-DOMAIN |
| `CAP-16-D02` | Identifier strategy | **Use UUIDv7 identities for manuscript, section, block, claim-plan, figure and table objects** | Use array position and heading text as identity | Stable sortable identifiers survive reordering, export and revision and align with the platform identity model. | None |
| `CAP-16-D03` | Template hierarchy | **Use governed generic research-type bases plus versioned venue-profile overlays** | Create one hard-coded template per venue | Layered profiles reduce duplication and preserve generic operation when venue rules are unavailable or change. | ADR-MANUSCRIPT-TEMPLATES |
| `CAP-16-D04` | Venue guidance intake | **Accept official URLs/files and researcher uploads with source, retrieval date, hash, rights and verification status** | Let the model infer venue requirements from its training | Venue requirements are volatile and must be inspectable rather than fabricated. | None |
| `CAP-16-D05` | Interchange model | **Keep the internal semantic model canonical and map to JATS 1.4 as an interoperability target** | Use JATS XML directly as the live editor state | JATS is an excellent exchange standard but is too publisher-oriented to serve as the sole interactive authoring domain. | ADR-MANUSCRIPT-INTERCHANGE |
| `CAP-16-D06` | Template evolution | **Preview structural diffs and require explicit migration; never silently move authored content** | Automatically reshape existing manuscripts when templates change | Researcher-authored content and section meaning must remain under human control. | None |
| `CAP-16-D18` | Venue profile freshness | **Store verification status and expiration/recheck policy and mark dependent blueprints stale when guidance changes** | Treat imported venue instructions as permanently current | Venue instructions change and must participate in dependency propagation. | None |
...
|---|---|
| `packages/contracts/manuscript` | versioned portable schemas and validation |
| `packages/core/manuscript` | blueprint aggregate, invariants and profile resolution |
| `services/core-api/manuscripts` | commands, queries and impact preview |
| `workers/export-profile` | venue source normalization and validation |
| `packages/test-fixtures/manuscript` | generic, venue, migration and invalid-profile fixtures |
...
|---|---|
| `ManuscriptProject` | Versioned ManuscriptProject state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `ManuscriptBlueprint` | Versioned ManuscriptBlueprint state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `VenueProfile` | Versioned VenueProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `TemplateProfile` | Versioned TemplateProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `ManuscriptSection` | Versioned ManuscriptSection state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `ManuscriptBlock` | Versioned ManuscriptBlock state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `ClaimPlan` | Versioned ClaimPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `PublicationArtifactPlan` | Versioned PublicationArtifactPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `ProfileSource` | Versioned ProfileSource state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
| `TemplateMigration` | Versioned TemplateMigration state with stable identity, lifecycle/status, provenance and policy metadata appropriate to manuscript domain and template governance. |
...
- `Profile overlays cannot remove or move accepted author content without an explicit migration decision.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `All section/block identities are stable across reorder and export.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `Venue requirements are either verified source-grounded, researcher-provided/unverified, or unavailable; never model-invented.` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-03 durable identity/staleness, CAP-09 reproducibility exports and the approved Research Intent Contract.
- **Downstream:** empirical, theory and critical blueprints in S02-S04; Blueprint workspace/export in S05; drafting in CAP-18.
...
- Use `manuscript-blueprint.html` as the governed reference.
- Show provenance/freshness for every venue rule and an impact preview before profile migration.
- Keep blueprint approval distinct from template validity and venue verification.
...
**Reference-first rule.** Current reference: `RO-UI-ACADEMIC-MINIMAL-1.3`. Relevant pages: `manuscript-blueprint.html`, `study-design.html`, `intent-contract.html`, `new-project.html`. If planned behavior materially differs, first update the style guide, workflow catalog, page contract and HTML prototype; run reference validators; obtain explicit human approval and a new reference ID; then implement. For CAP-12 or CAP-13 administration surfaces with no existing approved page, the plan may establish nonvisual contracts, but UI implementation remains blocked on this process.

## 7. Security, privacy, rights and research integrity

- Classify uploaded venue guidance and preserve rights/source metadata.
- Do not send private blueprint/venue files to external providers without policy approval.
...
|---|---|---|
| invalid or contradictory venue profile | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| template update conflicts with accepted section content | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| export mapping lacks a required semantic field | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| profile source becomes unavailable or stale | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | manuscript, data |
...
|---|---|---|
| JSON Schema and invariant validation for all manuscript objects | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| template overlay precedence and deterministic resolution | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| JATS 1.4 mapping round-trip for supported structures | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| migration preview and no-silent-restructure regression tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| open a 150-section blueprint in under the approved interactive threshold | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| profile resolution and impact preview remain deterministic and bounded | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| export mapping reports every unsupported field rather than dropping it | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-03 durable identity/staleness, CAP-09 reproducibility exports and the approved Research Intent Contract. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

empirical, theory and critical blueprints in S02-S04; Blueprint workspace/export in S05; drafting in CAP-18. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-16/index.html`; this slice page is `planning/review-site/CAP-16/CAP-16.S01.html`. Automation requesting feedback or approval must provide those links and place resolved recommendations and override controls near the top.
...
1. Run `python tools/planctl.py ready CAP-16 --require-approved`; stop and print the generated review URL if it fails.
...
9. Request independent slice review. If approved, allow `taskctl` to advance directly to the next ready slice in `CAP-16`.

...
slice_id: CAP-13.S02
title: Regional tenant data planes and isolation tiers
status: proposed
wave: W11
priority: P3
deployment_profiles:
...
| Slice | `CAP-13.S02` — Regional tenant data planes and isolation tiers |
| Slice outcome | Tenant data is hosted in declared regions using pooled, dedicated-schema, dedicated-database, or dedicated-deployment isolation as policy requires. |
| Wave / priority | `W11` / `P3` |
| Deployment profiles | `CLD` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-13.S02.T01`, `CAP-13.S02.T02`, `CAP-13.S02.T03` |
| Slice dependencies | `CAP-13.S01.T03`, `CAP-12.S02.T03` |
...
Tenant data is hosted in declared regions using pooled, dedicated-schema, dedicated-database, or dedicated-deployment isolation as policy requires.
...
**Implementation thesis.** Provide regional tenant data planes with selectable isolation tiers and explicit migration, quota, network and storage guarantees instead of an ambiguous shared SaaS boundary.
...
- Shared/isolated/dedicated tier implementation.
- Tenant isolation and tier migration.
...
- **Consumes:** `cloud control plane` and formal dependencies `CAP-13.S01.T03`, `CAP-12.S02.T03`.
- **Produces:** Tenant data is hosted in declared regions using pooled, dedicated-schema, dedicated-database, or dedicated-deployment isolation as policy requires.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-13.S01.T03`, `CAP-12.S02.T03`.
...
|---|---|---|---|---|
| `CAP-13-D02` | Isolation tiers | **Offer shared, isolated and dedicated tiers with explicit guarantees and migration paths** | Use one implicit shared-tenancy model for all customers | Research organizations vary in risk, regulatory and performance requirements. | None |
| `CAP-13-D04` | Orchestration | **Use Kubernetes, GitOps and infrastructure-as-code for managed cloud only** | Extend the university Compose profile manually into production cloud | Managed multi-region SaaS requires declarative scaling and repeatable controls. | ADR-CLOUD-ORCHESTRATION |
| `CAP-13-D05` | Cluster isolation | **Default-deny network policies, namespace/resource quotas and stronger isolated/dedicated options** | Trust application authentication inside a flat cluster network | Kubernetes itself describes multi-tenancy as a spectrum requiring policy, fairness and data-plane controls. | None |
| `CAP-13-D15` | Residency/deletion | **Bind projects to a region, version retention policy and prove deletion across primary, backup and derivative stores** | Treat region as a UI preference | Residency and deletion claims must be technically enforced and auditable. | None |
...
|---|---|
| `deploy/cloud/data-plane` | Kubernetes/GitOps/IaC |
| `platform/tenancy` | namespaces, network, quotas and node/cluster tiers |
| `services/migration/tenant-tier` | export, move and validate |
...
|---|---|
| `DataPlane` | Versioned DataPlane state with stable identity, lifecycle/status, provenance and policy metadata appropriate to regional tenant data planes and isolation tiers. |
| `IsolationTier` | Versioned IsolationTier state with stable identity, lifecycle/status, provenance and policy metadata appropriate to regional tenant data planes and isolation tiers. |
| `TenantNamespace` | Versioned TenantNamespace state with stable identity, lifecycle/status, provenance and policy metadata appropriate to regional tenant data planes and isolation tiers. |
| `TierMigration` | Versioned TierMigration state with stable identity, lifecycle/status, provenance and policy metadata appropriate to regional tenant data planes and isolation tiers. |
| `IsolationTest` | Versioned IsolationTest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to regional tenant data planes and isolation tiers. |
...
- `DataPlaneProvisioner.create/upgrade/delete` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `IsolationPolicy.apply/verify` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `TierMigrationService.plan/execute/rollback` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Downstream:** cloud identity, workers and compliance.
...
- Customers can see selected region/tier and documented guarantees without exposing platform internals.
- Tier changes present expected downtime, cost and verification evidence.
- Researchers experience the same project/workflow semantics after migration.
...
- Default-deny network policy, least-privilege RBAC and quotas for shared tiers.
- Stronger isolated/dedicated tiers use dedicated nodes, clusters, databases or keys as defined.
- Storage/object/vector/workflow resources are tenant scoped and continuously tested.
...
|---|---|---|
| Namespace policy absent or CNI ignores NetworkPolicy | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Noisy neighbor exhausts shared resources | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Tier migration interrupted | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Backup restored into wrong tenant/region | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Estimate / risk | `L` / `high` |
| Review gate | `cloud-architecture-review` |
| Verification profiles | cloud |
...
|---|---|---|
| Cross-tenant API/storage/network/workflow attack matrix | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Quota and fairness tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Shared-to-isolated/dedicated migration with rollback | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Region/tier recovery and deletion evidence | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Per-tier SLO and resource guarantees explicit | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Provisioning/migration time measured | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Shared tenants cannot exceed allocated compute/storage/request budgets | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
cloud control plane. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

cloud identity, workers and compliance. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-13/index.html`; this slice page is `planning/review-site/CAP-13/CAP-13.S02.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

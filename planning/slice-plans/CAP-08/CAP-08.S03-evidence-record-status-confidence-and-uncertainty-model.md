...
slice_id: CAP-08.S03
title: Evidence record, status, confidence, and uncertainty model
status: proposed
wave: W3
priority: P0
deployment_profiles:
...
| Slice | `CAP-08.S03` — Evidence record, status, confidence, and uncertainty model |
| Slice outcome | Extracted content is stored with decomposed certainty and explicit epistemic status. |
| Wave / priority | `W3` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-08.S03.T01`, `CAP-08.S03.T02`, `CAP-08.S03.T03` |
| Slice dependencies | `CAP-08.S02.T03`, `CAP-03.S03.T02` |
...
Extracted content is stored with decomposed certainty and explicit epistemic status.
...
**Implementation thesis.** Persist evidence as immutable, source-grounded assertions whose epistemic state, confidence components, disputes and staleness evolve through explicit transitions instead of overwriting earlier machine or human judgments.
...
- Evidence record persistence, lineage and dependencies.
- State machine for observed/extracted/inferred/verified/disputed/adjudicated/stale.
- Decomposed confidence, uncertainty and missingness representation.

### 2.2 Explicit non-goals

- One confidence probability.
- Deleting rejected candidates from audit history.
- Treating verified as universally true beyond source/context.
...
- **Consumes:** `CAP-08.S02.T03`, `CAP-03.S03.T02` and the handoffs in Section 13.
- **Produces:** Extracted content is stored with decomposed certainty and explicit epistemic status.
...
- All slice dependencies are approved or an explicitly approved integration stub exists: `CAP-08.S02.T03`, `CAP-03.S03.T02`.
...
|---|---|---|---|---|
| **Record identity** | UUIDv7 evidence assertion identity plus immutable revision/attempt IDs; semantic duplicate linking is separate. | Use field/value as primary key. | Multiple independent observations, interpretations and disputes can share values without being the same record. | [RFC 9562 - Universally Unique IDentifiers](https://www.rfc-editor.org/rfc/rfc9562.html) |
| **Transition authority** | Commands enforce allowed state transitions, actor role and required evidence; adjudication creates a new resolution record. | Direct status column edits. | The audit model must explain who changed epistemic status and why. | [PROV-O: The PROV Ontology](https://www.w3.org/TR/prov-o/) |
...
|---|---|
| `services/core/research_observatory/modules/evidence_graph/evidence.py` | Evidence aggregate, states, commands and policies. |
| `services/core/research_observatory/adapters/storage/evidence_repository.py` | Transactional persistence and indexed queries. |
| `packages/contracts/evidence` | Record, status, confidence, missingness and dependency schemas. |
| `services/core/research_observatory/modules/staleness` | Source/schema/model/decision dependency invalidation. |
| `packages/ui/evidence` | Status badges, history and confidence explanation. |
| `tests/domain/evidence` | State-machine and lineage property tests. |
...
|---|---|
| `EvidenceRecord` | subject/predicate/object or field, source anchors, context, schema and original wording |
| `EvidenceRevision` | candidate/accepted values, creator activity and immutable timestamp |
| `EvidenceStatusTransition` | from/to, actor, reason, prerequisites and event |
| `ConfidenceAssessment` | retrieval, anchor, extraction, schema-fit, verification and interpretation components |
| `EvidenceDispute` | competing records/interpretations, participants, rationale and resolution link |
...
- `EvidenceCommand.create_candidate/verify/dispute/adjudicate/mark_stale` validates policy and emits outbox events.
- `EvidenceQuery` returns current view plus complete history.
- `DependencyGraph.link(output, evidence_revision)` drives exact staleness propagation.
- Exports distinguish direct observation from inference and human interpretation.
...
- Status semantics and colors match the approved style guide across Evidence Matrix, source viewer and graphs.
- History drawer explains transitions and dependent outputs.
- Confidence panel lists components, assessor/method and what could reduce uncertainty.
- Disputed alternatives remain side-by-side rather than being hidden by the current view.
...
- Authorization distinguishes read, extract, verify and adjudicate roles.
- Sensitive evidence fields carry classification through query/export/egress.
- Append-only history and tamper-evident ledger references protect accountability.
...
|---|---|
| Illegal transition | Reject with required prerequisite/role and leave state unchanged. |
| Concurrent adjudication | Optimistic version conflict; require refresh/reconciliation. |
| Source/schema change | Mark exact dependents stale, retain prior record and queue review. |
| Partial transaction | Evidence, transition, dependency and outbox commit atomically. |
...
| Verification profiles | `evidence`, `data` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S03.T01`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | `evidence`, `service`, `desktop` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S03.T02`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
...
| Review gate | `agent-review` |
| Verification profiles | `evidence`, `desktop` |
...
1. Confirm the approved capability decision packet, this approved slice plan, dependencies, portable contracts, and criterion-to-evidence IDs for `CAP-08.S03.T03`. Add a failing success-path test and at least one material denial/failure/restart case before production code.
2. Implement the domain/core path behind the selected port, with explicit transaction or idempotency boundaries, bounded resources, durable checkpoints, cancellation, and restart semantics.
...
|---|---|
| Record write | p95 < 100 ms local for ordinary transition transaction. |
| History query | p95 < 250 ms for 100 revisions/transitions. |
| Stale propagation | 10,000 directly/indirectly affected objects identified < 10 s with checkpointed follow-up. |
| Matrix filter | Indexed status/schema/project queries p95 < 500 ms for 100,000 records. |
...
- records by state/schema
- transition/illegal attempts
- dispute/adjudication rates
- confidence component coverage
- stale propagation lag
...
- Consumes extraction candidates and source/schema/model lineage.
- Provides authoritative evidence assertions to verification, matrix, graph, synthesis and novelty.
- Later technical-report results use the same status/provenance pattern without mixing evidence classes.
...
| Risk | Mitigation |
|---|---|
| Status misuse | Domain policy, UI definitions and state-machine tests. |
| Confidence laundering | No aggregate default and provenance on each component. |
| History growth | Indexed immutable events, snapshots for reads, retention without deletion of scholarly decisions. |

## 19. Required ADRs and human decisions

- ADR-CAP08-EVIDENCE: evidence aggregate identity, state machine and confidence components.

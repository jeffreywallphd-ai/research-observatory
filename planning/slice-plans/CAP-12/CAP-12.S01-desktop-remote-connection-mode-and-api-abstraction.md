...
slice_id: CAP-12.S01
title: Desktop remote connection mode and API abstraction
status: proposed
wave: W10
priority: P2
deployment_profiles:
...
| Slice | `CAP-12.S01` — Desktop remote connection mode and API abstraction |
| Slice outcome | The canonical client can connect securely to a university project home while retaining local caches and clear deployment context. |
| Wave / priority | `W10` / `P2` |
| Deployment profiles | `UNI` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-12.S01.T01`, `CAP-12.S01.T02`, `CAP-12.S01.T03` |
| Slice dependencies | `CAP-11.S06.T03` |
...
The canonical client can connect securely to a university project home while retaining local caches and clear deployment context.
...
**Implementation thesis.** Add institution-hosted projects without forking the researcher experience by introducing one versioned client gateway, secure OIDC onboarding and a deliberately bounded local read cache.
...
- Local/remote client gateway and capability negotiation.
- University connection profiles and oidc onboarding.
- Encrypted rights-bounded disconnected-read cache.
...
- **Consumes:** `G5 Windows release and portable Core API contracts` and formal dependencies `CAP-11.S06.T03`.
- **Produces:** The canonical client can connect securely to a university project home while retaining local caches and clear deployment context.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-11.S06.T03`.
...
|---|---|---|---|---|
| `CAP-12-D01` | Client architecture | **Keep one desktop UI and place local/remote implementations behind a versioned client gateway** | Fork the desktop into a separate institutional application | One experience and domain contract prevents semantic drift between deployment profiles. | ADR-REMOTE-CLIENT |
| `CAP-12-D04` | Authentication | **Use system-browser OIDC authorization code with PKCE and short-lived sessions** | Embed university credentials inside the desktop webview | OIDC federation and PKCE are appropriate for native clients and avoid password handling. | ADR-INSTITUTIONAL-IDENTITY |
| `CAP-12-D09` | Offline cache | **Allow encrypted, rights-bounded read cache with explicit expiration and no offline writes by default** | Mirror whole institutional projects locally | Remote projects must preserve rights and conflict guarantees while providing limited resilience. | None |
| `CAP-12-D18` | Service compatibility | **Use explicit API capability negotiation and supported-version windows** | Assume desktop and server are always upgraded together | Independent desktop/server release cycles need safe compatibility behavior. | None |
...
|---|---|
| `packages/client-gateway` | local and remote implementations |
| `packages/ui/connection-profiles` | server and identity onboarding |
| `services/cache/remote-preview` | encrypted cache and invalidation |
...
|---|---|
| `ConnectionProfile` | Versioned ConnectionProfile state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop remote connection mode and api abstraction. |
| `ServerCapabilityManifest` | Versioned ServerCapabilityManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop remote connection mode and api abstraction. |
| `RemoteSession` | Versioned RemoteSession state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop remote connection mode and api abstraction. |
| `CachedProjection` | Versioned CachedProjection state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop remote connection mode and api abstraction. |
| `CachePolicy` | Versioned CachePolicy state with stable identity, lifecycle/status, provenance and policy metadata appropriate to desktop remote connection mode and api abstraction. |
...
- `ClientGateway.call/subscribe` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ConnectionProfileService.validate/connect` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `RemoteCache.store/read/clear` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** G5 Windows release and portable Core API contracts.
- **Downstream:** institutional service foundation and identity enforcement.
...
- Keep the same workflow navigation and clearly label whether a project is local or institution-hosted.
- Show server identity, certificate, policy, capabilities and version compatibility before connection.
- Offline remote mode is read-only by default and names exactly what is cached or unavailable.
...
- Use system-browser OIDC authorization code with PKCE; never collect institutional passwords.
- Pin/validate TLS and prevent arbitrary insecure server URLs.
- Cache rights, expiry and clearing behavior are enforced and visible.
...
|---|---|---|
| Server unreachable or version incompatible | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| OIDC cancellation, timeout or group claim change | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Certificate/policy change after profile creation | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Cached item revoked or expired | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Verification profiles | desktop, server |
...
| Estimate / risk | `L` / `high` |
| Review gate | `agent-review` |
| Verification profiles | desktop, server, security-local |
...
|---|---|---|
| Contract parity tests between local and remote gateways | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| OIDC/PKCE happy, denial, logout and reauthentication flows | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Cache expiry, rights revocation and explicit clearing | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Seamless navigation between local and hosted projects | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Connection and capability negotiation complete within published threshold | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Remote calls support cancellation and bounded retries | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Cache size and age remain policy bounded | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
G5 Windows release and portable Core API contracts. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

institutional service foundation and identity enforcement. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-12/index.html`; this slice page is `planning/review-site/CAP-12/CAP-12.S01.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

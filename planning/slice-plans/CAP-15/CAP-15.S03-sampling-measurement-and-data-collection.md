...
slice_id: CAP-15.S03
title: Sampling, measurement, and data collection
status: proposed
wave: W7
priority: P0
deployment_profiles:
...
| Slice | `CAP-15.S03` — Sampling, measurement, and data collection |
| Slice outcome | The selected design contains an implementable and reviewable empirical data plan. |
| Wave / priority | `W7` / `P0` |
| Deployment profiles | `LOC`, `LAB`, `ALL` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-15.S03.T01`, `CAP-15.S03.T02`, `CAP-15.S03.T03` |
| Slice dependencies | `CAP-15.S02.T03` |
...
The selected design contains an implementable and reviewable empirical data plan.
...
**Implementation thesis.** Convert the selected design logic into an implementable data plan by explicitly connecting populations/cases, sampling rationale, constructs, measures, rights and procedures.
...
- Population/case sampling and recruitment.
- Construct operationalization and measure selection.
- Data collection/intervention/procedure/artifact plan.
...
- **Consumes:** `approved design logic alternative` and formal dependencies `CAP-15.S02.T03`.
- **Produces:** The selected design contains an implementable and reviewable empirical data plan.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-15.S02.T03`.
...
|---|---|---|---|---|
| `CAP-15-D06` | Sampling plan | **Model target population/cases, frame, access, recruitment, inclusion, attrition and power/saturation rationale** | Store only a target sample-size field | Sampling adequacy and generalizability depend on a complete, design-specific plan. | None |
| `CAP-15-D07` | Power and saturation | **Use validated calculators/simulation or qualitative saturation rationale with assumptions and human review** | Let an LLM invent a sample size | Sample rationale must be reproducible and method-appropriate. | None |
| `CAP-15-D08` | Measurement traceability | **Link constructs to definitions, instruments/indicators, psychometric/credibility evidence, licensing and alternatives** | Select measures by semantic similarity alone | Construct validity and rights depend on explicit traceability. | None |
| `CAP-15-D09` | Data collection | **Version procedures, instruments, interventions, timing, checks, interview/observation protocols and computational run manifests** | Keep procedural details only in free-form notes | Implementable protocols require structured operational detail. | None |
...
|---|---|
| `modules/study_design/sampling` | frames, power/saturation and access |
| `modules/study_design/measurement` | construct-measure evidence and rights |
| `modules/study_design/procedures` | instruments, timing and run manifests |
...
|---|---|
| `PopulationOrCaseSet` | Versioned PopulationOrCaseSet state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `SamplingPlan` | Versioned SamplingPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `RecruitmentPlan` | Versioned RecruitmentPlan state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `Measure` | Versioned Measure state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `Operationalization` | Versioned Operationalization state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `InstrumentRights` | Versioned InstrumentRights state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `ProcedureStep` | Versioned ProcedureStep state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
| `CollectionArtifact` | Versioned CollectionArtifact state with stable identity, lifecycle/status, provenance and policy metadata appropriate to sampling, measurement, and data collection. |
...
- `SamplingPlanner.propose/evaluate` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `MeasurementPlanner.map_constructs` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `ProcedurePlanner.compile` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** approved design logic alternative.
- **Downstream:** analysis, validity, ethics and reproducibility.
...
- Surface missing sampling frames, access constraints and assumptions rather than filling them.
- Measure cards show construct fit, evidence, licensing/adaptation and alternatives.
- Procedure timelines reveal dependencies, participant burden and required artifacts.
...
- Never reproduce copyrighted instruments without permission.
- Recruitment and participant data are not collected by the planning feature unless separately governed.
- Power/sample calculations expose formulas, assumptions and software/version.
...
|---|---|---|
| No feasible sampling frame | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Instrument unavailable or license unknown | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Power assumptions unsupported | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Procedure conflicts with selected design or participant burden | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
|---|---|---|
| Sampling/power/saturation cases with known outputs | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Construct-to-measure traceability and rights denials | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Procedure completeness and timing consistency | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Cross-method expert review | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Calculations deterministic and reproducible | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Unknown access/rights remain blockers, not invented defaults | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Design tables remain usable for large multi-study protocols | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
approved design logic alternative. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

analysis, validity, ethics and reproducibility. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-15/index.html`; this slice page is `planning/review-site/CAP-15/CAP-15.S03.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

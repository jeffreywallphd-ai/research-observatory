...
slice_id: CAP-13.S03
title: Cloud identity, entitlement, metering, and billing
status: proposed
wave: W11
priority: P3
deployment_profiles:
...
| Slice | `CAP-13.S03` — Cloud identity, entitlement, metering, and billing |
| Slice outcome | Organizations can govern membership and plans while usage is measured transparently by value-driving resource. |
| Wave / priority | `W11` / `P3` |
| Deployment profiles | `CLD` |
| Platform targets | `platform-neutral` |
| Backlog tasks | `CAP-13.S03.T01`, `CAP-13.S03.T02`, `CAP-13.S03.T03` |
| Slice dependencies | `CAP-13.S01.T02` |
...
Organizations can govern membership and plans while usage is measured transparently by value-driving resource.
...
**Implementation thesis.** Separate federated identity, product entitlement, replayable metering and external billing so access and research semantics do not depend on one payment provider.
...
- Cloud federation and access lifecycle.
- Versioned entitlements/quotas.
- Idempotent usage ledger and billing adapter.
...
- **Consumes:** `control/data-plane tenant foundations` and formal dependencies `CAP-13.S01.T02`.
- **Produces:** Organizations can govern membership and plans while usage is measured transparently by value-driving resource.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-13.S01.T02`.
...
|---|---|---|---|---|
| `CAP-13-D07` | Cloud identity | **Federate OIDC, require MFA policy support and separate human from workload identities** | Maintain local SaaS passwords as the primary identity | Enterprise customers need federation and centralized access policy. | None |
| `CAP-13-D08` | Entitlements | **Use versioned product entitlements and quotas independent of payment-provider objects** | Gate features directly on billing-provider plan IDs | Domain entitlements must survive provider changes and support contracts/grants. | None |
| `CAP-13-D09` | Metering | **Emit idempotent CloudEvents to an append-only usage ledger** | Calculate billable usage from mutable operational metrics | Billing and cost governance need replayable, deduplicated source events. | None |
| `CAP-13-D10` | Cost normalization | **Map cost and usage to FOCUS 1.4-compatible records and dimensions** | Use provider-specific billing schemas throughout the product | A normalized model supports multi-cloud reconciliation and internal cost attribution. | None |
...
|---|---|
| `services/cloud-identity` | OIDC federation and MFA policy |
| `services/entitlements` | grants, plans and quotas |
| `services/metering` | CloudEvents ledger and billing adapters |
...
|---|---|
| `CloudIdentity` | Versioned CloudIdentity state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cloud identity, entitlement, metering, and billing. |
| `EntitlementGrant` | Versioned EntitlementGrant state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cloud identity, entitlement, metering, and billing. |
| `UsageEvent` | Versioned UsageEvent state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cloud identity, entitlement, metering, and billing. |
| `UsageLedgerEntry` | Versioned UsageLedgerEntry state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cloud identity, entitlement, metering, and billing. |
| `InvoiceReference` | Versioned InvoiceReference state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cloud identity, entitlement, metering, and billing. |
| `BillingAdjustment` | Versioned BillingAdjustment state with stable identity, lifecycle/status, provenance and policy metadata appropriate to cloud identity, entitlement, metering, and billing. |
...
- `IdentityBroker.authenticate/link` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `EntitlementService.authorize/consume` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `MeteringService.record/idempotent_replay` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** control/data-plane tenant foundations.
- **Downstream:** cost governance and SaaS launch.
...
- Users can understand organization membership, role, quota and billing-impacting actions.
- Usage views distinguish estimates, normalized cost and invoice state.
- Billing failure does not silently corrupt project entitlements or delete data.
...
- Federation issuer/tenant linking prevents account confusion.
- Metering events are signed/authenticated, immutable and deduplicated.
- Billing provider receives the minimum identity and charge metadata.
...
|---|---|---|
| Duplicate/out-of-order usage events | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Billing provider unavailable | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Entitlement downgraded with running work | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Federated identity collision | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Estimate / risk | `L` / `high` |
| Review gate | `finance-security-review` |
| Verification profiles | cloud |
...
|---|---|---|
| OIDC federation/MFA/session matrix | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Entitlement and quota denial tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Metering replay/deduplication/reconciliation | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Billing adapter sandbox and correction flow | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Authorization remains available during billing outage within explicit grace policy | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Metering lag and reconciliation tolerances declared | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| No mutable metric is the sole billing source | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
control/data-plane tenant foundations. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

cost governance and SaaS launch. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-13/index.html`; this slice page is `planning/review-site/CAP-13/CAP-13.S03.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.

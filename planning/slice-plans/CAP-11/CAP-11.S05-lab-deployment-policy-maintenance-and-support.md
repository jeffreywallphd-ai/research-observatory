...
slice_id: CAP-11.S05
title: Lab deployment, policy, maintenance, and support
status: proposed
wave: W5
priority: P0
deployment_profiles:
...
| Slice | `CAP-11.S05` — Lab deployment, policy, maintenance, and support |
| Slice outcome | A laboratory can manage multiple independent PCs while retaining local project operation and predictable support. |
| Wave / priority | `W5` / `P0` |
| Deployment profiles | `LAB` |
| Platform targets | `windows-x64` |
| Backlog tasks | `CAP-11.S05.T01`, `CAP-11.S05.T02`, `CAP-11.S05.T03` |
| Slice dependencies | `CAP-01.S05.T03`, `CAP-02.S05.T03` |
...
A laboratory can manage multiple independent PCs while retaining local project operation and predictable support.
...
**Implementation thesis.** Enable a lab administrator to deploy and maintain independent local installations without centralizing research projects or weakening user privacy.
...
- Unattended signed deployment and policy.
- Verified model/parser cache seeding.
- Redacted diagnostics and maintenance runbooks.
...
- **Consumes:** `CAP-01 installer and CAP-02 portable project/storage controls` and formal dependencies `CAP-01.S05.T03`, `CAP-02.S05.T03`.
- **Produces:** A laboratory can manage multiple independent PCs while retaining local project operation and predictable support.
...
- Every formal dependency is complete or an explicitly approved integration stub exists: `CAP-01.S05.T03`, `CAP-02.S05.T03`.
...
|---|---|---|---|---|
| `CAP-11-D12` | Lab deployment | **Provide signed unattended deployment, machine policy layers, cache seeding and redacted support bundles** | Require manual per-user installation and configuration | Laboratories need repeatable deployment without centralizing project content. | None |
| `CAP-11-D16` | Installation formats | **Use Tauri-supported signed Windows installer artifacts with enterprise deployment examples** | Distribute unpacked binaries or development commands | Signed installers support ordinary researchers and institutional software distribution. | ADR-WINDOWS-PACKAGING |
...
|---|---|
| `packaging/windows/lab` | silent installer and policy examples |
| `tools/cache_bundle` | manifest, signing and import |
| `tools/support_bundle` | redaction, preview and collection |
...
|---|---|
| `MachinePolicy` | Versioned MachinePolicy state with stable identity, lifecycle/status, provenance and policy metadata appropriate to lab deployment, policy, maintenance, and support. |
| `CacheBundleManifest` | Versioned CacheBundleManifest state with stable identity, lifecycle/status, provenance and policy metadata appropriate to lab deployment, policy, maintenance, and support. |
| `DeploymentAttempt` | Versioned DeploymentAttempt state with stable identity, lifecycle/status, provenance and policy metadata appropriate to lab deployment, policy, maintenance, and support. |
| `SupportBundle` | Versioned SupportBundle state with stable identity, lifecycle/status, provenance and policy metadata appropriate to lab deployment, policy, maintenance, and support. |
| `MaintenanceRun` | Versioned MaintenanceRun state with stable identity, lifecycle/status, provenance and policy metadata appropriate to lab deployment, policy, maintenance, and support. |
...
- `PolicyResolver.merge(vendor,institution,machine,user,project)` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `CacheSeeder.verify/install` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
- `SupportBundle.build/preview/export` — versioned request/response schemas, typed errors, explicit authorization/rights context and idempotent operation identity.
...
- **Upstream:** CAP-01 installer and CAP-02 portable project/storage controls.
- **Downstream:** release candidate and later cross-platform admin patterns.
...
- Machine-enforced settings are visibly distinguished from user preferences.
- Cache seeding reports source, license, size, architecture and verification state.
- Support bundles show every included file/field before export.
...
- Protected policy cannot be overridden by ordinary users.
- Cache bundles are signed/checksummed and restricted to approved models/parsers.
- Support archives omit source documents, prompts and unpublished text by default.
...
|---|---|---|
| Interrupted unattended install | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Cache bundle architecture/version mismatch | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Policy schema invalid or contradictory | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
| Support bundle contains a secret or path identifier | Persist operation state and exact affected identifiers; preserve canonical truth; mark incomplete derivatives stale or rebuildable; show recovery/alternative. | Deterministic fixture plus restart or denial assertion. |
...
| Review gate | `agent-review` |
| Verification profiles | ai, security-local, e2e-local |
...
|---|---|---|
| Clean lab machine deployment and uninstall | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Policy precedence and denial tests | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Offline cache seed and update/rollback | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
| Redaction regression corpus and admin runbook rehearsal | Automated where deterministic; otherwise reviewed protocol with immutable evidence. | Pass declared threshold with no hidden critical/high exception. |
...
|---|---|---|
| Unattended install completes within documented window | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Seed bundle can be verified before copying large payloads | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
| Maintenance leaves independent projects intact | Measure on representative declared profile and store report with manifest. | Threshold accepted in capability packet or benchmark ADR. |
...
CAP-01 installer and CAP-02 portable project/storage controls. The slice validates IDs, revisions, rights/policy state, schema/version compatibility and completeness before accepting the handoff. Missing or stale upstream requirements remain explicit and may block only the affected operation.

### 13.2 Downstream contract

release candidate and later cross-platform admin patterns. The handoff consists of stable portable IDs, revisioned domain objects, evidence/provenance links, manifests, status/uncertainty, policy decisions and documented invalidation triggers—not internal tables, OS paths, cluster objects or SDK types.
...
The static decision page is `planning/review-site/CAP-11/index.html`; this slice page is `planning/review-site/CAP-11/CAP-11.S05.html`. Automation requesting feedback or approval must provide those links and place unresolved options near the top.
